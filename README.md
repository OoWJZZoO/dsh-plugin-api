...
>
> 政策强制闭合约定（2026-09-01）：`security.egress` 对已登记受支持官方路径（llm model discovery、mcp stdio/http transport）自动管治——出站策略是 **denylist**：初始不注册任何策略，未命中 deny 策略的出站目标保持官方原版行为，只有显式 deny 策略拦截；合作型第三方在调用公共接口（`security.egress.register` / `lease.acquire|release`）时受支持，直接绕过（裸网络/进程原语）不在保证内；`executions.recovery` 自动消费单窗口至多一次，合作型求值走 `executions.recovery.evaluate`；`events.define` 提供合作型自定义事件生产（canonical/custom 分域、owner-conflict 确定性拒绝、stale publisher 失效），不提供对抗性同进程隔离；**canonical 系统事件的派发受 producer 判定约束（2026-09-14 收紧）**：`event → producer owner` 映射由 canonical 事件目录承载，非 producer 经门面派发得到 typed `denied` 且事件不被派发，未声明 producer 的条目 fail-closed；第三方自定义事件仍走 `events.define`，语义不变；其余官方路径与具名 edge gap（web/subprocess/terminal/shell/connection/telemetry/llm-provider/remote）的边界见 `docs/specs/plugin-api-policy-enforcement-closure/delivery-report.md`。
...
### M10 域
- **整树收敛交付物**（`docs/specs/plugin-api-m10-contract-convergence/convergence/`）：三张表——消费者行为表（17 项目 + 子包，逐行为的目标公共 API/authority/证据）、公共成员表（由 canonical registry 机械派生的 537 行全量视图）、实现/装配表（逐能力的 owner 包、source/carrier、版本条件、缺位行为、验证入口）——加 `reconciliation.md`（映射对账、M7 B4 五项义务、九线遗留 L1–L21、OBS-14 三项归属、能力缺口登记）与 `standards-pending.md`（待人类确认的分册修订清单）。三表以 registry `publicPath` 为连接键，由 `scripts/convergence-verify.mjs` 机械校验（并在 `npm test` 中执行）。
- `pluginApi.services.appExit`：官方 launcher 提供的进程退出 seam（A 类 advanced passthrough，1:1 直通、门面零附加状态、成员 `optional`）。成员级分级为 **exclusive 本性**、**不进 Composable Profile**（`composableProfile: false`），authority 归官方 launcher 进程生命周期；服务缺席（部署未挂 launcher）⇒ `services.appExit` 面 disabled（`isActive:false`）、`exit()` 抛 typed `PluginApiFeatureDisabledError`；官方服务在场但成员缺失 ⇒ 面保持 active、该成员整个不出现（`optional` 语义）。两种情况门面都**绝不合成** `process.exit`，也不提供优雅关机编排。
- `pluginApi.sessions.interactions`：待处理交互的受限视图与应答（`list({ sessionId?, cursor? })` / `get({ id })` / `respond({ id, action, answer?, reason?, signal? })` / `availability`）。视图字段为 `{ id, kind, sessionId, summary, createdAt, answerShape }`，其中 `id` 是门面铸造的公共身份（不泄漏官方 registry id），`summary` 在出口有界脱敏，视图另带逐 kind `sources`（`approval` 可达、`question` 在本 runtime 诚实 `unavailable`）。pending 集从会话日志的 durable 对 `approval/asked` 减 `approval/decided` 折叠，决策在任何一侧发生都会在下一次读取时消失。应答走**兜底 answerer**：门面以 append 方式注册在官方 `approval/request` 瀑布上（绝不 `prepend`，官方 mux answerer 在场时不会进入），只在被读取过的会话（watch 租约，默认 60s）上持有请求，并**只**在显式 `respond`、请求 abort、租约到期三类事件下结算（后两类使用官方自己的 no-answer 取值 `cancelled`/`unavailable`）——门面不代答、无重试、无默认答案；`answer` 自由答复对 approval 一律 `rejected`。视图与持有共用同一有界 id 注册表，未持有的同 pending 项应答为 `unavailable`（不冒充 `stale`）。
- `pluginApi.sessions.selection`：目标 session 的模型/effort 读取与切换（`get({ sessionId })` / `set({ sessionId, selection, expected?, signal? })` / `availability`）。读取与提交都经 `services.apiProxy` 审计白名单新加的两条**窄成员**（`sessionsModels`/`sessionsSelectModel`，按路径寻址、`optional`，不暴露 `sessions` 整对象、缺成员不连带停用 `downloads`/`respond`）；视图 `{ sessionId, provider, model, effort, revision, source, committedAt, observedAt }` 中 `source` 按可观察证据披露有效值来自哪一层（`committed` / `fallback-logged-request-config` / `fallback-deployment-default` / `unknown`），`committedAt` 只在门面确有提交证据时给出，否则 `null`。`set` 以调用方经 `get` 观察到的 `expected` 做值级比较换（不匹配 ⇒ typed `conflict`，**不触**提交），部分字段变更按官方当前值补齐后单点提交；官方 seam 为 last-write-wins，比较与提交窗口内的残余竞态如实声明。冷 session 读取会经官方 resolver resume（官方读取入口自身的契约），availability 如实报告。
- `pluginApi.workflows`：受控的 workflow 运行入口（`start(request)` / `availability`）。`start` 同步校验请求形状、经调用方 fiber 派生 owner、把 parent 引用经官方 agent registry 校验为 live agent（身份一致比对），再把请求原样交给官方 `workflowEngine`；返回的 holder-owned run 被包装为门面 handle（`{ id, ownerId, meta, status(), observe(listener), result, cancel(reason?), dispose() }`，官方 run id 不另造身份）。终态由引擎裁决并如实映射（`completed → success` 含脚本返回值 / `cancelled → aborted` / `error → error`），`result` never rejects 且恰好结算一次；取消是信号不是终态；`observe` 是对既有 `workflow/*` 事实流的 run-scoped 过滤订阅。引擎仍是唯一执行者与终态 authority：门面零缓存、零全局 run 注册表、零新增事件；`task.start` 不承载第二执行器。
- `pluginApi.sessions.compaction`：受控的主动压缩入口（`run({ agent, mode: 'now' | 'range', range?, signal?, sourceCommandId? })` / `availability`）。门面在副作用前校验目标/模式/范围，经**替代行 provider 的 operation 子面**触发既有压缩引擎（marker + 版本 + 行状态四条件门控，懒解析），把引擎的判别式结果映射为统一 operation 终态（`compacted` 含 lineage / `no-candidate` / `rejected` 带策略 reason / `aborted` / 失败码带 stage），并沿用既有 `manual`/`direct` provenance 词汇。引擎仍是唯一执行者与事实 producer：门面零 `compaction/*` emit、不铸造 operation 身份、不自建锁；门控不通过时 `run()` 抛 typed disabled、`availability()` 诚实报告。
- `pluginApi.credentials`：受控的凭据写入面（`set(ref, value, { expectedRevision? })` / `unset(ref, { expectedRevision? })` / `availability`）。门面在副作用前校验 ref 模式、非空值与官方 backend 可写性（env 影子 → `read-only`），包装官方 provider 的唯一写链，并以**管理存储层**（`source: 'file'`）证据复核「提交后抛错」的诚实结果（`committed` / `unchanged` / `conflict` / `revision-unknown` / `read-only` / `internal`）。并发比较是**声明的门面模拟**（官方 seam 无 CAS）+ per-ref 单飞门，官方写链仍是最终裁决；revision 来自官方 `credentials/updated` 事实且只在进程内。凭据值**绝不**出现在任何结果、日志、审计或快照出口（host 侧先脱敏；无法证明脱敏即省略）。`services.credentials` 只读直通（`resolve`/`describe`）契约原样，白名单不回流 `set`/`unset`。
