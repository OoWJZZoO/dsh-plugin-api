...
test('client regression: the boundary-era and current artifacts agree on every pre-existing face, active and degraded', async () => {
  ...
  const preExisting = (live) => JSON.parse(JSON.stringify({
    ...live,
    featureNames: live.featureNames.slice(0, 9),
    featureActivity: live.featureActivity.slice(0, 9),
  }))
...
test('client independence: the current artifact activates and forwards through the raw module substrate alone', async () => {
  ...
  assert.equal(live.featureNames.length, 16)
  assert.equal(live.featureNames.slice(9).every((name) => name.startsWith('client')), true)
  assert.equal(live.featureActivity.slice(9).every(Boolean), true, 'all seven new faces must be active on the raw substrate')
...
test('host regression: the boundary-era and current hosts agree on every pre-existing face', async () => {
  ...
  try {
    ...
    const currentFace = {
      ...current.face,
      systemPromptMembers: current.face.systemPromptMembers.filter((name) => !helperMembers.includes(name)),
    }
