...
test('apply mounts agent after events and exposes a working registry read API', () => {
  ...
  assert.deepEqual(features.map((f) => f.name), ['tools', 'events', 'agent', 'llm', 'llm/admission', 'session', 'execRoute', 'settings', 'systemPrompt', 'services'])
...
test('agent guard failure disables only agent and keeps facade active', () => {
  ...
  assert.equal(features.find((f) => f.name === 'settings')?.isActive, true)
  assert.equal(features.find((f) => f.name === 'systemPrompt')?.isActive, true)
  assert.equal(features.find((f) => f.name === 'services')?.isActive, true)
