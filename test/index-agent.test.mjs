...
test('apply mounts agent after events and exposes a working registry read API', () => {
  ...
  assert.deepEqual(features.map((f) => f.name), ['events', 'agent', 'web', 'llm/admission'])
...
test('agent guard failure disables only agent and keeps facade active', () => {
  ...
  assert.equal(features[1].name, 'agent')
  assert.equal(features[1].isActive, false)
  assert.match(features[1].reason, /agents\.get/)
  ...
  assert.match(features[3].reason, /agents\.get/)
