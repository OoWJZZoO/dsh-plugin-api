...
test('events guard failure disables only events and keeps facade active', () => {
  ...
  assert.equal(features[17].name, 'recovery')
  assert.equal(features[17].isActive, true)
  assert.equal(features[18].name, 'coordination')
  assert.equal(features[18].isActive, true)
  assert.equal(features[19].name, 'diagnostics')
  assert.equal(features[19].isActive, true)
  assert.equal(features[20].name, 'usage')
  assert.equal(features[20].isActive, true)
  assert.equal(features[21].name, 'toolDiscovery')
