...
test('events guard failure disables only events and keeps facade active', () => {
  ...
  assert.deepEqual(features[10].name, 'sessionRoute')
  ...
  assert.equal(features[17].name, 'execution')
  assert.equal(features[17].isActive, true)
  assert.equal(features[19].name, 'coordination')
  assert.equal(features[19].isActive, true)
  assert.equal(features[20].name, 'workspaceTransactions')
  assert.equal(features[20].isActive, true)
  assert.equal(features[21].name, 'diagnostics')
  assert.equal(features[21].isActive, true)
  assert.equal(features[22].name, 'usage')
  assert.equal(features[22].isActive, true)
  assert.equal(features[23].name, 'tasks')
...
test('web service absence keeps the services feature active while disabling only the web facade', () => {
  ...
  assert.equal(features[17].name, 'execution')
  assert.equal(features[17].isActive, true)
  assert.equal(features[18].name, 'recovery')
