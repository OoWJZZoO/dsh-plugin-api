...
test('satisfiesContract is true when the actual generation matches and increment is at least required', () => {
  assert.equal(satisfiesContract('0.1', '0.1.0-rc.6'), true)
  assert.equal(satisfiesContract('0.1', '0.1.9'), true)
  assert.equal(satisfiesContract('0.1', '0.2.0'), true)
  assert.equal(satisfiesContract('0.1', '0.10.0'), true)
})

test('satisfiesContract is false when generations differ or the actual increment is smaller', () => {
  assert.equal(satisfiesContract('1.0', '0.1.0-rc.6'), false)
  assert.equal(satisfiesContract('0.2', '0.1.9'), false)
})
...
test('parseFacadeVersion splits the full unique version into runtime, api and maintenance parts', () => {
  assert.deepEqual(parseFacadeVersion('0.1.0-rc.6-0.1.0'), { runtime: '0.1.0-rc.6', api: '0.1', maintenance: '0' })
  assert.deepEqual(parseFacadeVersion('0.1.0-rc.6-0.7'), { runtime: '0.1.0-rc.6', api: '0.7' })
  ...
  assert.deepEqual(parseFacadeVersion('1.2.3-rc.1-0.10.3'), { runtime: '1.2.3-rc.1', api: '0.10', maintenance: '3' })
  assert.deepEqual(parseFacadeVersion('0.1.0-rc.6-0.2.1'), { runtime: '0.1.0-rc.6', api: '0.2', maintenance: '1' })
  assert.deepEqual(parseFacadeVersion('0.1.0-rc.6-0.7.3'), { runtime: '0.1.0-rc.6', api: '0.7', maintenance: '3' })
})
...
test('parseFacadeVersion returns null for unparseable input', () => {
  ...
  assert.equal(parseFacadeVersion('0.1.0-rc.6-0.2-x'), null)
  assert.equal(parseFacadeVersion('0.1.0-rc.6-0.7.3.4'), null)
})
