...
test('facade-owned registration handles expose owner, generation and identity-bound dispose', async () => {
  ...
  const handle = service.llm.adapters.decorations.register({ id: 'adapter-1' })
  assert.ok(handle, 'facade-owned decoration registration returns a handle')
  ...
  const reclaimed = handle.dispose()
  assert.equal(reclaimed.ok, true, 'dispose delegates to the facet')
  assert.equal(reclaimed.code, 'revoked')
})
