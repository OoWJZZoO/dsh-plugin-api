...
assert.equal(names[names.length - 27], 'remote', 'remote stays directly before execution')
  ...
  assert.equal(names[names.length - 13], 'sessionActivity', 'session activity projection appends in the integration wave tail')
  ...
  assert.equal(names[names.length - 10], 'sessionInteraction', 'session interaction operation appends in the integration wave tail')
  assert.equal(names[names.length - 9], 'attention', 'attention appends in the integration wave tail')
  assert.equal(names[names.length - 8], 'checkpoints', 'checkpoints stays directly before decision participation')
  assert.equal(names[names.length - 7], 'decisionParticipation', 'decision participation is the last FEATURE_MOUNTERS entry')
