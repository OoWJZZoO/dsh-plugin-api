...
test('apply mounts remote then the observation faces (execution, diagnostics) and exposes pluginApi.remote', () => {
  ...
  assert.equal(names[names.length - 7], 'remote', 'remote stays directly before execution')
  assert.equal(names[names.length - 6], 'execution', 'execution stays directly before recovery')
  assert.equal(names[names.length - 5], 'recovery', 'recovery stays directly before coordination')
  assert.equal(names[names.length - 4], 'coordination', 'coordination stays directly before diagnostics')
  assert.equal(names[names.length - 3], 'diagnostics', 'diagnostics stays directly before usage')
  assert.equal(names[names.length - 2], 'usage', 'usage mounts directly before toolDiscovery')
