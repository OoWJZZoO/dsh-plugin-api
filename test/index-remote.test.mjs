...
assert.equal(names[names.length - 25], 'remote', 'remote stays directly before execution')
  ...
  assert.equal(names[names.length - 11], 'sessionActivity', 'session activity projection appends in the integration wave tail')
  ...
  assert.equal(names[names.length - 8], 'sessionInteraction', 'session interaction operation appends in the integration wave tail')
  assert.equal(names[names.length - 7], 'attention', 'attention appends in the integration wave tail')
  assert.equal(names[names.length - 6], 'checkpoints', 'checkpoints stays directly before decision participation')
  assert.equal(names[names.length - 5], 'decisionParticipation', 'decision participation is the last FEATURE_MOUNTERS entry')
