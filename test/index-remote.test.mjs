...
test('apply mounts remote then the observation faces (execution, diagnostics) and exposes pluginApi.remote', () => {
  ...
  assert.equal(names[names.length - 5], 'remote', 'remote stays directly before execution')
  assert.equal(names[names.length - 4], 'execution', 'execution stays directly before recovery')
  assert.equal(names[names.length - 3], 'recovery', 'recovery stays directly before diagnostics')
