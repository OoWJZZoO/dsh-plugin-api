...
test('apply mounts remote then the observation faces (execution, diagnostics) and exposes pluginApi.remote', () => {
  ...
  assert.equal(names[names.length - 3], 'remote', 'remote stays directly before execution')
  assert.equal(names[names.length - 2], 'execution', 'execution mounts before diagnostics')
  assert.equal(names[names.length - 1], 'diagnostics', 'diagnostics is the last FEATURE_MOUNTERS entry')
