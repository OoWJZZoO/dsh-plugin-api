...
test('apply mounts remote then the observation faces (execution, diagnostics) and exposes pluginApi.remote', () => {
  ...
  assert.equal(names[names.length - 11], 'remote', 'remote stays directly before execution')
  assert.equal(names[names.length - 10], 'execution', 'execution stays directly before recovery')
  assert.equal(names[names.length - 9], 'recovery', 'recovery stays directly before coordination')
  assert.equal(names[names.length - 8], 'coordination', 'coordination stays directly before workspaceTransactions')
  assert.equal(names[names.length - 7], 'workspaceTransactions', 'workspaceTransactions stays directly before diagnostics')
  assert.equal(names[names.length - 6], 'diagnostics', 'diagnostics stays directly before usage')
  assert.equal(names[names.length - 5], 'usage', 'usage stays directly before tasks')
  assert.equal(names[names.length - 4], 'tasks', 'tasks stays directly before toolDiscovery')
  assert.equal(names[names.length - 3], 'toolDiscovery', 'tool discovery mounts directly before context')
