...
assert.equal(names[names.length - 22], 'remote', 'remote stays directly before execution')
  assert.equal(names[names.length - 21], 'execution', 'execution stays directly before recovery')
  assert.equal(names[names.length - 20], 'recovery', 'recovery stays directly before coordination')
  assert.equal(names[names.length - 19], 'coordination', 'coordination stays directly before workspaceTransactions')
  assert.equal(names[names.length - 18], 'storage', 'storage mounts directly before workspaceTransactions')
  assert.equal(names[names.length - 17], 'workspaceTransactions', 'workspaceTransactions stays directly before diagnostics')
  assert.equal(names[names.length - 16], 'diagnostics', 'diagnostics stays directly before tasks')
  assert.equal(names[names.length - 15], 'tasks', 'tasks stays directly before toolDiscovery')
  assert.equal(names[names.length - 14], 'toolDiscovery', 'tool discovery mounts directly before skillsActivation')
  assert.equal(names[names.length - 13], 'skillsActivation', 'skills activation mounts directly before context')
  assert.equal(names[names.length - 12], 'context', 'context mounts directly before profile')
  assert.equal(names[names.length - 11], 'profile', 'profile stays directly before llmAdapters')
  assert.equal(names[names.length - 10], 'llmAdapters', 'adapter decoration mounts directly before sessionChannel')
  assert.equal(names[names.length - 9], 'sessionChannel', 'sessionChannel stays directly before sessionActivity')
  assert.equal(names[names.length - 8], 'sessionActivity', 'session activity projection appends in the integration wave tail')
  assert.equal(names[names.length - 7], 'sessionPlanMode', 'plan mode control appends after session activity')
  assert.equal(names[names.length - 6], 'sessionPermissionPresets', 'permission preset control appends after plan mode control')
  assert.equal(names[names.length - 5], 'sessionInteraction', 'session interaction operation appends in the integration wave tail')
  assert.equal(names[names.length - 4], 'attention', 'attention appends in the integration wave tail')
  assert.equal(names[names.length - 3], 'checkpoints', 'checkpoints stays directly before decision participation')
  assert.equal(names[names.length - 2], 'decisionParticipation', 'decision participation is the last FEATURE_MOUNTERS entry')
