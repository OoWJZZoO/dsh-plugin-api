...
function createFakeExecutor({
  handshakeShapes = { builtForRuntime: '0.1.0-rc.6', apiProtocol: '0.5' },
  ...
  const body = [
    ...
    '  write({ type: "result", outcome: env.FAKE_HANDSHAKE_OUTCOME ?? "success", result: { builtForRuntime: env.FAKE_RUNTIME ?? "0.1.0-rc.6", apiProtocol: env.FAKE_API ?? "0.5" } })',
    '  process.exit(0)',
    '}',
...
test('a stalled executor settles error with a timeout reason classification', async () => {
  ...
  writeFileSync(script, [
    ...
    'if (command === "handshake") {',
    '  process.stdout.write(JSON.stringify({ type: "result", outcome: "success", result: { builtForRuntime: "0.1.0-rc.6", apiProtocol: "0.5" } }) + "\\n")',
    '  process.exit(0)',
    '}',
