...
test('slice D: a declarative storage domain carries the plugin business records', async () => {
  ...
  assert.equal(opened[0].name, `workspace__${result.envelope.owner}__todos`, 'the official unit is owner/scope namespaced')
  assert.notEqual(result.envelope.owner, 'pluginx', 'the derived caller identity replaces the caller-supplied owner label')
