...
test('combined host publishes additive immutable M2 shapes once without synthetic public surfaces', () => {
  ...
  assert.deepEqual(state.listeners.map(({ name }) => name).sort(), ['llm/stream', 'session/event', 'tools/pre-execute'])
...
test('throwing mandatory substrate and logger disable only dependent M2 surfaces while boot continues', () => {
...
test('durable retained references, repeated cleanup, and stale cleanup cannot affect a fresh epoch or unrelated LLM', () => {
  ...
  assert.equal(state.listeners.filter(({ name }) => name === 'session/event').length, 1)
})
