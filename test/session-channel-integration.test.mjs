...
test('facade: channel registrations carry the derived caller identity', async () => {
  ...
  // An illegal declaration — including a missing or non-object spec — fails
  // typed, never as a bare destructuring TypeError.
  assert.throws(() => first.redaction.register(), (error) => error.code === 'PLUGIN_API_CHANNEL_REDACTION_SPEC_INVALID')
  assert.throws(() => first.redaction.register(null), (error) => error.code === 'PLUGIN_API_CHANNEL_REDACTION_SPEC_INVALID')
  assert.equal(typeof first.auth.pairingProvider?.register, 'function', 'the pairing provider entry is published')
  assert.throws(() => first.auth.pairingProvider.register(), (error) => error.code === 'PLUGIN_API_CHANNEL_AUTH_SPEC_INVALID')
  assert.throws(() => first.auth.pairingProvider.register(null), (error) => error.code === 'PLUGIN_API_CHANNEL_AUTH_SPEC_INVALID')
  assert.throws(() => first.auth.register(), (error) => error.code === 'AUTH_REGISTER_INVALID_INPUT')
