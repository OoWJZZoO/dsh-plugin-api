...
test('observe returns a frozen projection with state, active attempt, run links, owner, terminal outcome, and provenance availability', async () => {
  ...
  assert.equal(typeof subscription.taskId, 'string')
  assert.ok(subscription.initialState)
  assert.deepEqual(Object.keys(subscription).sort(), ['current', 'dispose', 'epoch', 'initialState', 'subscribe', 'taskId'])
  assert.ok(Object.isFrozen(subscription), 'the public handle is frozen')
  assert.equal('listeners' in subscription, false, 'the internal listener set never escapes')
  assert.equal('disposed' in subscription, false, 'the internal liveness flag never escapes')
  assert.equal('signal' in subscription, false, 'the internal abort signal never escapes')
  assert.equal('abortHandler' in subscription, false, 'the internal abort handler never escapes')
  ...
  // a released handle still answers a typed read and a no-op subscription
  const afterRelease = await subscription.current()
  assert.equal(afterRelease.code, 'inactive')
...
test('unavailable/truncated evidence is stated explicitly and never claims a complete history', async () => {
  ...
  const current = await subscription.current()
  // the registry proved the record is absent: `missing`, not `unavailable`
  ...
  assert.equal(history.code, 'missing')
  const got = await owner.api.get('task-void')
  assert.equal(got.ok, false)
  assert.equal(got.code, 'missing')
  assert.equal('found' in got, false)
})
