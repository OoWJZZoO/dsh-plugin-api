...
function createMountHarness({ services = new Map(), marker = false, manifest = null, facadeContract = { runtime: '0.1.0-rc.6', api: '0.5' } } = {}) {
  const registry = {
    snapshot: () => [],
    isActive: () => false,
    disable() {},
  ...
  const Service = createPluginApiService({ apiVersion: '0.5', registry, coreActive: true })
  const ctx = {
    fiber: { uid: 1, state: 2 },
...
test('replacement active: pluginApi.session.branches forwards operations and availability', () => {
  ...
  const { service, featureRegistry, logger, auxiliaryManifests, facadeContract } = createMountHarness({
    ...
    manifest: { version: '0.1.0-rc.6-0.5', api: '0.5' },
  })
...
test('facade inactive: branch add-on rejects with inactive/feature-disabled typing and availability is honest', () => {
  const { service } = createMountHarness({ manifest: { version: '0.1.0-rc.6-0.5', api: '0.5' } })
