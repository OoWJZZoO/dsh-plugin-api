...
test('ability-anchor finite append mapping appends only supported messages without hand-authored surface metadata', () => {
  ...
  const user = service.sessions.durable.appendMessage(session, 'user/message', message(
    'user-1',
    'user',
    { kind: 'user' },
  ...
  const assistant = service.sessions.durable.appendMessage(session, 'assistant/message', {
    turn: 1,
    step: 1,
    message: message(
      'assistant-1',
  ...
  const result = service.sessions.durable.appendMessage(session, 'tool/result', {
    turn: 1,
    step: 1,
    message: message(
      'tool-1',
...
test('ability-anchor mapping rejects title, replacement, and atomic-turn attempts', () => {
  ...
  for (const [kind, payload, options] of [
    ...
    assert.throws(
      () => service.sessions.durable.appendMessage(session, kind, payload, options),
  ...
  assert.throws(() => service.sessions.durable.appendMessage(null, 'user/message', user), (error) => {
