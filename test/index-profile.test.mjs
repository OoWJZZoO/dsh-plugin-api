...
test('apply mounts profile last with a working composed surface', () => {
  ...
  assert.equal(features[features.length - 9].name, 'profile')
  assert.equal(features[features.length - 9].isActive, true)
  assert.equal(features[features.length - 8].name, 'llmAdapters')
  assert.equal(features[features.length - 8].isActive, false)
  assert.equal(features[features.length - 7].name, 'sessionChannel')
  assert.equal(features[features.length - 6].name, 'sessionActivity')
