...
test('apply mounts profile last with a working composed surface', () => {
  ...
  assert.equal(features[features.length - 16].name, 'profile')
  assert.equal(features[features.length - 16].isActive, true)
  assert.equal(features[features.length - 15].name, 'llmAdapters')
  assert.equal(features[features.length - 15].isActive, false)
  assert.equal(features[features.length - 14].name, 'sessionChannel')
  assert.equal(features[features.length - 13].name, 'sessionActivity')
  assert.equal(features[features.length - 12].name, 'sessionPlanMode')
  assert.equal(features[features.length - 11].name, 'sessionPermissionPresets')
  assert.equal(features[features.length - 10].name, 'sessionInteraction')
  assert.equal(features[features.length - 9].name, 'attention')
  assert.equal(features[features.length - 8].name, 'checkpoints')
  assert.equal(features[features.length - 7].name, 'decisionParticipation')
  assert.equal(features[features.length - 6].name, 'scopedAgentContributions')
  assert.equal(features[features.length - 5].name, 'credentials')
  assert.equal(features[features.length - 4].name, 'sessionCompaction')
