...
test('apply mounts profile last with a working composed surface', () => {
  ...
  assert.equal(features[features.length - 8].name, 'profile')
  assert.equal(features[features.length - 8].isActive, true)
  assert.equal(features[features.length - 7].name, 'llmAdapters')
  assert.equal(features[features.length - 7].isActive, false)
  assert.equal(features[features.length - 6].name, 'sessionChannel')
  assert.equal(features[features.length - 5].name, 'sessionActivity')
  assert.equal(features[features.length - 4].name, 'sessionInteraction')
  assert.equal(features[features.length - 3].name, 'attention')
  assert.equal(features[features.length - 2].name, 'checkpoints')
  assert.equal(features[features.length - 1].name, 'decisionParticipation')
