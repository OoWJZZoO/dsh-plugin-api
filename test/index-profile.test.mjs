...
test('apply mounts profile last with a working composed surface', () => {
  ...
  assert.equal(features[features.length - 10].name, 'profile')
  assert.equal(features[features.length - 10].isActive, true)
  assert.equal(features[features.length - 9].name, 'llmAdapters')
  assert.equal(features[features.length - 9].isActive, false)
  assert.equal(features[features.length - 8].name, 'sessionChannel')
  assert.equal(features[features.length - 7].name, 'sessionActivity')
  assert.equal(features[features.length - 6].name, 'sessionPlanMode')
