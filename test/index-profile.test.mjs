...
test('apply mounts profile last with a working composed surface', () => {
  ...
  assert.equal(features[features.length - 14].name, 'profile')
  assert.equal(features[features.length - 14].isActive, true)
  assert.equal(features[features.length - 13].name, 'llmAdapters')
  assert.equal(features[features.length - 13].isActive, false)
  assert.equal(features[features.length - 12].name, 'sessionChannel')
  assert.equal(features[features.length - 11].name, 'sessionActivity')
  assert.equal(features[features.length - 10].name, 'sessionPlanMode')
  assert.equal(features[features.length - 9].name, 'sessionPermissionPresets')
  assert.equal(features[features.length - 8].name, 'sessionInteraction')
  assert.equal(features[features.length - 7].name, 'attention')
  assert.equal(features[features.length - 6].name, 'checkpoints')
  assert.equal(features[features.length - 5].name, 'decisionParticipation')
  assert.equal(features[features.length - 4].name, 'scopedAgentContributions')
  assert.equal(features[features.length - 3].name, 'credentials')
  assert.equal(features[features.length - 2].name, 'sessionCompaction')
