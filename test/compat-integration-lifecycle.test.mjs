...
test('combined host publishes additive immutable compat shapes once without synthetic public surfaces', () => {
  ...
  assert.deepEqual(state.listeners.map(({ name }) => name).sort(), [
    'agent/error', 'agent/request', 'fs/edit-intent', 'fs/write-intent',
    ...
    'subagent/end', 'subagent/start', 'tools/execute', 'tools/pre-execute',
    'tools/pre-execute', 'tools/pre-execute', 'tools/result', 'workflow/end', 'workflow/start',
  ])
