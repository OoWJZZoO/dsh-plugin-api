...
test('combined host publishes additive immutable compat shapes once without synthetic public surfaces', () => {
  ...
  assert.deepEqual(state.listeners.map(({ name }) => name).sort(), ['agent/error', 'agent/request', 'approval/request', 'llm/stream', 'llm/stream', 'llm/stream', 'llm/stream', 'session/created', 'session/disposed', 'session/event', 'session/event', 'session/event', 'tools/execute', 'tools/post-execute', 'tools/pre-execute', 'tools/pre-execute', 'tools/pre-execute', 'tools/result'])
