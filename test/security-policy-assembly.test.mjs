...
test('the FEATURE_MOUNTERS tail order pins stay intact (remote..profile at the end)', () => {
  ...
  assert.equal(names[names.length - 9], 'sessionActivity', 'session activity projection appends first in the integration wave tail')
  ...
  assert.equal(names[names.length - 6], 'sessionInteraction', 'session interaction operation appends after plan mode control')
  assert.equal(names[names.length - 5], 'attention', 'attention appends after sessionInteraction')
  assert.equal(names[names.length - 4], 'checkpoints', 'checkpoints stays directly before decision participation')
  assert.equal(names[names.length - 3], 'decisionParticipation', 'decision participation stays directly before scoped contributions')
  assert.equal(names[names.length - 2], 'scopedAgentContributions', 'scoped agent contributions is the last FEATURE_MOUNTERS entry')
