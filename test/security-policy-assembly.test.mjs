...
test('the FEATURE_MOUNTERS tail order pins stay intact (remote..profile at the end)', () => {
  ...
  assert.equal(names[names.length - 8], 'sessionActivity', 'session activity projection appends first in the integration wave tail')
  ...
  assert.equal(names[names.length - 1], 'scopedAgentContributions', 'scoped agent contributions is the last FEATURE_MOUNTERS entry')
