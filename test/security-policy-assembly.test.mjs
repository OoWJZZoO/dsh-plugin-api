...
test('the FEATURE_MOUNTERS tail order pins stay intact (remote..profile at the end)', () => {
  ...
  // the remote..profile relative order pins shift by exactly that entry.
  assert.equal(names[names.length - 12], 'remote', 'remote stays directly before execution')
  assert.equal(names[names.length - 11], 'execution', 'execution stays directly before recovery')
  assert.equal(names[names.length - 10], 'recovery', 'recovery stays directly before coordination')
  assert.equal(names[names.length - 9], 'coordination', 'coordination stays directly before workspaceTransactions')
  assert.equal(names[names.length - 8], 'workspaceTransactions', 'workspaceTransactions stays directly before diagnostics')
  assert.equal(names[names.length - 7], 'diagnostics', 'diagnostics stays directly before tasks')
  assert.equal(names[names.length - 6], 'tasks', 'tasks stays directly before toolDiscovery')
  assert.equal(names[names.length - 5], 'toolDiscovery', 'tool discovery mounts directly before skillsActivation')
  assert.equal(names[names.length - 4], 'skillsActivation', 'skills activation mounts directly before context')
  assert.equal(names[names.length - 3], 'context', 'context mounts directly before profile')
  assert.equal(names[names.length - 2], 'profile', 'profile stays directly before sessionChannel')
