...
| U20 adapter decoration lifecycle（M6 最后批次，2026-08-27 Stage 2 批量确认门） | 官方 `dsh-llm` 无 adapter-registration identity、确定性 decoration 排序、metadata-overlay、disposer/取消与 topology reconciliation 的公开 seam；replacement 在保留官方 `llm` 行完整 host 契约的同时增加有界 decoration 生命周期 | `llm`（`dsh-llm`，1407 行，当前 host-only） | 高 | 高 | **已交付**：replacement 包 `@deepseek-ai/dsh-plugin-api-llm` 作为 current workaround（row `plugin-api-llm`，唯一 owner `@deepseek-ai/dsh-llm`；主门面 `llmAdapters` 条件投影，host-only §10 六项全否）；U20 保留为上游提案，退役条件见 feature-list §3 U20 行 |
| remote-session-channel（M6 最后批次，B+R 混合，2026-08-27 确认） | 跨设备 session channel 无官方整体 owner：认证抽象与 session cursor/replay 为 B 门面，transport 载波与 channel RPC 派发分属 connection/gateway 两官方组件（跨组件 feature，feature-list §3.1.1 已报备登记） | `connection`（`dsh-client-connection`）+ `typert-gateway`（`dsh-api-gateway`） | 高 | 高 | **已交付**：B 门面 `pluginApi.sessionChannel`（可插拔认证抽象 + 拉取式游标/重放）+ replacement 包 `@deepseek-ai/dsh-plugin-api-session-channel-connection`（row `plugin-api-session-channel-connection`，唯一 owner `@deepseek-ai/dsh-client-connection`）与 `@deepseek-ai/dsh-plugin-api-session-channel-gateway`（row `plugin-api-session-channel-gateway`，唯一 owner `@deepseek-ai/dsh-api-gateway`）作为 current workaround，各自自带 client 半面；U21/U22/U23 保留为上游提案，退役条件见 feature-list §3 U21–U23 行 |
