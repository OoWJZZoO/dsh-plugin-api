...
|---|---|---|
| `docs/standards/refactor/public-api-shape.md` | 适用 | 目标 host/client namespace、领域优先命名、旧 alias 删除、capability registry、`services.*` 分层已在第 1–4、9 节覆盖。 |
| `docs/standards/refactor/composition-and-authority.md` | 适用 | composition mode、Composable Profile、owner/key/generation、authority closure、claim/preflight、失败隔离已在第 5–7 节覆盖。 |
| `docs/standards/refactor/domain-composition.md` | 适用 | 各目标领域的 owner、冲突、顺序、scope、resource 和 authority 最低要求已在第 8 节覆盖。 |
| `docs/standards/refactor/capability-and-services.md` | 适用 | feature admission/retirement、公共面减法、`services.*` 静态白名单、成员级 availability 和 passthrough 分级已在第 4、5、9、12 节覆盖。 |
| `docs/standards/refactor/versioning-and-protocols.md` | 适用 | 冻结的 `<A>-<B>.<C>.<D>` 基线、插件协商、wire revision、durable schema、装配等价性已在第 10、13 节覆盖。 |
| `docs/standards/refactor/plugin-state-and-lifecycle.md` | 适用 | owner-scoped private storage、三档 durable scope、schema envelope、disable/reload/uninstall/purge 生命周期已在第 11 节覆盖。 |
| `docs/standards/refactor/ordering.md` | 适用 | 固定 priority、注册顺序、领域 reducer、producer/consumer 分离和禁止全局依赖图已在第 7、8、13 节覆盖。 |
| `docs/standards/refactor/sdk-and-conformance.md` | 当前不适用 | 本 feature 不实现独立 SDK、type-only 包、conformance 命令或运行时 registry 服务；仅交付可被未来评估复用的 registry、类型和测试事实。 |
