...
SPEC1 Stage 0–2 已确认（M6 第四批次批量确认门）；Stage 3 Tasks 已通过对抗性审查（2026-08-25「无偏差」，建议项已就地吸收）。Stage 4 全部任务已实现并验证（方括号勾选如实记录），全局终审（首轮 + 复审轮）均返回「无偏差」，本批次交付完成；实现细节修订清单见最终交付报告。
...
- [x] **1. 运行时事实核验（对应 requirements PTD-3.4/3.5 与 design「官方 loader 能力审计」表）**
...
- [x] **2. 纯函数归一化/校验模块（lib/tool-discovery-normalize.js）+ 单测（TDD）**
...
- [x] **3. typed errors 模块（lib/tool-discovery-errors.js）+ 单测**
  - 3.1 新增错误类（全部继承 `PluginApiError`，code 用能力词，无治理代号）：
    - `DISCOVERY_REGISTRATION_INVALID`、`DISCOVERY_ENTRY_CONFLICT`、`DISCOVERY_ENTRY_UNKNOWN`、`DISCOVERY_ENTRY_DISPOSED`、`DISCOVERY_ENTRY_DEACTIVATED`、`DISCOVERY_ENTRY_FAILED`、`DISCOVERY_SCOPE_UNRESOLVED`、`DISCOVERY_ACTIVATION_SUPERSEDED`（激活定义到达前被同 scope 新 claim 取代的迟到结果）；
...
- [x] **4. 引擎主体（lib/tool-discovery.js）：registry、activation、generation、audit、availability + 单测（TDD）**
...
- [x] **6. 装配面集成（真实官方 SystemPrompt 服务 + 迷你 ctx）：provider 与 hint + 测试**
...
- [x] **7. 服务装配（lib/plugin-api-service.js `// tool-discovery` 注册块）+ 测试**
...
- [x] **8. guards probe + 宿主装配（lib/guards.js probe 分支；lib/index.js `// tool-discovery` 注册块）+ 索引级集成测试**
