...
| `tools/post-execute` | `{exec, result}`（result 只读） | 决策对象（accept + content/value 改写、block + feedback；官方词汇另携带可选 `additionalContexts` 数组——执行期按官方源回写的超集，门面透传不解释） | `{kind:'accept', content?\|value?, additionalContexts?}` \| `{kind:'block', feedback, additionalContexts?}` | waterfall（无执行续延） | 保留 | exec.signal | scope-filtered | contain + accept 现结果 | 同上；内容脱敏归 `security.redaction` | A 类稳定化；无 R |
...
| `fs/write-intent` / `fs/edit-intent` | `{target:{targetKey, displayPath}, exec}`，freeze `all` | 决策对象（放行 / 拒绝） | proceed（无决定/undefined）\| deny（携带 reason） | 屏障式：异步捕获完成后才 `next()` 放行 | **副作用前完成**（写入等全部策略结算） | exec/signal 贯通 | facade | contain + 放行 | priority + 注册顺序 | A 类稳定化（producer 为 filesystem authority）；无 R。**deny 通道注记**（Stage 4 探针 P5）：官方把 waterfall 返回值直接作为 `writeText`/`editText` 的 CAS `expected` 条件消费，无返回值型 deny 通道；有效 deny 决定 = 分类后决定路径，转化为 typed denial 错误从参与槽位传播（官方唯一 deny 机制，工具以 isError 收口）；意外 throw/rejection 仍 containment + 放行 |
| `compaction/request` | `{agent, session, trigger, range, sourceCommandId?}` | 决策对象 | proceed（无决定，以 undefined 表达）\| `{kind:'reject', reason?}` \| `{kind:'replace-range', start, end}`（扁平 inclusive 整数边界、start <= end；嵌套 range 形状按 producer 契约为 malformed——producer 契约见 compaction-events event-contract `decideCompactionRequest`） | waterfall | 保留 | 载荷 signal | facade | contain + proceed | 同上 | A 类稳定化（producer 为已交付 compaction replacement）；触发操作归 compaction-operation 线，本线只承载参与 |
| `session-title/candidate` | `{agent, session, message:{seq,text,source}}` | 决策对象 | proceed（无决定，以 undefined 表达）\| exclude(reason?) \| replace(`{message:{seq}, reason?}`，reason 可选——producer 契约见 session-title event-contract `decideSessionTitleCandidate`) | waterfall | 保留 | —（catalog 无 signal） | facade | contain + proceed | 同上 | A 类稳定化（producer 为已交付 session-title replacement）；title provider authority 不变 |

## 机制设计

### 统一参与承载（内部，不设公共万能入口）

- **安装**：`registerParticipation(name, entry)`（bus substrate 内部）：校验点位在**当前 active catalog** 中存在且 dispatch 模式为 waterfall（现行 runtime catalog 以 `mode` 字段承载派发模式、无独立 eventSemantics 字段；decision 语义的准入由领域 registry 层的显式枚举承载，bus 不重复判定语义类别）；创建与现行内部订阅同构的条目（priority、scope、order），经 `reconcile` 安装为官方 ctx 钩子。官方 dispatch 模式与 payload 冻结策略（`freezeByPolicy`）原样保留——**不新增派发内核，不改变 deepFreeze 语义**。
...
| 失败 | guard |
|---|---|
| 策略 throw / rejection / 迟到 promise | slot containment + 点位默认决定 + 有界诊断；不抛穿 dispatch / apply。**例外（fs 意图点位）**：有效 deny 决定是分类后的决定路径而非故障，转化为 typed denial 错误传播使 producer 以 isError 收口（官方唯一 deny 通道，见逐点位保真表 deny 通道注记）；意外失败仍 containment + 放行 |
