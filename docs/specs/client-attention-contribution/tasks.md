...
- [ ] **4.1 契约 probe 门（R12 AC2 钉定；两条分支杜绝中间态）**：只读 probe 官方 `@deepseek-ai/dsh-client-runtime` 与 web profile 行——(a) 官方行 `client-runtime` 存在且无 owner；(b) 浏览器模块契约可复刻：`slots`（含 `slots/changed`）、`conversationEvents`、`conversationViews`、`connection/reset`、sessions/workspaces reflect outward face；(c) client manifest inject `[dsh-client-connection, dsh-typert-registry, dsh-api-remotes]` 与装配语义（R12 §10 证据 row 1/5/6）；(d) connection/reset 与模块 HMR 语义（browser-side state/reconnect，被复刻契约）；probe 产出证据记录。**分支 A — probe 失败（官方契约不可复刻）**：4.2/4.3/4.4 一并停止，**不落任何禁用官方行的 patch**（官方行行为照常），按 Wave 4.6 fallback 语义降级并在交付报告与 9.2 上报。**分支 B — probe 证明官方契约可复刻、仅扩展边界与 design §3/§4 不符**：允许进入 4.2–4.4 完成复刻并**跳过 4.5 扩展**，该情境按「条件性人类裁决门」上报。仅当 probe 全部通过（契约可复刻且扩展边界与 design 一致）才进入 4.2→4.5 完整链。
...
- [ ] **8.8 wire revision 与文档同步**：attention 转发消息族（`AttentionUpdate`/`AttentionRequest`/`AttentionOutcome`）wire revision 登记；`docs/standards/capability-strategy.md` §5 装配表新增两替代行（api-remotes / client-runtime）；README/公共契约现状注/AGENTS.md 需要的同步项更新（验收：装配表与 registry 一致；`versioning-and-protocols.md` 适用条款对齐，文档与 registry 一致）。
...
2. Wave 3/4 各自 probe 门（3.1/4.1）与其扩展实现（3.5/4.5）串行；probe 只允许两条分支，杜绝中间态——**分支 A**：官方契约不可复刻 ⇒ 该 slice 的脚手架（3.2/4.2）、复刻（3.3/4.3）、client 半面（3.4/4.4）一并停止，**不落任何禁用官方行的 patch**，官方行行为照常，按 fallback 语义降级并在交付报告与 9.2 记录；**分支 B**：契约可复刻、仅扩展边界与 design §3/§4 不符 ⇒ 进入复刻链并**跳过扩展**（3.5/4.5），该情境按「条件性人类裁决门」上报；probe 全部通过才进入复刻 + 扩展完整链。
