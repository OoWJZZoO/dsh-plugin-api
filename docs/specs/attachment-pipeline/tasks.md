...
> status: Stage 4 bounded leaf implementation and verification complete; this task record is included in the attachment completion boundary, with Wave D integration explicitly handed off.
...
- The Stage 3 write scope was limited to `design.md` and this `tasks.md`; that
  boundary was approved and committed before Stage 4 began. The checklist below
  is the approved Stage 4 execution record. Its implementation scope remains
  limited to the feature-owned files listed below and does not authorize Stage 4
  work outside this feature or any later stage.
...
- The sole approved P0 baseline exception is `test/usage-query.test.mjs`: it may
  receive only a deterministic fixture/clock correction when the guarded full
  suite is otherwise red, with no usage production-code change or unrelated
  baseline edit.
...
The attachment Stage 4 completion commit contains the completed
...
`git diff --check`; after the eventual Stage 4 completion commit, the worktree
must be clean. Stage 3 approval and the Stage 3 document commit are
prerequisites to Stage 4, and this task record does not waive either gate. The
leaf completion boundary does not include integration-owned Wave D assembly.

## Tasks

Execution order is top to bottom. Every item is a testable Stage 4
...
- [x] 1.1 Create the attachment auxiliary package and its official patch entry
...
- [x] 1.2 Add the vendored official local-store fork and local invariant helper
...
- [x] 1.3 Test the import boundary and package manifest: imports of
...
- [x] 2.1 Implement pure record/model validators in
...
- [x] 2.2 Implement the five source adapters (`file`, `paste`, `data-uri`,
  ...
  (AP-2, AP-3, AP-6)
- [x] 2.3 Make source, trust, declaration, deadline, concurrency, and
...
- [x] 3.1 Implement the approved media profiles and effective policy in
...
- [x] 3.2 Encode the duration failure semantics exactly: a timed profile with
  ...
  (AP-3)
- [x] 3.3 Implement frozen `pipeline.capabilities()` and batch validation.
...
- [x] 4.1 Implement `pipeline.registerTransform` with explicit
...
- [x] 4.2 Implement `pipeline.transform` so a permitted result creates a new
  ...
  immediately after async admission and immediately before commit: every caller
  AbortSignal, deadline, registration/operation token, owner/generation/current
  target, and the same operation token passed to `_persist` must still be valid;
  complete media/byte/pixel/media-type/trust/digest admission plus the same
  duration check from task 3.2 must pass. Internal deadline expiry or concurrency loss returns typed
...
- [x] 4.3 Add stale-result and failure containment checks before every
...
- [x] 5.1 Implement `packages/attachments/lib/journal.js` under the approved
  `attachments/v1/pipeline/records` root with atomic temp-write/rename/fsync,
  read verification, persistent media metadata (`mediaType`, `bytes/length`,
  dimensions, and duration), one scope authority per record, and reuse of
  official content-addressed objects for byte dedupe without sharing
  provenance. Missing or mismatched metadata must fail closed. Recover the
  current record/index from the journal after restart. Test atomicity, read
  verification, restart fixtures, scope separation, and provenance isolation.
  (AP-2, AP-7)
- [x] 5.2 Implement `projection.resolve` and `projection.open` with required
  ...
  AP-6, AP-7)
- [x] 5.3 Implement owner-scoped cleanup with explicit retention reason,
  ...
  cleanup I/O; before record deletion and every object delete/rename/fsync,
  atomically recheck current generation and all references across every scope
  and owner, preventing deletion of a still-referenced object. Prevent an
  aborted cleanup from committing deletion. Test repeated cleanup, retained
  references, cleanup races, cancellation, and cross-scope denial without
  deleting another owner's state. (AP-6, AP-7)

### 6. Read-only projection, admission evidence, and visibility

- [x] 6.1 Implement `projection.provenance` as a read-only reconstruction of
...
- [x] 6.2 Implement the separate projection owner and
  `project(ref, { targetRoute, admission })`: require an explicit route and
  exactly `{ accepted, source, observedAt }`, first confirm that the record is
  owned by the current owner/generation, and return only identity, generation,
  route, media metadata, admission, and bounded provenance. A stale generation
  must not project successfully. Test frozen result shape and mutation
  isolation. (AP-5, AP-6)
- [x] 6.3 Make projection fail closed: missing/malformed admission, missing
  ...
  (AP-5, AP-10)
- [x] 6.4 Apply visibility and redaction at projection and diagnostic
...
- [x] 7.1 Implement `packages/attachments/lib/apply.js` to inspect
  ...
  states. (AP-8, AP-9)
- [x] 7.2 Add exact identity/config checks for runtime full identity,
  ...
  results. (AP-8, AP-9)
- [x] 7.3 Complete post-registration contract probing and rollback: verify
  frozen official limits, required methods/error codes, pipeline members, real
  disposal state transition, disposal idempotence, and owner-marker cleanup;
  on probe failure first roll back the replacement, then apply the
...
- [x] 8.1 Wire `pluginApi.attachments` through only the attachment-owned
  ...
  surface. The real loader/version gate must fail closed for absent marker,
  disabled row, unavailable auxiliary manifest, and version mismatch. Do not
  add a guard branch or enter `FEATURE_MOUNTERS`. Test marker, loader,
  provider, and version mismatch independence. (AP-4, AP-8, AP-9)
- [x] 8.2 Keep `pluginApi.services.attachments` as the official static
  ...
  (AP-1, AP-8, AP-9)
- [x] 8.3 Test facade lifecycle and teardown, including marker publication and
...
- [x] 9.1 Exercise source, decoder, transform, journal, projection, cleanup,
...
- [x] 9.2 Preserve the host-only client audit with six separate negative
...
- [x] 9.3 Verify the existing U10 entry and retirement condition read-only:
...
- [x] 10.1 Run the focused attachment tests and record evidence for official
...
- [x] 10.2 Run the repository-guarded `npm test` from the plugin-api root, not
  ...
  zero-modification audit. Record exit-zero results and the attachment
  completion-commit boundary, including clean-worktree evidence. (AP-8, AP-9,
  AP-10)
- [x] 10.3 Supply the attachment leaf's row/behavior evidence to the
...
## Stage 4 handoff condition

All 31 executable checkboxes above are complete in this worktree. The focused
command
`node --test packages/attachments/test/*.mjs test/attachment-pipeline.test.mjs test/governance-token-audit.test.mjs`
reported 42 tests passed and 0 failed, including official fork per-function
behavior and sharp import/replacement evidence, the real loader/version gate
and six host-only negative assertions, post-registration behavioral disposal
probes and fallback/no-double-run, the official `web.fetch({ url }, signal)`
body contract, binary-output `maxBytes`, duration and transform pre-commit
guards, persistent media metadata, restart/current-generation and global
reference cleanup protection, unique owner-token disposal, journal integrity,
batch image limits, and redaction cases. The repository-guarded `npm test`
command reported 1,244 tests passed, 0 failed under the required memory guard.
The governance-token audit, `git diff --check`, and the
official-package zero-modification audit must also pass.

The P0 baseline exception was limited to `test/usage-query.test.mjs`: its
existing `day` fixture was not connected to ledger `createdAt`/`settledAt`, so
the date assertion depended on the wall clock. The test now uses a fixed clock
and keeps the existing sample fixture; no usage production implementation or
other baseline file was changed.

Wave D integration handoff remains explicit: this leaf did not modify
`packages/full/package.json` or `packages/full/test/**`. The integration owner
must still perform the full-bundle dependency and aggregate assembly comparison
against `main + all auxiliary packages` under temp contract §5. The attachment
leaf creates its bounded Stage 4 completion commit after the checks above; the
integration owner must apply the later Wave D assembly in its own scope. No
additional Stage 4 review is a prerequisite for this leaf commit.
