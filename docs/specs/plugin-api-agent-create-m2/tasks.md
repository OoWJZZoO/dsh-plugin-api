...
- [x] 1.1 In the package public API documentation, add the host-only A11 surface:
...
- [x] 1.2 State the tiering, including that provider primitives are supported advanced ordered-lifecycle APIs rather than ordinary creation APIs.
- [x] 1.3 State the A-class transparency contract: exact caller context, arguments, raw synchronous result/official registry Promise, handle, disposer, official errors, lifecycle publication, and teardown behavior pass through unchanged.
- [x] 1.4 State member-local degradation and P1/P2 behavior, including that factory absence and provider-slot occupancy are official call-time outcomes rather than availability changes.
- [x] 1.5 Do not document unimplemented route helpers, client APIs, synthetic lifecycle behavior, or a public generic extension registry.
...
- [x] 2.1 Add `lib/agent-create-api.js` as a zero-harness-dependency internal module accepting injected probe, active-state, and safe-logger operations.
- [x] 2.2 Implement six independent non-invoking probes for `create`, `resume`, `register`, `enter`, `announce`, and `setFactory`, run only after the existing M1 agent guard passes.
- [x] 2.3 Persist private per-member capability state and expose a stable frozen `availability` snapshot whose outer and nested provider objects are frozen and whose leaves are booleans. Make `provider.isActive` true exactly when its three leaves are true.
- [x] 2.4 Build consumer-context-bound closures which narrowly contain only service/member resolution failures. On such a failure, mark only that member unavailable, write one redacted deduplicated adapter-owned diagnostic, and throw a named `PluginApiFeatureDisabledError('agent', ...)` before official invocation.
- [x] 2.5 For a resolved official member, execute exactly one `Reflect.apply(member, registry, exactArgs)` and return its raw output. Do not catch official method errors/rejections or observe/wrap returned promises, handles, or disposers.
- [x] 2.6 Keep factory absence, factory-slot occupancy, duplicate ID, invalid announce, setup/persistence/signal/rollback errors, and all official runtime state outside capability state and facade containment.
- [x] 2.7 Add `test/agent-create-api.test.mjs` focused adapter tests for:
...
- [x] 3.1 In the integration-owned `lib/plugin-api-service.js` composition work, extend the core-inactive and whole-agent P2 views to declare all A11 consumer members, the provider namespace and its exact three members plus `isActive`, and a frozen all-false availability matrix.
- [x] 3.2 Ensure disabled calls check core inactive first, otherwise use existing `PluginApiFeatureDisabledError('agent')`; disabled views must not resolve `agents`.
- [x] 3.3 Preserve current P1/P2 behavior and all non-agent facade surfaces unchanged. Do not introduce an error class, a general P4 convention, or a public extension registration API.
- [x] 3.4 Extend `test/plugin-api-service-agent.test.mjs` to verify declared A11 disabled shape, P1/P2 precedence, named member errors, frozen all-false availability, provider `isActive === false`, and absence of official service access.
...
- [x] 5.1 Run all new and modified focused tests from tasks 2-4 with `node --test`.
- [x] 5.2 Run the complete repository `node --test` suite.
- [x] 5.3 Inspect test failures, if any, and correct only A11-spec-conformant implementation or test defects. If an issue changes approved Goal or Requirements acceptance criteria, stop and request human direction; otherwise synchronize the affected requirements/design/tasks wording before final reporting.
- [x] 5.4 Run `git diff --check` and inspect `git status --short` to ensure the deliverable contains only A11 implementation, tests, approved specs, and required documentation/register updates.
...
- [x] 6.1 Update `docs/specs/plugin-api-features/feature-list.md` to mark `plugin-api-agent-create-m2` delivered with the approved consumer/provider shape, six-leaf availability, caller-context forwarding, identity, and coordinated facade constraints.
- [x] 6.2 Update `AGENTS.md` section 8 with the delivered A11 entry and its scope, A-class passthrough, member-local availability, exact identity, caller-fiber ownership, and integration boundary constraints.
- [x] 6.3 Re-run the focused documentation assertions or relevant test suite after record updates.
- [x] 6.4 Run final `git diff --check`, review the final status, and commit all Stage 4 implementation, tests, specs, and delivery records on the feature branch before reporting completion or cleaning any worktree.
