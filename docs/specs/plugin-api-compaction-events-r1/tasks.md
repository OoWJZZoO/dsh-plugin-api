...
> 执行注：按用户明确指示，**Stage 4 不再调用子代理做对抗性审查**；改为代理自审 + 全量测试 + 迁移验收。未批准前不创建/修改任何实现产物。
...
- [ ] 1.4 回归
  - 运行现有 `node --test` 全绿；`git diff --check` 通过。
...
- [ ] 7.2 dsh-pro-ex-ability-anchor 引用同步
...
- [ ] 8.3 交付审计
  - 官方包文件 checksum 断言未修改；全套测试通过；`git diff --check`；Stage 4 完成提交。
