...
> 状态：Stage 3 tasks（无人值守授权下由协调者代行确认）
...
- [ ] 1.1 实现 C1 manifest helper 与 C8 Typert artifact facade，并建立各自的纯 leaf 测试。
...
- [ ] 2.1 实现 host `settingsRemote` leaf 和可回滚的 Typert remote publication。
...
- [ ] 3.1 实现 C9 connection leaf，随后实现 ST6 real codec leaf。
...
- [ ] 4.1 实现 C6 remote event bridge leaf。
  - 新增 `client.remote.$on/$dispatch` adapter，只允许 official forwarded-event allowlist，保留 payload wire shape、listener order、cancellation、return/rejection and containment semantics。
  ...
  - 覆盖 allowlist rejection before transport、invalid payload、remote unavailable、listener failure、exact disposer/stale cleanup and unrelated-feature survival。
  - _Requirements: 2.2–2.5, 9.1–9.5, 10.2–10.7_

## 5. W3a remote-core worktree: C2

- [ ] 5.1 实现 generic `clientRemoteContribution` mount owner。
...
- [ ] 6.1 实现 `clientSettingsScope` typed scope。
  - 新增 `client.settingsScope.bind(spec)`，提供 `getSnapshot/subscribe/load/set/unset`，通过 C9 connection 保持 exact official settings wire and Promise/cancellation behavior。
  ...
  - 覆盖 immutable snapshots、subscription order/cancel、remote loss, pending call rejection, AbortSignal, repeated bind, disposer isolation and stale scope state。
  - _Requirements: 2.2–2.5, 4.1–4.4, 7.1–7.5, 10.2–10.7_

## 7. W3c slots worktree: C4 → C5

- [ ] 7.1 实现 C4 slots facade，随后接入 C5 `slots/changed` descriptor。
...
- [ ] 8.1 实现 settings-specific remote contribution on top of C2 and ST6.
...
- [ ] 9.1 完成 pre-merge read-only audit，并按固定顺序合并 W1–W4 committed boundaries。
...
- [ ] 9.2 执行 shared unification wave，接线 host/client composition and failure paths。
...
- [ ] 9.3 完成真实 consumer migration and final M3 verification。
...
- Stage 3 gate: `git diff --check` passes and this task list is committed before any M3 implementation worktree is derived.
...
- Final gate: integration batch proves all 11 M3 rows, both consumer migrations, host/client boot, full tests, no official package modification, and no M4 implementation. Any changed public acceptance boundary requires a new contract decision before implementation continues.
