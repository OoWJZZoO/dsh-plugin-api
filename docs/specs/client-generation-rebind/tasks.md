...
Stage 3 Tasks 已获批准并提交（Stage 3 commit `baeeffb`）；Stage 4 实现与验证已完成，以下复选框记录执行状态。本文件不改变已批准 Requirements/Design 边界。按当前授权，Stage 4 变更保持未提交，待主编排代理整体终审。
...
- [x] 7.2 新增 `test/client-generation-rebind-migration.test.mjs` 的 migration-shaped client integration coverage，使用已批准的 official client fixtures 验证 remote/settings/slot consumers 经稳定 facade 绑定、旧 contribution/hack 路径不能写回 stale state 或绕过公开 API；对 `../dsh-read-image` 与 `../dsh-pro-ex-ability-anchor` 只做外部只读观察，不修改外部仓库；另以临时隔离的 `DSH_HOME` profile 加载当前 worktree，运行 4G 限制下的真实 dev/headless boot 证据，不把生产/社区兼容性或 transport reconnect guarantee 写成验收承诺。（覆盖：CG-4 AC1–AC6、CG-5 AC2–AC4、CG-6 AC1–AC4、CG-8 AC3–AC5；Design：Testing Strategy、Boundary and Non-Duplication；契约：§6、§7；AGENTS.md §5）

- [x] 8.1 运行 focused lifecycle/bundle/integration/migration tests 后运行仓库规定的 `npm test`（使用脚本内置 4G 护栏，不裸跑 `node --test`），并用 `git diff --check`、目标文件范围/owner/夹带检查、`git status --short --untracked-files=all`、官方 DSH 包零修改检查和治理 token 审计收集证据；分别记录 focused green、全套 green、外部只读 boot 观察及未验证的生产/社区兼容性，确认 client-only、无 host guard/mounter、五 epoch 独立、bind/rebind、stale guard、官方契约保真，以及“核心服务/初始化失败才 disabled/inert、listener/disposer/diagnostic failure 全部 contained”的分层 fail-safe 均有对应证据。按当前授权，Stage 4 变更保持未提交，待主编排代理整体终审。（覆盖：CG-1 至 CG-9；Design：Testing Strategy、Requirements Coverage、Error Handling and Fail-Safe；契约：§0、§3、§6、§7）

### Stage 4 execution evidence (status only)

- focused client/lifecycle/bundle/integration/migration collection: 191/191 passed;
- repository `npm test`: 1258/1258 passed. The dynamic-date fixture at `test/usage-query.test.mjs:66` was reproduced and fixed with a scoped Node `Date` mock; this is an integration-owner test-only baseline correction outside the client feature, with no usage implementation change, and is called out here rather than hidden;
- lifecycle repair evidence: official SlotRegistry-style collapse teardown/redeclare, package-shared remote leases with two owner-bound disposers, and raw modules `invalidate()` fallback/cleanup are covered by focused tests; modules fallback emits an explicit bounded diagnostic and advances only the modules epoch;
- external read-only observations: `../dsh-read-image` tests 29/29 passed (its pre-existing dirty files were not modified) and `../dsh-pro-ex-ability-anchor` tests 126/126 passed. A separate temporary `DSH_HOME` copied `dev`/`headless` profile boot with the current worktree package and 4G `systemd-run` limit returned dev HTTP 200 and headless `OK`; the original profiles, credentials, production `dsh web`, and external repositories were not modified. No production/community compatibility or transport reconnect guarantee is claimed;
- `git diff --check` passed; target-file/status and governance-token audits passed; official DSH aggregate SHA-256 remained `cf16c829391784a030402d7cb18d6c58955433e48741576ea83e093037b8b213`.
