...
2. WHEN a catalog entry is read, THEN it SHALL contain at least: `mode` (`on` | `emit` | `serial` | `parallel` | `bail` | `waterfall`), `payload` type description/reference, `scopeFiltered` boolean, `source` feature id, `type` (`A` | `B` | `R`), and the unified policy fields per `plugin-api-m1-integration`: `scopeKey` (`'args[0].agent' | 'args[1].scope' | null | undefined`), optional `fault` (`'contain' | 'created' | 'propagate'`, default `'contain'`), optional `freeze` (`'all' | { deep: string[] } | 'except-signal'`, default `'all'`).
...
>
> **修订注记（`plugin-api-compaction-events-r1`）**：catalog entry `type` 联合扩为 `'A' | 'B' | 'R'`。R 类辅助包贡献 `compaction/*` 事件词汇，作为独立的 `rSlice`（`{ name, entries, isActive }`）交给 `createEventsBus`：内部订阅元数据使用静态全集，公开 `catalog` 快照仅在该 replacement 行 active（loader 条目 active + `ctx.compaction` 携带契约符号）时列出 R 条目；inactive 时 R 条目不进入快照（req 6.4），订阅仍按静态元数据走门面包装。
