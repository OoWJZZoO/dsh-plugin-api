...
- [ ] **1.1 创建 `packages/mcp/` 骨架**（MC-6, MC-7）
...
- [ ] **1.2 patch 组合测试（TDD 前置）**（MC-6, MC-7; 契约 §3/§7）
...
- [ ] **1.3 版本解析与一致性纯函数基座**（MC-7; design Component 1）
...
- [ ] **2.1 复刻 transports 与命名/schema/output/executor 纯逻辑**（MC-1, MC-4, MC-5）
...
- [ ] **2.2 复刻分页同步与 fetch-before-swap**（MC-1, MC-3, MC-4）
...
- [ ] **2.3 复刻连接主管（generation / reconnect / dispose / HMR）**（MC-1, MC-5, MC-10）
...
- [ ] **2.4 official-fork-integrity 基线测试**（MC-1; 沿用 compaction-events/session-title 的 `official-fork-integrity` 模式）
...
- [ ] **3.1 catalog reducer 与只读投影**（MC-2, MC-4, MC-10）
...
- [ ] **3.2 generation 生命周期接入**（MC-2, MC-3, MC-5）
...
- [ ] **3.3 同步串行化与 racing（latest-wins）**（MC-3）
...
- [ ] **3.4 `mcp/catalog-changed` 通知与 onChange**（MC-2; 契约 §3 不新增 catalog slice）
...
- [ ] **3.5 可见性与失败隔离（redaction + containment）**（MC-10）
...
- [ ] **4.1 apply 入口：自检矩阵、服务注册、fail-safe**（MC-6, MC-7）
...
- [ ] **4.2 六项 client 阴性检查回归 fixture**（MC-8; 契约 §7 MCP 加测）
...
- [ ] **5.2 feature 登记（U-series + replacement 行）**（MC-9; 契约 §3）
