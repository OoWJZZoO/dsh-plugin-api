...
- 交付：`packages/mcp/**`（辅助 replacement bundle `@deepseek-ai/dsh-plugin-api-mcp`，row `plugin-api-mcp`）+ `docs/specs/plugin-api-features/feature-list.md` 登记（U10 上游提案行、§3.1 R 类登记行、§7 delivered 条目）。
