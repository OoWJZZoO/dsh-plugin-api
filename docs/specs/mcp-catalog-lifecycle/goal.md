...
该 feature 已按用户明确批准作为 `dsh-mcp-client` 官方 loader 边界的 R 类 replacement 推进；Stage 1 仍只定义可测试需求，runtime identity、完整官方契约复刻和 replacement patch 的具体架构留到获批 Requirements 后的 Design。
...
- 如果采用 replacement，必须完整复刻被替代官方行的 ctx service/event 面，只增加 MCP 生命周期契约；必须有 runtime/package identity matrix、唯一 owner、boot 自检、版本失配局部停用和退役上游提案。
