...
| llm.admissionPolicies.register | host | policy | decide | ordered | facade | llm domain authority | owner-conflict | latest-wins | namespace availability() | register(spec) returns the shared frozen handle; the owner is derived from the caller context and the generation is facade-minted; the facade owns the policy registry and consumes it through the resolved-model-info wrapper path |
...
| llm.routing.observe | host | projection | subscribe | pure | facade | llm domain authority | not-applicable | — | namespace availability() | projection subscription returning the shared observation handle (the route view for one session plus its change feed) |
| llm.routing.observe.handle | host | projection | read | pure | session | session authority | not-applicable | — | namespace availability() | frozen { current(), subscribe(listener), dispose(), epoch }; current() is the domain read face (a degraded view once released), subscribe(listener) returns an unsubscribe function and is a no-op from a released handle, and dispose() is an identity-bound discriminated result (revoked / stale) |
...
| llm.routing.candidates.register | host | resourceRegistry | register | additive | facade | llm domain authority | owner-conflict | latest-wins | namespace availability() | register(spec) returns the shared frozen handle; owner derived and generation minted by the facade; the registration table is keyed by (owner, id) |
...
| agents.providers.register.handle | host | resourceRegistry | register | additive | facade | agent registry authority | owner-conflict | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() }; the announce variant is irreversible, so its dispose always answers the typed stale no-op |
...
| executions.observe | host | projection | subscribe | pure | facade | execution authority | not-applicable | — | namespace availability() | projection subscription returning the shared observation handle (current status, the change feed and the subscription generation) |
...
| executions.observe.handle | host | projection | read | pure | facade | execution authority | not-applicable | — | namespace availability() | frozen { current(), subscribe(listener), dispose(), epoch }; current() is the domain read face (a degraded view once released), subscribe(listener) returns an unsubscribe function and is a no-op from a released handle, and dispose() is an identity-bound discriminated result (revoked / stale) |
...
| tools.discovery.catalog.register | host | resourceRegistry | register | additive | facade | tools authority | owner-conflict | latest-wins | namespace availability() | register(spec) returns the shared frozen handle; the caller-supplied owner string is deleted — the owner is derived from the caller context and feeds the generation |
...
| tools.discovery.catalog.register.handle | host | resourceRegistry | register | additive | facade | tools authority | owner-conflict | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() } |
...
| prompts.contribute.handle | host | contribution | register | additive | facade | system prompt authority | owner-conflict | deduplicate | namespace availability() | frozen { id, ownerId, seq, dispose() } |
...
| mcp.observe | host | projection | subscribe | pure | facade | mcp replacement authority | not-applicable | — | namespace availability() | projection subscription returning the shared observation handle (the catalog snapshot and its change feed) |
| mcp.observe.handle | host | projection | read | pure | session | session authority | not-applicable | — | namespace availability() | frozen { current(), subscribe(listener), dispose(), epoch }; current() is the domain read face (a degraded view once released), subscribe(listener) returns an unsubscribe function and is a no-op from a released handle, and dispose() is an identity-bound discriminated result (revoked / stale) |
...
| tasks.observe | host | projection | subscribe | pure | facade | task authority | not-applicable | — | namespace availability() | projection subscription returning the shared observation handle (current task state, the change feed and the subscription generation); the returned handle additionally carries the registered extension members taskId and initialState |
| tasks.observe.handle | host | projection | subscribe | pure | facade | task authority | not-applicable | — | namespace availability() | frozen { current(), subscribe(listener), dispose(), epoch } plus the registered extension members taskId and initialState; current() is the redacted task state (a degraded view once released), subscribe(listener) returns an unsubscribe function and is a no-op from a released handle, and dispose() is an identity-bound discriminated result (revoked / stale) |
...
| diagnostics.observe | host | projection | subscribe | pure | facade | diagnostics authority | not-applicable | — | namespace availability() | projection subscription returning the shared observation handle (the filtered check view and its change feed) |
| diagnostics.observe.handle | host | projection | read | pure | session | session authority | not-applicable | — | namespace availability() | frozen { current(), subscribe(listener), dispose(), epoch }; current() is the domain read face (a degraded view once released), subscribe(listener) returns an unsubscribe function and is a no-op from a released handle, and dispose() is an identity-bound discriminated result (revoked / stale) |
...
| storage.availability | host | selfDescription | read | pure | workspace | storage binding authority | not-applicable | — | frozen { status } | availability() returning the frozen three-value status object; the domain detail fields (scope / durability / epoch) are not yet disclosed |
...
| llm.routing.candidates.register.handle | host | resourceRegistry | register | additive | facade | llm domain authority | owner-conflict | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() } |
...
| llm.routing.health.probe.register.handle | host | resourceRegistry | register | additive | facade | llm domain authority | owner-conflict | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() } |
...
| tools.guard.register.handle | host | policy | decide | ordered | facade | tools authority | not-applicable | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() } |
| tools.presentation.register.handle | host | resourceRegistry | register | additive | facade | tools authority | not-applicable | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() } |
...
| diagnostics.register.handle | host | resourceRegistry | register | additive | facade | diagnostics authority | latest-wins | latest-wins | namespace availability() | frozen { id, ownerId, generation, dispose() } |
...
| llm.requestTransforms.register | host | policy | decide | ordered | facade | llm authority | owner-conflict | latest-wins | namespace availability() | register(spec) returns the shared frozen handle; the owner is derived from the caller context and the generation is facade-minted; the facade owns the registry and consumes it through the llm/stream re-entry path |
...
| llm.routing.health.probe.register | host | resourceRegistry | register | additive | facade | routing authority | owner-conflict | latest-wins | namespace availability() | register(spec) returns the shared frozen handle; owner derived and generation minted by the facade; the registration table is keyed by (owner, id) |
| agents.providers.register | host | resourceRegistry | register | additive | facade | agents authority | owner-conflict | latest-wins | namespace availability() | register(spec) requires an explicit variant kind (factory \| announce \| enter), forwards the official verb verbatim and returns the shared frozen handle for every variant; an invalid or missing kind raises a typed error naming the legal variants; the announce variant is irreversible, so its dispose answers the typed stale no-op with that reason |
...
| tools.guard.register | host | policy | decide | ordered | facade | tools authority | not-applicable | latest-wins | namespace availability() | guard registration; the official registration verb performs the installation and its disposer is wrapped into the shared frozen handle |
| tools.presentation.register | host | resourceRegistry | register | additive | facade | tools authority | not-applicable | latest-wins | namespace availability() | presentation registration; the official registration verb performs the installation and its disposer is wrapped into the shared frozen handle |
...
| executions.recovery.checkpoints.restore | host | operation | execute | coordinated | session | checkpoint restore authority (single operation terminal) | fencing | exclusive | namespace availability() | typed {ok:true, code:accepted\|completed, operation:{id, ownerId, status(), observe(), dispose()}, observedAt} \| {ok:false, code:invalid-input, reason} (pre-start refusal, no observedAt, no control object) \| {ok:false, code:unavailable, reason} (caller owner derivation unavailable, no observedAt, no control object) \| {ok:false, code:missing\|conflict\|denied\|unavailable, reason, observedAt} (refusal while a plan is in force, no control object; the planner code is passed through, so unsupported-schema also appears here) \| {ok:false, code:aborted\|denied, reason, operation, observedAt} (refusal of a stop-then-restore against a live operation). The former read-only status snapshot is now operation.status(); accepted is used only when the terminal is not yet decidable |
