...
| tasks.observe | host | projection | subscribe | pure | facade | task authority | not-applicable | — | namespace availability() | projection subscription returning the shared observation handle (current task state, the change feed and the subscription generation) |
