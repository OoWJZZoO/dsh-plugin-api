...
- **观察**：`observe()` —— 官方事实流（session 日志 `permission/preset`、`sandbox/mode`、`approval/policy` 经 `session/event` firehose）之上的 projection-observe 面。
- **自描述**：`availability()`。
...
### C4. 观察面 `observe()`（Req 4、5）

- 形状与句柄：同 plan-mode 线（`{ current(), subscribe(listener), dispose(), epoch }` + `sessions.permissionPresets.observe.handle` 登记行）。
- 机制（A 类）：单条 firehose 订阅，过滤 `permission/preset`、`sandbox/mode`、`approval/policy` 三种事件（官方投影的完整折叠输入）；对订阅目标 session 重读官方 `current` + `options`，组合视图变化才投递 `{ target, preset, options, currentValue, observedAt }`。事实全部来自官方派发与官方读（Req 5.3 的核验义务）。
