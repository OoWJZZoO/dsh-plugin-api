...
- [ ] **1.1 TDD the static audit catalog in `lib/session-durable-catalog.js`.**
...
- [ ] **1.2 TDD the installed-contract builders and fail-closed record predicates in `lib/session-durable-catalog.js`.**
...
- [ ] **2.1 TDD live target validation and epoch-owned registration mechanics in `lib/session-durable-feature.js`.**
...
- [ ] **2.2 TDD `onDurable()` and `onceDurable()` in `lib/session-durable-feature.js`.**
...
- [ ] **3.1 TDD JSON snapshot, message-envelope, options, and provenance preflight helpers in `lib/session-durable-feature.js`.**
...
- [ ] **3.2 TDD `appendMessage()` in `lib/session-durable-feature.js`.**
...
- [ ] **4.1 TDD durable P1/P2 stubs and descriptor-preserving session composition in `lib/plugin-api-service.js`.**
...
- [ ] **4.2 TDD epoch publication, reset, and stale-cleanup protection in `lib/plugin-api-service.js`.**
...
- [ ] **5.1 TDD `sessionDurable` feature guard additions in `lib/guards.js`.**
...
- [ ] **5.2 Add audit-only peer identities and package tests.**
...
- [ ] **5.3 TDD host mounter and transactional apply integration.**
...
- [ ] **6.1 Add and run the consumer mapping regression fixture.**
...
- [ ] **6.2 Run feature and sibling baseline regression; explicitly record deferred migration acceptance.**
...
- [ ] **6.3 Synchronize delivery records, validate the final tree, and commit Stage 4.**
