...
- [x] 1.2 新建 `lib/tool-abort.js`（纯工厂模块）
  ...
  - 文件头 JSDoc 说明职责与两模式。
- [x] 1.3 新建 `test/tool-abort.test.mjs`（零 harness 纯单测）
...
- [x] 2.1 `lib/index.js` 新增命名导出 `buildToolAbortedErrorFactory(resolveModule = require)`
...
- [x] 2.3 `lib/plugin-api-service.js` 双态挂载
...
- [x] 2.4 新建 `test/index-tools-abort.test.mjs`（门面集成，走 apply + mock ctx，镜像现有 index-* 测试）
...
## 3. 真实官方 identity 测试 + peerDependency 复核 + 全量回归（req 2.1–2.4, 4.4, 5.4）

- [x] 3.1 新建 `test/tool-abort-official.test.mjs`（真实官方类，锁定 AC 2.1–2.4）
...
- [x] 3.2 复核**不**新增 peerDependency（执行期修正，见 design C4/D5）
  - 确认 `package.json` peerDependencies **不**新增 `@deepseek-ai/dsh-tools`（仓库不变式：peerDependencies = 门面 host 静态消费 + durable audit 的**精确并集**，见 `test/package.test.mjs`；dsh-tools 仅惰性消费、缺包降级，故不声明）；
  - 运行 `test/package.test.mjs` 确认精确并集断言保持通过（req 3.2/4.4 与 fail-safe 语义不变）。
- [x] 3.3 全量回归
...
- [x] 5.1 迁移编辑（跨仓库）
  ...
  - 替换唯一调用点（`lib/index.js:384`）为 fiber ctx 解析 + 兜底，解析**与调用**都包在 try/catch（pro-ex 家法：`ctx.get` 对未登记服务名可能抛；tools feature 禁用时门面 `toolAbortedError` 是抛 typed 错误的 `fail`，也必须吞掉走兜底）：
    `const makeAbortedError = () => { try { const p = typeof ctx.get==='function' ? ctx.get('pluginApi') : undefined; const f = p?.tools?.toolAbortedError; return typeof f==='function' ? f() : <裸 AbortError 兜底> } catch { return <裸 AbortError 兜底> } }`；
