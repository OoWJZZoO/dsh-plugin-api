...
>
> **维护修订（包政策推行，Stage 4 后执行）**：原 Stage 4 产物已按新包政策维护——`packages/session-title-r1/` → `packages/session-title/`，row id/feature/契约符号去除 `r1`；辅助包版本改为 `0.1.0-rc.6-0.5` + `dsh.api: 0.5`，peerDependencies 增加主包精确版本，apply 增加主包版本一致性校验（不一致仅停用 R 特性、保留官方等价 fallback）；主包 catalog slice 增加已安装辅助包版本校验与一次显式诊断；`packages/full/` 全量聚合 bundle 同时装配本替代行；新增 `test/package-policy.test.mjs` 与 `test/governance-token-audit.test.mjs`。
