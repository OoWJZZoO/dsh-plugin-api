...
SPEC1 Stage 2 草案 v2。本文承接 v2 Requirements（同批待确认）。v2 相对 v1 的实质修订：从"v1 零 R、superseded/aborted/settle 靠 reconstruction"改为**采纳 R**——与 `session-interaction-operation` 共用 `dsh-agent-loop` owner 包上的 loop boundary slice（attempt 生命周期/队列事实），把终态与排队判定升级为 observed；切片缺位时投影降级并如实报告。
...
边界不变：投影不拥有执行（调度/重试/终态提交/取消/通知都属各自 authority）；本 feature 只读已提交事实并显式标注推断；不是 scheduler/execution owner。新增的 v2 设计要点：**保真度分层**——full 安装（切片 active）交付 observed 级终态/排队事实；切片 inactive 时降级为 reconstruction 并在 availability 明示；重启后 in-memory 切片事实不冒充 durable（R6 AC2 规则）。
...
生命周期：切片 active 时 attempt 事实适配器提供 observed 输入；切片 inactive 时该适配器关闭，对应字段回到 derivation 规则（reconstruction/unknown），availability 报 degraded。宿主重启后 in-memory attempt 事实丢失：重建视图只对 durable 可再现的事实保留 observed，其余按 R6 AC2 降级。
...
- **durable joiner / observer hub / availability probe**：同 v1；epoch 重建关闭旧 epoch observer。

### 3. Loop boundary slice（R，与 interaction 共享；本 feature 的消费契约）

- 位置：`packages/agent-loop`（既有 replacement 包）内共享切片模块；官方行 `agent-loop` disabled+insert 结构不变（R1）。切片承担三个功能域（同一实现、分属三 feature 契约）：
...
|---|---|---|
| `agent/attempt/start` | attemptId、executionId（request 发起时有）、sessionId、seq、observedAt | activity 开始/执行中证据（observed） |
...
- 自检/版本/owner/无双跑：同 interaction R11 语义（fork 既有自检延续 + 切片契约 probe）；失败 ⇒ log + 切片不激活 + 官方契约行为照常（投影降级），绝不双跑、绝不留下官方行禁用而无替代的空洞。
...
|---|---|---|---|
| `dsh-agent-loop`（`agent-loop` 行，既有 owner 包） | attempt 终态/排队事实、supersede/abort observed 判定 | **采纳 R（共享切片）** | fork 完整复刻官方行（R2）；事实只在真实提交点发出；无该切片时相应能力只是降级而非丢失 |
...
- availability：含 fidelity 分层（Requirement 10 AC2）；健康/诊断归 diagnostics。
- 生命周期：owner 初始化 = probes → store 懒建 → observer hub；停用 = hub → adapters（含切片订阅）→ store；切片订阅随切片 lifecycle 联动，绝不留下 stale 订阅。
- stale containment：写 store/通知前检查 owner/epoch/terminal 资格；切片事实与官方事件同等遵守。

## Testing Strategy

1. 纯契约与降级规则（R2/R3/R6/R8）：derivation 表逐行 fixture；overclaim（silence→success、reconstructed→observed）断言不可能。
2. 切片（R9）：官方契约 parity、版本错配、boot 自检、owner 冲突、无双跑、移除恢复；切片 active 时 observed 终态/排队 fixture；inactive 时 reconstruction/unknown fixture。
3. 官方源绑定（R1/R4/R5）：真实/契约保真 fixture；两 synthetic 插件反序。
4. 重连/stale/重启（R6/R7）：迟到事件、重复 durable、cursor gap、epoch rollover、旧 disposer、host 重启后 observed→reconstructed 降级。
5. redaction/audience（R2/R11）：snapshot/日志敏感字段机械断言。
6. availability（R10）：逐源缺失 + 切片 inactive 的 status/reason 断言；P1/P2 路径。
...
- `durable-state-and-scope.md`: partially applicable。投影与切片事实均非 durable；重启降级规则显式（R6 AC2）；无新 scope/存储。
...
4. **attempt 事实不冒充 durable**：in-memory 事件、重启丢失 → 降级规则兜底；不为此把投影改成 durable writer（边界 R11）。
5. **host-only v1**：client 原始投影后置（同 v1 偏离注）。
...
4. 契约 §7.1 共享文件边界：本线在并行期只写 `docs/specs/session-activity-projection/**`；`packages/agent-loop` 共享切片由 interaction/activity/checkpoint 三线以契约登记方式共同指向同一 owner 包实现，集成波统一装配与注册（feature-list §3.1 联合登记行）。

## Design Completion Condition

本设计覆盖 v2 requirements（R1–R13）：共享切片契约与保真分层成文；数据模型含 fidelity 字段与降级规则；R 决策表逐组件证据齐全；残余 C 附证据与退役条件；registry/catalog 拟新增行齐备；失败/guard 策略逐钩子声明。用户确认前的修订就地更新本文与 requirements 对应条目。
