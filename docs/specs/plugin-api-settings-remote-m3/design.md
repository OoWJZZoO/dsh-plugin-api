...
The leaf uses the official `isTypertRemoteSegment`, `bindTypertRemote`, and `remoteMethods` exports. The remote service is a feature-local service instance registered with `ctx.reflect.provide`, so the returned disposer is the official fiber-owned provider disposer. Its dedicated prototype prevents the official Remote marker table from ever being attached to `Object.prototype`. The object carries the frozen `typertRemote` binding and two marked methods:

- `get()` reads `settings.get(namespace)`, obtains the matching `settings.describe({ redactSecrets: true })` descriptor, and returns a detached JSON-safe `{ value }` snapshot.
...
The `settingsRemote` guard is append-only in `lib/guards.js` and probes `ctx.get`, the settings service (`describe`/`get`/`mutate`), `ctx.reflect.provide`, and the official Typert protocol (`TypertRemoteService`, `isTypertRemoteSegment`, `bindTypertRemote`, `remoteMethods`, `Remote`). Guard failures are P2 and do not alter the optional M1 settings service behavior.
