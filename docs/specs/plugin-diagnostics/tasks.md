...
- [x] 1.2 实现 `redactDiagnosticValue` 纯函数：secret 键（token/secret/authorization/credential/password/api-key/cookie 等）值一律 `[redacted]`；`evidence` 仅保留 `{package, version, runtime, capability}` 四个 bounded 字段、其余键丢弃；`boundedDetail` 限长（≤ 200 字符）并剥离多行/控制符；nested depth 超过 3 折叠为 `[bounded]`；脱敏/分类过程抛错时 fail-closed 产出 `[redaction-failed]` 标记（PD-6）。
- [x] 1.3 单元测试 `test/diagnostics-normalize.test.mjs`：
...
- [x] 2.1 实现 `createDiagnosticsOwner({ ctx, logger, coreActive, registry, publication, probeTimeoutMs })`：
...
- [x] 2.2 单元测试（并入 `test/diagnostics.test.mjs`，按主题分 test 块）：
...
- [x] 3.1 实现 probe 调度器：注册后并行发起 `run({signal, generation})`（独立检查并发，互不阻塞）；结果经 1.x 规范化后交给 reducer（见任务 4）。
...
- [x] 3.2 单元测试（并入 `test/diagnostics.test.mjs`）：
...
- [x] 4.1 实现 reducer 与提交路径：规范化后的结果首次提交记录 `firstObservedAt`、每次提交刷新 `lastUpdatedAt`（ISO 字符串）；与既有 committed snapshot 结构等价（fingerprint 比较）时 no-op（等价更新合并，PD-5 前置）。
...
- [x] 4.2 实现 scope index 查询：`get({scope, ownerId?, audience?})` 只返回该 scope（+可选 ownerId 过滤）的 committed 快照，按 `(ownerId, checkId)` 确定性排序；返回 frozen `{scope, checks, state}`，`state` 聚合规则：any `failed` ⇒ `failed`；否则 any `degraded|pending|unknown` ⇒ `degraded`；否则 checks.length>0 且全 `healthy` ⇒ `healthy`；否则（含空）⇒ `unknown`（PD-2/PD-3/PD-7）。
...
- [x] 4.3 单元测试（并入 `test/diagnostics.test.mjs`）：
...
- [x] 5.1 实现通知总线：`onChange({scope, ownerId?}, listener)` 注册；listener 收到 `{snapshot, observerEpoch}`（快照为已提交 frozen snapshot 或稳定引用）。
...
- [x] 5.2 单元测试（并入 `test/diagnostics.test.mjs`）：
...
- [x] 6.1 实现可选 client publication：owner 接受 `publication`（设计决策 6）。
...
- [x] 6.2 单元测试（并入 `test/diagnostics.test.mjs`）：
...
- [x] 7.1 `lib/guards.js`：追加一个 `else if (featureName === 'diagnostics')` 分支——唯一有意义的载荷探针 `ctx.get`（“diagnostics cannot resolve official services for host evidence”），deps 不新增；不 star 其他分支（契约 §3）。命名完全中立，无治理字母（AGENTS §6）。
- [x] 7.2 `lib/plugin-api-service.js`（对齐 sessionRoute 槽位模板；契约 §3 共享文件写入模式，见 §4 偏差登记）：
...
- [x] 8.1 `docs/specs/plugin-api-features/feature-list.md`：追加本 feature 的 §2 namespace 小节（`pluginApi.diagnostics` 形状与三方法、词汇、scope/owner/audience/epoch 语义，含 `{scope, checks, state}` 包装与 client publication 的 clientAvailability 呈现，命名与 design §Data Models 保持一致/以 design 解释落点）与 §7 登记行（Feature=plugin-diagnostics、范围=PD-1–PD-8、状态=delivered、Spec 目录、关键约束/设计一栏）；只改本 feature 对应行（契约 §3）。
- [x] 8.2 验证闸（契约 §7）：
...
## 偏离与整合议题登记（§4 落实 + 实现期修订注记）

- **B1（已批准）plugin-api-service.js 分支数**：契约 §3 文字为“disabled 工厂 + constructor 一行 + mountFeature 一个分支”；实现按 sessionRoute 槽位模板追加了 `_readSlot('diagnostics')` 与 `_disabledSurfaceFor('diagnostics')` 各一个分支（prepared 事务 rollback 的 `_restoreDisabledSurface` 正确性必需，`_readSlot` 返回 slot api、`_disabledSurfaceFor` 返回 disabled 表面，缺之则回滚无法还原 disabled 表面），属同一共享文件的标准逐分支追加，非跨 feature 语义变更。已随 Stage 3 批准。
- **B2（已批准）既有共享测试断言 = 已知 integration-owned deltas**：契约冻结 `test/index.test.mjs`/`test/index-events.test.mjs` 并声明顺序断言由 integration owner 统一维护；本 leaf **不**触碰任何既有测试文件（避免跨 worktree 冲突/重复数值）。Stage 4 全量 `npm test`：**1094 tests / 1078 pass / 16 fail**，16 项失败全部为新增 `diagnostics` feature 引起的共享 count/order/new-face 断言，经逐一取证无实现缺陷：
  - `features.length === 16`（现 17）断言：`index.test.mjs`（冻结）、`index-events.test.mjs`（冻结）、`index-agent/session/session-durable/system-prompt/tools/llm` 等共享 index-*.test.mjs；
  - `remote` 为 FEATURE_MOUNTERS 末项断言：`index-remote.test.mjs`（`diagnostics` 追加其后）；
  - host-boundary 回归 `featureName`/`featureActivity` 各 +1（`{name:'diagnostics', isActive:true}`）：`official-passthrough-independence.test.mjs`（boundary SHA `3cad40e` 早于 M6；其余 pre-existing 面 systemPrompt/tools/abortShape/remotePublish/servicesNames 逐字节一致，source/artifact audit 通过）。
  - 上述 deltas 需 Wave C integration owner 统一修复（数量 +1、末项、boundary 基准更新），不在本 leaf 处理。
- **B3（已批准）client publication 真实接线**：本阶段实现可选 seam + injected harness 测试（init/change 出版、client-only availability、缺失/不兼容 degradation、破碎出版不抛），**不接 `service.remote`**；真实 host↔client 出版通道接线属 Wave C integration 议题（design §4 “IF available” 的落地归属），本 feature 缺省即显式 client-unavailable、host 保持 active。
- **实现期 spec 修订（无验收边界动摇）**：
  - focused 命令 `test/diagnostics*.test.mjs` → `test/*diagnostics*.test.mjs`（覆盖 `index-diagnostics.test.mjs`，对抗性审查 advisory #1）；
  - 任务 2.1 补充重复注册 bounded 冲突日志（落实 PD-1 “SHALL report the conflict”，对抗性审查 advisory #2）；
  - 任务 8.1 登记 clientAvailability 与 `{scope, checks, state}` 包装命名与 design §Data Models 对齐（对抗性审查 advisory #3）；
  - 全局终审后：新增 apply 级**隔离**集成测试「a sibling feature failing keeps the diagnostics feature and the rest of the facade active」（`tools` guard 失败 → `diagnostics` 仍 active、apply 不抛），并记录 apply 级 diagnostics 自身 guard 失败场景的覆盖口径（见下）；
