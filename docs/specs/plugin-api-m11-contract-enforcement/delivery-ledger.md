...
`scripts/registry-validate.mjs` 本线新增**三条** entry 级校验（前两条随第一交付批落盘，第三条于第十五轮随 `callShape` 回填加入，见 §3.0.4）：
