...
- **未完成子项**：registry 驱动的静态枚举未执行完。**仍未收口**的成员族（第十二轮重新核对；`executions.recovery.*`、`executions.visibility.register`、四族 `*.decisions.register` / `*.executionPolicies.register` / `*.assemblyPolicies.register`、`security.{policy,redaction,egress}.register`、`llm.adapters.*` 已收口）：`sessions.channels.auth.{register,pairingProvider.register}` 与 `sessions.channels.redaction.register`（owner 仍是常量 root）、`skills.activation.register`、client `lifecycle.register`、`tools.executionMode.register`（**注**：该官方动词是分类查询、不是注册，registry 行文字漂移，与本项无关）；generation 校验提升仍受阻于 `skills.activation.*` 两处委派型 handle。
