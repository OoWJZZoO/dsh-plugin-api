...
| 中级 M-1′（边界口径与台账不符，部分本仓族未被断言） | 口径与实现对齐：`NATIVE_DISPATCHER_ROWS` 之外，可观测性由「official seam 且非本仓实现」判定；同时把 `storage.*` 与 `llm.routing.*` 纳入断言（host 断言行数 106，其中新增 live storage facility；routing owner 在本 harness 无法构造） |
...
| B5（Task 3.1 registry 半） | **已完成**（第十五轮）— 全 562 行回填 `callShape`（79 async / 256 sync / 227 not-applicable）、词表与 validator 校验落盘、`test/call-shape.test.mjs` 断言可观测面（host 107 行 + client 24 行）；判定口径与两条修订见 §3.0.4 |
