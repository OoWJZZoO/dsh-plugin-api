...
batch-2 交付（`48c2537`）后的一次独立实效复核（在装配后的活面上逐成员实际调用，不看台账自洽）发现 6 处残留偏差，其中 1 处为 batch-2 引入的功能性回归。这些修补按 **ANY** 工作流（AGENTS §3.0.2）立项，逐项修补点、复核排除项、开放项与执行记录见同目录 [`repair-ledger.md`](./repair-ledger.md)。其中原列 ANY 边界外的健康证据面（`llm.routing.health.observe`）经人类授权后于同日第二轮处置（改名 `llm.routing.health.report`，见该文件 §7）。本节的结论以该文件的「§6 执行与验证记录」与「§7 追加修补」为准。
