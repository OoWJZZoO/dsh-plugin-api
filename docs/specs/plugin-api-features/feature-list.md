...
| U10 `mcp-catalog-lifecycle` | 官方 `dsh-mcp-client` 无 dispatch 点；忠实 host 复刻之上增加只读 server/tool catalog 与 lifecycle 投影 | `mcp-client`（`dsh-mcp-client`，610 行，当前 host-only） | 中 | 高 | **本次交付**（运行时名 `plugin-api-mcp`，owner `@deepseek-ai/dsh-mcp-client`；U10 保留为上游提案，replacement 为 current workaround，退役条件见 §3 U10 行） |
