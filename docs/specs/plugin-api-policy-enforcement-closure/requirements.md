...
6. WHEN the egress policy authority is unavailable or a policy evaluation fails on an official outbound path THEN the owning component SHALL keep the official outbound behavior (egress is a denylist whose default is allow), report the affected path as degraded or unavailable, and SHALL NOT block the side effect on the basis of a missing authority or a failed callback.
7. WHEN a cooperative third-party plugin owns an outbound path THEN `security.egress` SHALL expose a supported public decision or authorization interface that uses the same target vocabulary, reducer, denylist default, lifecycle, and audit authority as official automatic enforcement.
...
- `docs/standards/api-idioms.md`: applicable. M8 delivered the current public idiom contract into the canonical registry and the standards corpus; this feature follows those current facts while correcting proven M8 accounting and enforcement gaps.
