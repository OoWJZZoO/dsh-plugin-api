...
export function createSessionActivityProjection({ ctx, logger, enabled = true, observe = true, now } = {}) {
  ...
  const view = createSessionActivityView({ store })
  ...
  const adapters = createSessionActivityAdapters({ ctx, store: observedStore, logger })
  const availabilityProbe = createSessionActivityAvailability({
    enabled,
    adapters,
    sliceState: () => adapters.sliceState(),
  })
  ...
  return {
    ...
    setSliceState: (state) => adapters.setSliceState(state),
    ingestAttemptFact: (event, payload) => adapters.ingestAttemptFact(event, payload),
    joinDurable: (session) => adapters.joinDurable(session),
