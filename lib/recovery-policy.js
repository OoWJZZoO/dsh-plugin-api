 ...
 * inherits a neighbour's status. Wave 4 binds each path to its official owner.
 */
...
export function createRecoveryPolicyOwner({ logger, now, idFactory } = {}) {
  ...
  const evaluate = async (rawInput) => evaluateWindow(rawInput, 'cooperative')
  ...
  const decide = async (rawInput) => {
    const decision = await evaluateWindow(rawInput, 'automatic')
    return deepFreeze({ ok: true, decision })
  }
  ...
  const status = (path) => {
    const known = RECOVERY_AUTOMATIC_PATHS.includes(path)
    if (!known) {
      return deepFreeze({ status: 'unavailable', reason: 'unregistered path', observedAt: currentTime() })
    }
    // Wave 4 binds the automatic owners; until then every path reports
    // unavailable truthfully and never inherits a neighbour's status.
    return deepFreeze({ status: 'unavailable', reason: 'automatic consumption owner not bound', observedAt: currentTime() })
  }

  /** Public selfDescription coverage projection. */
  const coverage = () => {
    const automatic = {}
    for (const path of RECOVERY_AUTOMATIC_PATHS) automatic[path] = 'unavailable'
    const registration = disposed ? 'unavailable' : 'active'
    const cooperative = disposed ? 'unavailable' : 'active'
    const overall = disposed ? 'unavailable' : 'degraded'
    return deepFreeze({
      status: overall,
      registration,
      cooperative,
  ...
  const api = Object.freeze({
    ...
    availability,
    _internal: Object.freeze({ decide, commit, status }),
  })
