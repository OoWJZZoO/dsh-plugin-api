...
export function createRecoveryPolicyOwner({ logger, now, idFactory } = {}) {
  ...
  const consume = (decisionId, options = {}) => {
    const id = boundedString(decisionId, MAX_ID)
    const record = id ? decisions.get(id) : undefined
    if (!record) throw new RecoveryPolicyConsumeError('decision is unavailable or stale')
    if (record.consumed) throw new RecoveryPolicyConsumeError('decision has already been consumed')
    const suppliedAttempt = boundedString(read(options, 'attemptId'), MAX_ID)
    if (record.expectedAttemptId && suppliedAttempt !== record.expectedAttemptId) {
      throw new RecoveryPolicyConsumeError('attempt identity does not match the decision')
    }
    if (!record.expectedAttemptId && record.public.execution?.attemptId
      && suppliedAttempt !== record.public.execution.attemptId) {
      throw new RecoveryPolicyConsumeError('attempt identity does not match the decision')
    }
    record.consumed = true
    const receipt = {
      decisionId: record.public.decisionId,
      action: record.public.action,
      consumed: true,
      attemptId: suppliedAttempt ?? record.public.execution?.attemptId,
  ...
  const projectVisibility = (decision, options = {}) => {
    try {
      if (!isObject(decision)) return undefined
      const audience = boundedString(read(options, 'audience'), 30) ?? 'debug'
      if (audience === 'model' && read(options, 'elevate') !== true) return undefined
      const policyId = boundedString(read(options, 'policyId'), MAX_ID)
      const policy = policyId ? visibilityPolicies.get(policyId) : undefined
      const fields = policy?.current ? new Set(policy.fields) : new Set(['action', 'reason', 'source', 'observedAt', 'uncertainty'])
      const result = {}
      if (fields.has('action')) result.proposedAction = boundedString(read(decision, 'action'), 30)
      if (fields.has('reason')) result.reason = clonePublicValue(read(decision, 'reason'))
      if (fields.has('source')) result.source = clonePublicValue(read(decision, 'reason')?.source)
      if (fields.has('observedAt')) result.observedAt = boundedString(read(decision, 'observedAt'), MAX_STRING)
      if (fields.has('uncertainty')) result.uncertainty = audience === 'model' ? 'explicitly-elevated' : 'bounded'
      if (fields.has('evidenceProvenance')) result.evidenceProvenance = clonePublicValue(read(decision, 'evidenceProvenance'))
      return deepFreeze(result)
    } catch {
  ...
  const api = Object.freeze({
    ...
    capability: Object.freeze({ declare: declareCapability }),
    policy: Object.freeze({ register: registerPolicy }),
    evaluate,
    consume,
    visibility: Object.freeze({ register: registerVisibility, project: projectVisibility }),
    availability,
    adapters: createRecoveryAdapters(),
  })
