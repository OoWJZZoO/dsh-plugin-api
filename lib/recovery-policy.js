...
/**
 * Registered automatic consumption paths (from the policy inventory
 * enforcement matrix). A path outside this set reports unavailable and never
 * inherits a neighbour's status. The automatic consumption owners bind each
 * path through the host facade.
 */
export const RECOVERY_AUTOMATIC_PATHS = Object.freeze([
  'recovery/model',
  'recovery/tool',
  'recovery/task',
...
function normalizeCapability(input) {
  ...
  // The declaring plugin's identity is not this member's subject: the pair
  // names the operation under recovery (its scope owner and scope generation),
  // so it carries domain names instead of borrowing the caller-identity one.
  const scopeOwner = identity(read(input, 'scopeOwner'), 'capability scopeOwner')
  const scopeGeneration = identity(read(input, 'scopeGeneration'), 'capability scopeGeneration')
...
export function createRecoveryPolicyOwner({ logger, now, idFactory } = {}) {
  ...
  // official consumption owner is wired. A path without a probe (or a probe
  // that reports false) truthfully reports unavailable and never inherits a
  // neighbour's status.
  ...
  const capabilityKey = (scopeOwner, operationId) => `${scopeOwner}\u0000${operationId}`
  const capabilityLookup = (value) => {
    if (!isObject(value)) return undefined
    const operationId = boundedString(read(value, 'operationId'), MAX_ID)
    const scopeOwner = boundedString(read(value, 'scopeOwner'), MAX_ID)
    const scopeGeneration = boundedString(read(value, 'scopeGeneration'), MAX_ID)
    if (!operationId || !scopeOwner || !scopeGeneration) return undefined
    const record = capabilities.get(capabilityKey(scopeOwner, operationId))
    if (!record || !record.current || record.public.scopeGeneration !== scopeGeneration) return undefined
  ...
  const declareCapability = (input) => {
    ...
    const key = capabilityKey(publicRecord.scopeOwner, publicRecord.operationId)
    ...
    let active = true
    // The handle keeps the standard registry identity member names; on this
    // member they carry the declaration's scope pair (the recovered operation),
    // which is what the member's registration is about.
    return Object.freeze({
      id: publicRecord.operationId,
      ownerId: publicRecord.scopeOwner,
      generation: publicRecord.scopeGeneration,
      dispose: () => {
        if (!active) return staleResult('the capability declaration is already released')
        active = false
        if (capabilities.get(key) !== record) return staleResult('the capability declaration was superseded')
  ...
  const registerPolicy = (input) => {
    ...
    if (previous && previous.ownerId !== ownerId) {
      throw new RecoveryPolicyRegistrationError(`recovery policy "${id}" is already registered by another owner`)
    }
    if (previous) previous.current = false
    const record = {
      id,
    ...
    return Object.freeze({
      ...
      generation,
      dispose: () => {
        if (!active) return staleResult('the policy registration is already released')
        active = false
        if (policies.get(key) !== record) return staleResult('the policy registration was superseded')
        record.current = false
        policies.delete(key)
        return releasedResult()
      },
  ...
  /**
   * Evaluate one decision window. Returns `{ decision, matched }` where
   * `matched` counts the current policies that matched this failure. A `null`
   * decision means "no registered policy matched and no authority gate fired":
   * the automatic consumer falls back to the documented official default
   * (e.g. the official retry waterfall) for this window.
   */
  const evaluateWindow = async (rawInput, channel) => {
    const input = normalizeInput(rawInput)
    if (disposed) return { decision: safeDecision(input, 'recovery-unavailable'), matched: 0 }
    ...
    if (windowKey && windows.has(windowKey)) {
      ...
      if (existing) return { decision: existing.public, matched: 0 }
    }
    const finishGate = (decision) => {
      if (windowKey) rememberDecision(windowKey, decision, input, channel)
      return { decision, matched: 0 }
    }
    if (input.execution?.terminalOutcome) {
      return finishGate(safeDecision(input, 'terminal-outcome-committed'))
    }
    ...
    if (cancelled()) {
      return finishGate(safeDecision(input, 'recovery-cancelled', 'cancellation'))
    }
    const boundary = boundaryRequest(rawInput)
    if (boundary) {
      log(`unsupported-boundary:${boundary}`)
      return finishGate(safeDecision(input, 'unsupported-boundary', 'boundary'))
    }
    if (read(rawInput, 'budgetExhausted') === true || read(rawInput, 'usageBudgetExhausted') === true
      || read(rawInput, 'attemptsRemaining') === 0) {
      return finishGate(safeDecision(input, 'retry-budget-exhausted', 'budget'))
    }
    if (read(rawInput, 'deadlineExpired') === true) {
      return finishGate(safeDecision(input, 'retry-deadline-exhausted', 'deadline'))
    }
    ...
    if (selected) {
      if (windowKey) rememberDecision(windowKey, selected, input, channel)
      return { decision: selected, matched }
    }
    if (matched > 0) {
      const decision = safeDecision(input, terminalSafeCode(input, lastError))
      if (windowKey) rememberDecision(windowKey, decision, input, channel)
      return { decision, matched }
    }
    // No policy matched and no gate fired. The automatic channel falls back to
    // the documented official default (decision: null, no consumption
    // recorded); the cooperative channel keeps its documented safe-default
    // outcome.
    if (channel === 'automatic') return { decision: null, matched }
    return { decision: safeDecision(input, terminalSafeCode(input, lastError)), matched }
  }

  function rememberDecision(windowKey, decision, input, channel) {
    decisions.set(decision.decisionId, {
      public: decision,
      consumed: false,
      expectedAttemptId: decision.proposedAttemptId,
      channel: channel === 'automatic' ? 'automatic' : 'cooperative',
      ownerId: boundedString(read(input?.raw, 'ownerId'), MAX_ID),
      generation: boundedString(read(input?.raw, 'generation'), MAX_ID),
      executionId: input?.execution?.executionId,
  ...
  /**
   * Public cooperative evaluation. The returned decision is the recovery
   * decision projection: when visibility policies are registered, the record
   * is limited to the union of their declared audience fields (recovery-
   * visibility consumption at the projection point).
   */
  const projectDecision = (decision) => {
    if (!decision) return decision
    const allowed = new Set()
    let anyPolicy = false
    for (const record of visibilityPolicies.values()) {
      if (!record.current) continue
      anyPolicy = true
      for (const field of record.fields) allowed.add(field)
    }
    if (!anyPolicy || allowed.size === 0) return decision
    const projected = {
      decisionId: decision.decisionId,
      ...(decision.execution ? { execution: decision.execution } : {}),
    }
    for (const field of ['action', 'reason', 'source', 'observedAt', 'uncertainty', 'evidenceProvenance']) {
      if (allowed.has(field) && decision[field] !== undefined) projected[field] = decision[field]
    }
    return deepFreeze(projected)
  }

  const evaluate = async (rawInput) => projectDecision((await evaluateWindow(rawInput, 'cooperative')).decision)

  const hasCurrentPolicy = () => {
    for (const record of policies.values()) if (record.current) return true
    return false
  }

  /**
   * Internal automatic decision entry for one operation window. Evaluates the
   * active recovery policy generation once (window reuse) and records the
   * decision with automatic channel attribution for the single-consumption
   * guard. When no recovery policy is registered (or none matches and no
   * authority gate fires), `decision` is `null` and the consumer keeps its
   * documented official default behavior — the authority never silently
   * blocks or approves in that case.
   */
  const decide = async (rawInput) => {
    if (disposed || !hasCurrentPolicy()) {
      return deepFreeze({ ok: true, decision: null, consultedPolicies: 0 })
    }
    const { decision, matched } = await evaluateWindow(rawInput, 'automatic')
    if (decision === null) return deepFreeze({ ok: true, decision: null, consultedPolicies: matched })
    return deepFreeze({ ok: true, decision, consultedPolicies: matched })
  }

  /**
   * Internal atomic single-consumption commit. Guards owner / generation /
   * execution / attempt identity, terminal and cancellation state, then marks
   * the decision consumed. A stale or duplicate commit is a typed conflict and
   * never produces a second effect.
   */
  const commit = async (decision, operation) => {
    const at = currentTime()
    const decisionId = boundedString(read(decision, 'decisionId'), MAX_ID)
    if (!decisionId) {
      return deepFreeze({ ok: false, code: 'invalid-input', reason: 'decision must carry a decisionId', operation: 'commit', observedAt: at })
    }
    const record = decisions.get(decisionId)
    if (!record) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'unknown or superseded decision', operation: 'commit', observedAt: at })
    }
    if (record.consumed) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'decision already consumed', operation: 'commit', observedAt: at })
    }
    const op = read(operation, 'operation') ?? operation
    const ownerId = boundedString(read(op, 'ownerId'), MAX_ID)
    const generation = boundedString(read(op, 'generation'), MAX_ID)
    const executionId = boundedString(read(op, 'executionId'), MAX_ID)
    const attemptId = boundedString(read(op, 'attemptId'), MAX_ID)
    if (ownerId && record.ownerId && ownerId !== record.ownerId) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'owner identity mismatch', operation: 'commit', observedAt: at })
    }
    if (generation && record.generation && generation !== record.generation) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'policy generation mismatch', operation: 'commit', observedAt: at })
    }
    if (executionId && record.executionId && executionId !== record.executionId) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'execution identity mismatch', operation: 'commit', observedAt: at })
    }
    if (attemptId && record.attemptId && attemptId !== record.attemptId) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'attempt identity mismatch', operation: 'commit', observedAt: at })
    }
    if (read(op, 'terminal') === true || read(op, 'aborted') === true || read(op, 'cancelled') === true) {
      return deepFreeze({ ok: false, code: 'conflict', reason: 'operation is terminal or cancelled', operation: 'commit', observedAt: at })
    }
    ...
    return deepFreeze({
      ...
      attemptId: record.attemptId ?? record.expectedAttemptId,
      observedAt: at,
    })
  }

  const pathStatus = (path) => {
    if (!RECOVERY_AUTOMATIC_PATHS.includes(path)) return 'unavailable'
  ...
  /** Internal per-path coverage state (registration/cooperative/automatic). */
  const status = (path) => {
    if (path === 'recovery') {
      let active = 0
      for (const known of RECOVERY_AUTOMATIC_PATHS) if (pathStatus(known) === 'active') active += 1
      const aggregate = active === RECOVERY_AUTOMATIC_PATHS.length ? 'active' : active > 0 ? 'degraded' : 'unavailable'
      return deepFreeze({ status: aggregate, observedAt: currentTime() })
    }
    if (!RECOVERY_AUTOMATIC_PATHS.includes(path)) {
      return deepFreeze({ status: 'unavailable', reason: 'unregistered path', observedAt: currentTime() })
    }
    return deepFreeze({ status: pathStatus(path), observedAt: currentTime() })
  }

  /**
   * Install automatic path binding probes (host facade only). Each probe is
   * read lazily at coverage/status time so late-binding component owners
   * (replacement rows mounting after the facade) are reflected truthfully.
   */
  const bindProbes = (probes) => {
    if (!isObject(probes)) return false
    for (const [path, probe] of Object.entries(probes)) {
      if (!RECOVERY_AUTOMATIC_PATHS.includes(path)) continue
  ...
  /** Public selfDescription coverage projection. */
  const coverage = () => {
    const automatic = {}
    let activeCount = 0
    for (const path of RECOVERY_AUTOMATIC_PATHS) {
      const value = pathStatus(path)
      automatic[path] = value
      if (value === 'active') activeCount += 1
    }
    const registration = disposed ? 'unavailable' : 'active'
    const cooperative = disposed ? 'unavailable' : 'active'
    const overall = disposed
      ? 'unavailable'
      : activeCount === RECOVERY_AUTOMATIC_PATHS.length ? 'active' : 'degraded'
    return deepFreeze({
      status: overall,
      registration,
      cooperative,
  ...
  const registerVisibility = (input) => {
    ...
    if (previous && previous.ownerId !== ownerId) {
      throw new RecoveryPolicyRegistrationError(`recovery visibility policy "${id}" is already registered by another owner`)
    }
    if (previous) previous.current = false
    ...
    return Object.freeze({
      ...
      generation,
      dispose: () => {
        if (!active) return staleResult('the visibility registration is already released')
        active = false
        if (visibilityPolicies.get(key) !== record) return staleResult('the visibility registration was superseded')
        record.current = false
  ...
  const api = Object.freeze({
    ...
    availability,
    _internal: Object.freeze({ decide, commit, status, bindProbes }),
  })
