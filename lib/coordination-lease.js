...
export function createCoordinationLease({ ctx, logger, now, idFactory } = {}) {
  ...
  const failed = (code, { resource, operation, observed, reason } = {}) => deepFreeze({
    ok: false,
    code,
    ...(resource ? { resource } : {}),
    operation,
  ...
  async function watch(resource, options = {}) {
    ...
    subscription.current = async () => {
      if (subscription.disposed) {
        return deepFreeze({ code: 'inactive', resource: normalized.value, epoch: subscription.epoch, observedAt: observedAt(), availability: setEpoch() })
      }
      ...
      if (!found.ok) {
        return deepFreeze({
          ...
          availability: setEpoch(found.code === 'unknown' ? 'backend state could not be verified' : 'no current record'),
        })
      }
