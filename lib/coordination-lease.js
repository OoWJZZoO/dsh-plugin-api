...
export function createCoordinationLease({ ctx, logger, now, idFactory } = {}) {
  ...
  // Synchronous by contract: every input the answer depends on is resolved at
  // mount time (caps + epoch), so a general probe never reads a pending value.
  ...
  async function watch(resource, options = {}) {
    ...
    const onEvent = (event) => {
      ...
      if (sinceGeneration !== undefined && !subscription.resyncReported) {
        ...
        if (event.generation !== sinceGeneration) {
          created.publish(deepFreeze({ ...event, resync: true, epoch: subscription.epoch }))
          return
        }
        created.publish(deepFreeze({ ...event, resync: false, epoch: subscription.epoch }))
        return
      }
    ...
    const base = created.handle
    return Object.freeze({
      ...base,
      // The unsupported-backend notice is a first-subscription delivery; the
      // kernel owns additive listeners, the wrapper only adds the notice.
      subscribe(listener) {
        const off = base.subscribe(listener)
        if (watchUnsupported && !subscription.unsupportedReported && typeof listener === 'function') {
          ...
          try {
            ...
            // containment: the notice never throws into the subscriber
          }
        }
        return off
      },
