...
function createAgentBaseFactory({ active, createAgentLeaf }) {
  ...
  const enter = unavailable('providers.enter')
  const announce = unavailable('providers.announce')
  const setFactory = unavailable('providers.setFactory')
  const availability = Object.freeze({
    create: false,
    resume: false,
    register: false,
    providers: Object.freeze({ enter: false, announce: false, setFactory: false }),
  })
  ...
  return (consumerCtx) => {
    const view = {
      ...
      resume,
      register,
      providers: Object.freeze({ isActive: false, enter, announce, setFactory }),
      availability,
    }
...
function mountLlmFeature({ ctx, service, featureRegistry }) {
  ...
  service.mountFeature('llm', mergeFeatureSurface(llmApi, llmLeaf.api, nested))
...
function mountSkillsActivationFeature({ ctx, service, featureRegistry, logger, auxiliaryManifests, facadeContract }) {
  ...
  const ownerApi = {
    registerDescriptor: invoke('registerDescriptor'),
    registerSkill: invoke('registerSkill'),
