...
import { createProfileMutation, installedRuntimeVersion, facadeApiProtocol } from './profile-mutation.js'
...
function mountProfileFeature({ ctx, service, featureRegistry, logger, facadeContract, auxiliaryManifests }) {
  ...
  try {
    ...
    // The mutation face is a lazy per-caller provider: write verbs surface
    // even when the executor is absent (typed unavailable), while reads stay
    // intact. The provider receives the caller context so owner binding is
    // minted from the caller fiber (never from a bare caller-supplied
    // string).
    ...
    // boot-init orphan scan: fire once the executor handshake succeeds.
