...
function createAgentBaseFactory({ active }) {
  ...
  const availability = Object.freeze({
    ...
    resume: false,
    register: false,
    providers: Object.freeze({ register: false }),
  })
  ...
  return (consumerCtx) => {
    return {
      ...
      resume,
      register,
      providers: Object.freeze({ isActive: false, register: providersRegister }),
      availability,
    }
  }
}
...
function mountSkillsActivationFeature({ ctx, service, featureRegistry, logger, auxiliaryManifests, facadeContract }) {
  ...
  const ensure = () => {
    ...
    if (activation === null) {
      throw new PluginApiFeatureDisabledError('skillsActivation', 'the tool-skill replacement is not active or its version is mismatched')
    }
    return activation
  }
  ...
  const ownerApi = {
    ...
    register(spec) {
      const activation = ensure()
      if (spec && typeof spec === 'object' && 'skill' in spec) {
    ...
    availability() {
      ...
      const inner = typeof activation.availability === 'function' ? activation.availability() : {}
      return Object.freeze({ active: true, contract: true, versionMatch: versionOk === true, ...inner })
    },
  }
  let prepared
