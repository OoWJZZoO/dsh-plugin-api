...
import { eventsCatalog, mergeEventCatalogs } from './events-catalog.js'
...
function mountEventsFeature({ ctx, service, featureRegistry, logger }) {
  const includeTools = typeof featureRegistry?.isActive === 'function' && featureRegistry.isActive('tools')
  let catalog
  try {
    catalog = includeTools
      ? mergeEventCatalogs(getComposedEventsCatalog(), toolsEventsCatalog)
      : getComposedEventsCatalog()
  } catch (error) {
  ...
  // Idempotent re-apply: the mounted catalog must match the current composition state.
  const catalogMatches = Boolean(existingCatalog) &&
    (includeTools ? existingCatalog['tools/change'] !== undefined : existingCatalog['tools/change'] === undefined) &&
    existingCatalog['session/created'] !== undefined

  if (catalogMatches) {
    return () => {}
  }

  const events = createEventsBus({ ctx, catalog, logger, featureRegistry })
...
function mountLlmFeature({ ctx, service }) {
  // Idempotent re-apply: a reused service already carries the mounted API.
...
function mountSystemPromptFeature({ ctx, service, logger }) {
  // Idempotent re-apply: a reused service already carries the mounted API.
  if (service?.systemPrompt?.isActive === true) {
    return () => {}
  }
...
function mountSettingsFeature({ ctx, service, logger }) {
  // Idempotent re-apply: a reused service already carries the mounted API.
  if (service?.settings?.isActive === true) {
    return () => {}
  }
...
export function apply(ctx) {
  ...
  try {
    ...
    for (const [featureName, mount] of FEATURE_MOUNTERS) {
      const featureGuard = runFeatureGuard(featureName, ctx, { dshLlm, dshSystemPrompt })
