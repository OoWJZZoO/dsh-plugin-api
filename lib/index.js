...
import { PluginApiError, PluginApiFeatureDisabledError, PluginApiInactiveError, PluginApiSettingsNamespaceError } from './errors.js'
...
function facadeSettingsMutation(method) {
  ...
  // The official authority reports the unknown namespace as a plain Error that
  // names it, while the facade's own namespace guard uses the typed error, so
  // both shapes are recognized instead of only the one the facade mints.
...
function isUnknownSettingsNamespace(error) {
  if (error instanceof PluginApiSettingsNamespaceError) return true
