...
import { installClientRequestRoute } from './client-request-bridge.js'
// client request bridge (integration wave)
...
function mountSessionInteractionFeature({ ctx, service, featureRegistry, logger }) {
  ...
  const route = installClientRequestRoute({
    ...
    handler: async (args, signal) => {
      ...
      // The caller owner is derived host-side from the serving context, never
      // from caller-supplied strings in the wire frame (owner reporting is
      // the host's privilege, never the caller's).
      if (method === 'sessions.request') return owner.request(payload, undefined)
      if (method === 'sessions.cancel') return owner.cancel(payload, undefined)
