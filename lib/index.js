...
function mountSessionDurableFeature({ ctx, service, logger, featureRegistry, sessionDurableAudit } = {}) {
  ...
  epoch = service.mountFeature('sessionDurable', {
    facade,
    closeEpoch() {
      owner.close()
    },
  ...
  return () => {
    ...
    disposed = true
    if (!service.resetSessionDurable(epoch)) return false
    featureRegistry.disable('sessionDurable', 'session durable epoch disposed')
    return true
  }
}
...
export function apply(ctx) {
  ...
  try {
    ...
    for (const [featureName, mount] of FEATURE_MOUNTERS) {
      ...
      let disposer
      try {
        disposer = mount({ ctx, service, featureRegistry, logger, guardResults, sessionDurableAudit })
      } catch (error) {
