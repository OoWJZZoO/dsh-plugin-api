...
function mountEventsFeature({ ctx, service, featureRegistry, logger, guardResults }) {
  ...
  // Transitional idempotent re-apply discriminator (unified in task 2.5):
  // the mounted catalog must match the current guard-driven composition.
...
export function apply(ctx) {
  ...
  try {
    ...
    for (const [featureName, mount] of FEATURE_MOUNTERS) {
      ...
      let disposer
      try {
        disposer = mount({ ctx, service, featureRegistry, logger, guardResults })
      } catch (error) {
