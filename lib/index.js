...
function createAgentBaseFactory({ routeOf, active, createAgentLeaf }) {
  ...
  return (consumerCtx) => {
    ...
    // Official registry leaves degrade per member (P2 'agent'); they must
    // never affect the M0-M3 member surface when the registry is absent.
    let registry
    try {
...
function mountAgentFeature({ ctx, service, featureRegistry, logger }) {
  ...
  const compose = (consumerCtx, base) => {
    ...
    // member-local and can never break the M0-M3 agent read surface.
