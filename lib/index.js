...
import { ATTENTION_UPDATE_EVENT } from './attention-wire.js'
 ...
 * web browser through the api-remotes replacement forwarder (which reads
 * `hubSnapshot(kind)` for its snapshot seed), and is delivered to consumers
 * through the caller-bound attention surface.
 */
function mountAttentionFeature({ ctx, service, featureRegistry, logger }) {
  ...
  const buildSurface = (callerCtx) => {
    ...
    return Object.freeze({
      ...
      // internal forwarding seam: the api-remotes replacement snapshot
      // resolver reads the per-kind redacted snapshot seed here.
      hubSnapshot: (kind) => hub.snapshotAll(kind),
    })
  }
  let prepared
  try {
