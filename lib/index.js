...
function mountLlmFeature({ ctx, service, featureRegistry }) {
  ...
  const nested = typeof llm === 'object' && llm !== null ? {
    providers: Object.freeze({
      register: (...args) => {
        rejectStreamBackendEntry(args[0], 'llm.providers.register')
        return Reflect.apply(llm.registerConfigurableProviders, llm, args)
      },
    }),
    models: Object.freeze({
      register: (...args) => {
        rejectStreamBackendEntry(args[0], 'llm.models.register')
        return Reflect.apply(llm.registerModelDiscovery, llm, args)
      },
      list: () => modelsProjection.list(),
    }),
  } : {}
  ...
  service.mountFeature('llm', mergeFeatureSurface(llmApi, nested))
