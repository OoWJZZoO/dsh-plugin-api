...
import { compactionEventsCatalogSlice } from './compaction-events-catalog.js'
import { sessionTitleEventsCatalogSlice } from './session-title-events-catalog.js'
 ...
 * (requirements 3.2). The mode is decided at this single call, so it stays
 * stable for the host lifetime (requirements 3.3), surviving re-applies via
...
function mountEventsFeature({ ctx, service, featureRegistry, logger, guardResults }) {
  ...
  const events = createEventsBus({ ctx, catalog, rSlices: [compactionEventsCatalogSlice, sessionTitleEventsCatalogSlice], logger })
