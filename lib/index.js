...
function mountAdmissionFeature({ ctx, service, logger, featureRegistry }) {
...
function mountAgentFeature({ ctx, service, featureRegistry }) {
  // Unified idempotent re-apply signal (task 2.5).
  if (typeof featureRegistry?.isActive === 'function' && featureRegistry.isActive('agent')) {
    return () => {}
  }
...
// feature). llm mounts before llm/admission; web is served by the services
// namespace (pluginApi.services.web).
...
export function apply(ctx) {
  ...
  try {
    ...
    // Pass 2: mount each feature whose feature-level guard passed.
    for (const [featureName, mount] of FEATURE_MOUNTERS) {
      const featureGuard = guardResults[featureName]

      if (featureName === 'execRoute' && featureRegistry.isActive(featureName)) {
        continue
      }
      ...
      let mounted
      try {
        mounted = mount({ ctx, service, featureRegistry, logger, guardResults })
      } catch (error) {
        if (featureName === 'execRoute') {
      ...
      const prepared = mounted && typeof mounted === 'object' &&
      ...
      if (prepared) {
        ...
        try {
          ...
          try { mounted.rollback() } catch {}
          const problems = [{ name: featureName, detail: `failed to publish feature: ${error?.message ?? error}` }]
          featureRegistry.disable(featureName, formatProblems(problems))
          service.reportExecRouteP2Once?.('activation', 'publication', problems)
          continue
        }
        try {
          ...
          try { mounted.rollback() } catch {}
          const problems = [{ name: featureName, detail: `failed to activate feature registry: ${error?.message ?? error}` }]
          featureRegistry.disable(featureName, formatProblems(problems))
          service.reportExecRouteP2Once?.('activation', 'registry', problems)
        }
        continue
      }
