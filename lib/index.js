  ...
  if (typeof ctx?.on === 'function') {
    const refreshSlice = () => {
      try {
        ...
        owner.setSliceState({ active: boundary !== null, versionMatched: versionOk })
      } catch {
        owner.setSliceState({ active: false, versionMatched: false })
      }
    }
    // Probe-first: registered before the owner's evidence adapters so the
    // gate opens before the adapters ingest the same dispatch (no observed
    // window is ever lost to an unopened gate).
...
function mountSessionInteractionFeature({ ctx, service, featureRegistry, logger }) {
  ...
  const route = installClientRequestRoute({
    ...
    handler: async (args, signal) => {
      ...
      if (method === 'sessions.request') return owner.request(args.spec ?? {}, undefined)
      if (method === 'sessions.cancel') return owner.cancel(args.input ?? {}, undefined)
...
function mountAttentionFeature({ ctx, service, featureRegistry, logger }) {
  ...
  const route = installClientRequestRoute({
    ...
    handler: async (args, signal) => {
      ...
      if (op === 'contribute') return hub.contribute(args.spec ?? {}, caller)
      if (op === 'dismiss') return hub.dismiss(args.itemId, args.by ?? {}, caller)
      if (op === 'invoke') return hub.invoke(args.itemId, args.actionId, caller)
      if (op === 'snapshot') return hub.snapshotAll(args.kind)
