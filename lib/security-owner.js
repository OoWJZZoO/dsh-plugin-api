...
export function createSecurityOwner({ ctx, logger = {}, now = Date.now, rng, diagnosticsFacade = null } = {}) {
  ...
  // inherits a neighbour's status. Wave 3 binds each path to its official owner.
  ...
  const egressCoverage = () => {
    ...
    for (const path of EGRESS_AUTOMATIC_PATHS) {
      // Wave 3 binds each path to its official owner; until then every path is
      // truthfully reported unavailable (never inherited from a neighbour).
      automatic[path] = 'unavailable'
    }
    const registration = egress ? 'active' : 'unavailable'
    const cooperative = egress ? 'active' : 'unavailable'
    const status = !egress ? 'unavailable' : anyActive ? 'active' : 'degraded'
    return deepFreeze({
      status,
      registration,
      cooperative,
  ...
  const policyStatus = (domainOrPath) => {
    ...
    // from a neighbour), matching the recovery authority vocabulary; Wave 3
    // flips each path to active when its official owner binds.
