...
import { disposeResultOf, staleResult } from './contract-kernel.js'
...
export function createSecurityOwner({ ctx, logger = {}, now = Date.now, rng, diagnosticsFacade = null } = {}) {
  ...
  // The registration owner is the calling plugin's identity: the facade
  // derives it from the caller context and passes it here, so the public
  // register members take only the spec.
  const registerOn = (registry, wrapOnError = true) => (ownerId, spec) => {
    ...
    return Object.freeze({
      ...
      dispose: () => {
        if (disposed) return staleResult('the registration is already released')
        disposed = true
        const state = policyStates.get(key)
        if (state) state.active = false
        return disposeResultOf(originalDispose(), 'the registration was superseded')
      },
    })
  }
  ...
  const api = Object.freeze({
    ...
    get availability() {
      ...
      const faces = Object.freeze({
        ...
        audit: auditStatus.gapSince === undefined ? 'active' : 'degraded',
      })
      // Outer three-value status with the domain detail kept below it: a face
      // that is inert (or an audit with a recorded gap) is a degraded
      // namespace, never an active one, and a facade without any live face is
      // unavailable rather than merely inert.
      const live = Object.values(faces).filter((face) => face !== 'inert').length
      return Object.freeze({
        status: live === 0 ? 'unavailable' : (Object.values(faces).every((face) => face === 'active') ? 'active' : 'degraded'),
        faces,
        seams: Object.freeze({ ...seams }),
        secretPolicy: 'default-deny',
        reportChannel: reportChannel.state,
        audit: Object.freeze(auditStatus),
      })
    },
  })
