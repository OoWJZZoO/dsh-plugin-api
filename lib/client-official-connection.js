  ...
  for (const name of CLIENT_CONNECTION_METHODS) {
    Object.defineProperty(llm, name, {
      ...
      value: function (...args) {
        ...
        let method
        try {
          api = connection.api.llm
          method = api?.[name]
        } catch (error) {
          state.enabled = false
          safeLog(logger, 'dsh-plugin-api client connection method access failed', error)
          throw new PluginApiFeatureDisabledError(feature)
        }
        if (api === null || (typeof api !== 'object' && typeof api !== 'function') || typeof method !== 'function') {
          state.enabled = false
          safeLog(logger, 'dsh-plugin-api client connection method is not callable', new TypeError(`client connection member ${name} is not callable`))
          throw new PluginApiFeatureDisabledError(feature)
        }
