...
/**
 * Normalize an authority anchor-confirmation outcome. The owning authority
 * confirms existence read-only (`ok: true`) or reports a definite absence
 * (`ok: false`); a face without any confirmation mechanism yields the typed
 * `unconfirmable` result (never guessed as present).
 */
export async function normalizeAnchorConfirmation(face, kind, confirm, anchor) {
  if (typeof confirm === 'function') {
    try {
      const outcome = await confirm(anchor)
      if (isObject(outcome) && outcome.ok === true) return { ok: true }
      if (isObject(outcome) && outcome.ok === false) {
        return { ok: false, code: typeof outcome.code === 'string' ? outcome.code : 'missing', reason: boundedString(outcome.reason) ?? `${kind} anchor cannot be confirmed` }
      }
    } catch (error) {
      return { ok: false, code: 'unavailable', reason: `${kind} anchor confirmation failed: ${error instanceof Error ? error.message : String(error)}` }
    }
    return { ok: false, code: 'unconfirmable', reason: `${kind} authority returned an unreadable anchor confirmation` }
  }
  if (typeof face?.get === 'function' && typeof anchor?.id === 'string') {
    try {
      const found = await face.get(anchor.id)
      return found === undefined || found === null
        ? { ok: false, code: 'missing', reason: `${kind} anchor '${anchor.id}' no longer exists` }
        : { ok: true }
    } catch (error) {
      return { ok: false, code: 'unavailable', reason: `${kind} anchor check failed: ${error instanceof Error ? error.message : String(error)}` }
    }
  }
  return { ok: false, code: 'unconfirmable', reason: `${kind} authority cannot confirm the anchor` }
}

/**
 * Normalize a scope-resource existence check. Faces confirm via
 * `sessionKnown`/`get` (session scope) or `workspaceKnown` (workspace scope);
 * a face without any mechanism yields `unknown` (never guessed gone).
 */
export async function normalizeScopeResourceConfirmation(face, kind, resourceId) {
  try {
    if (typeof face?.sessionKnown === 'function' && kind === 'session') {
      const known = await face.sessionKnown(resourceId)
      return known ? { reachable: true } : { reachable: false, reason: `session '${resourceId}' no longer exists or is unreachable` }
    }
    if (typeof face?.get === 'function' && kind === 'session') {
      const found = await face.get(resourceId)
      return found === undefined || found === null
        ? { reachable: false, reason: `session '${resourceId}' no longer exists or is unreachable` }
        : { reachable: true }
    }
    if (typeof face?.workspaceKnown === 'function' && kind === 'workspace') {
      const known = await face.workspaceKnown(resourceId)
      return known ? { reachable: true } : { reachable: false, reason: `workspace '${resourceId}' no longer exists or is unreachable` }
    }
  } catch (error) {
    return { unknown: true, reason: `scope resource check failed: ${error instanceof Error ? error.message : String(error)}` }
  }
  return { unknown: true, reason: 'the owning authority cannot confirm the scope resource in v1' }
}
...
export function createBranchSource({ branches, logger } = {}) {
  ...
  return deepFreeze({
    ...
    confirmAnchor: (anchor = {}) => normalizeAnchorConfirmation(face(), 'branch', face()?.confirmAnchor, { ...anchor, id: anchor.branchId }),
    confirmScopeResource: (resourceId) => normalizeScopeResourceConfirmation(face(), 'session', resourceId),
...
export function createWorkspaceJournalSource({ transactions, logger } = {}) {
  ...
  return deepFreeze({
    ...
    confirmAnchor: (anchor = {}) => normalizeAnchorConfirmation(face(), 'workspace-journal', face()?.confirmAnchor, { ...anchor, id: anchor.transactionId }),
    confirmScopeResource: (resourceId) => normalizeScopeResourceConfirmation(face(), 'workspace', resourceId),
    capture: async ({ resourceId, reason, signal, caller = {} } = {}) => {
      const active = face()
...
export function createWorkspaceSnapshotSource({ snapshot, logger } = {}) {
  ...
  return deepFreeze({
    ...
    confirmAnchor: (anchor = {}) => normalizeAnchorConfirmation(face(), 'workspace-snapshot', face()?.confirmAnchor, { ...anchor, id: anchor.snapshotId }),
    confirmScopeResource: (resourceId) => normalizeScopeResourceConfirmation(face(), 'workspace', resourceId),
    capture: async ({ resourceId, reason, signal, caller = {} } = {}) => {
      const active = face()
...
export function resolveCaptureSource({ kind, scopeKind } = {}, authorities = {}) {
  if (!CAPTURE_SOURCE_KINDS.includes(kind)) {
    const unsupported = deepFreeze({
      ...
      confirmAnchor: async () => ({ ok: false, code: 'unconfirmable', reason: `capture source '${kind}' is not a registered v1 source` }),
      confirmScopeResource: async () => ({ unknown: true, reason: `capture source '${kind}' is not a registered v1 source` }),
    })
    return unsupported
  }
