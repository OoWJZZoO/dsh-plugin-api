 ...
 *
 * Facade class B (single item authority): every attention item
 ...
 * deliberately non-durable: nothing ever leaves hub memory.
 *
...
// ── Vocabulary (shared framing consumed, not redefined) ─────────────────────
...
export function createAttentionHub(options = {}) {
  ...
  function notifyObservers(change) {
    for (const observer of observers) {
      ...
      // The kernel handle owns listener containment and the released-handle
      // no-op rule; a live observer record only ever publishes.
      observer.created.publish(frozen)
    }
  }
  ...
  function validateSpec(spec) {
    ...
    // correlation vocabulary from the shared projection
  ...
  function normalizeCorrelation(correlation, ownerId, caller, isCurrent) {
    ...
    // (verifiability): when no fact-source seam is registered the reference is merely
  ...
  const hub = Object.freeze({
    ...
    contribute(spec, caller) {
      ...
      const handle = Object.freeze({
        ...
        seq: item.seq,
        dispose() {
          if (disposed) return staleResult('the contribution is already withdrawn')
    ...
    invoke(itemId, actionId, caller) {
      ...
      // The fact source is consumed, never judged here.
    ...
    /** Whole-hub redacted snapshot for the forwarding pipeline (rebind/refetch).
     *  When `kind` is given, items whose audience excludes that client kind are
     *  trimmed host-side before they leave the host boundary (fail-closed:
     *  cross-audience content never rides the stream). */
    snapshotAll(kind) {
      sweepAndFlush()
      if (hubNotActive()) return availabilityView()
      const snapshotItems = []
      ...
      for (const item of sorted) {
        ...
        if (kind !== undefined && buildRedactedItemPayload(payload, { kind }) === null) continue
        snapshotItems.push(payload)
      }
      return Object.freeze({
        epoch,
        seq,
    ...
    observe(caller, filters) {
      sweepAndFlush()
      if (hubNotActive()) {
        // A degraded hub answers the same observation contract: the kernel
        // handle carries the availability view as its read face, a no-op
        // subscribe and a discriminated dispose, so callers keep the one
        // observation pattern.
        const degraded = createObserverHandle({
          epoch: hubEpoch(),
          current: () => availabilityView(),
        })
        return degraded.handle
      }
      const observer = {
        caller,
        stale: false,
      ...
      const created = createObserverHandle({
        ...
        current: (live) => {
          if (!live || observer.stale) return Object.freeze([])
          return Object.freeze(
  ...
  function disposeLocked(id, ownerId, ownerGeneration, onDone) {
    ...
    if (!isLive(item)) {
      return staleResult('the contribution is already withdrawn')
    }
    // Cross-owner or stale-generation disposers lose commit eligibility
    // Generation is owner-specific and opaque.
...
export const CODE_REVOKED = 'revoked'
