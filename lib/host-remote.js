...
export function createHostRemoteApi({ ctx, protocol, active = true, logger } = {}) {
  ...
  /**
   * Register one Typert remote service and answer with the standard
   * resourceRegistry handle `{ id, ownerId, generation, dispose() }`.
   *
   * The owner is derived from the caller's context (root token when the caller
   * is untraceable) and the generation is minted by the publication core, so a
   * caller declares neither. Re-registering the same key with the same service
   * reference by the same owner is idempotent and answers with the existing
   * handle; the same key under a different owner or with a different service is
   * a typed conflict, never a silent overwrite.
   *
   * @param {string} serviceKey official Typert service key.
   * @param {object} service service object with callable members.
   ...
   */
  function register(serviceKey, service, callerCtx) {
    return publish(serviceKey, service, callerCtx)
  }

  function publish(serviceKey, service, callerCtx) {
    ...
    // Owner idempotence: same key + same owner + same reference → existing
    // handle, before any shape re-validation / re-home (so an already re-homed object is
    ...
    if (existing && existing.service === service && !existing.disposed) {
      if (existing.ownerId !== ownerId) {
        throw new PluginApiRemoteError(
          `Typert service key "${serviceKey}" is already registered by another owner`,
          { serviceKey },
        )
      }
    ...
    const record = owner.get(serviceKey)
    // The derived caller identity is recorded on the internal publication
    // record so the slot's owner and its handle stay consistent; it is never
    // read from the caller-supplied service definition.
  ...
  function handleFor(record) {
    ...
    const handle = createResourceHandle({
      id: record.serviceKey,
      ownerId: record.ownerId,
      generation: record.generation,
      revoke: () => {
        const released = disposer() === true
        if (released) delete record.handle
        return released
      },
    })
  ...
  return {
    api: Object.freeze({
      register,
      availability: () => Object.freeze({ status: 'active' }),
    }),
    dispose,
  }
}
...
export function createDisabledHostRemoteApi(active = true, reason = 'required host primitive is unavailable') {
  ...
  return Object.freeze({
    register: fail,
    // The disabled face names why it is unavailable, exactly like the live face
    // names its own state.
