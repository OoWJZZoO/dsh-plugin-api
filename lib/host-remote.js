/**
 * Generic host-side Typert Remote publication entry (`pluginApi.remote`).
 *
 * `pluginApi.remote.publish(serviceKey, service)` lets any community plugin
...
export function createHostRemoteApi({ ctx, protocol, active = true, logger } = {}) {
  ...
  function publish(serviceKey, service) {
    ...
    // AC 3.2 — method-name segment grammar.
    for (const method of methods) {
      validateRemoteSegment(protocol, method, `service method "${method}"`)
    }

    // AC 3.3 / 3.5 — signature check: the official gateway derives wire
    ...
    // AC 1.6 / 4.1 — read-only conflict pre-check BEFORE re-home, so a doomed
    // publication never mutates the caller's object (M3). The shared core
    // re-probes as a second safety layer under the same key.
    conflictPreCheck(ctx, owner, serviceKey, service)

    // AC 3.7 / 2.4 — publication-dedicated prototype (same function refs).
 ...
 * contract (or P1 inactive when the core is inactive).
 */
export function createDisabledHostRemoteApi(active = true, reason = 'required host primitive is unavailable') {
  const isActive = typeof active === 'function' ? active : () => Boolean(active)
  const fail = () => {
    if (!isActive()) throw new PluginApiInactiveError()
    throw new PluginApiFeatureDisabledError(HOST_REMOTE_FEATURE, reason)
  }
 ...
 *
 * Accepted shapes (AC 3.1 × 3.7): a plain object literal, a foreign/shared-
 * prototype object with own callable members, or an object this feature already
 * re-homed (methods recovered from the dedicated prototype, D2b). Rejected:
 * `null`, arrays, objects with no callable own members (including a class
 * instance whose methods live only on the class prototype — the B2 narrowing),
...
/**
 * Read-only conflict detection (AC 1.6 / 4.1): a live owner under the same key
