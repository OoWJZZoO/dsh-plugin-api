...
export function createHostRemoteApi({ ctx, protocol, active = true, logger } = {}) {
  ...
  function register(serviceKey, service) {
    return publish(serviceKey, service)
  }
  ...
  return Object.freeze({ isActive: true, publish, register, dispose })
}
...
export function createDisabledHostRemoteApi(active = true, reason = 'required host primitive is unavailable') {
  ...
  return Object.freeze({ isActive: false, publish: fail, register: fail, dispose() {} })
}
