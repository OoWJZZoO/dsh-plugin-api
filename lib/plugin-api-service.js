...
// session-branch facade
function composeSessionApi(baseSessionApi, durableSessionApi, branchesApi, channelsApi, interactionApi, activityApi, availabilityOf) {
  ...
  return class PluginApiService extends Service {
    ...
    constructor(ctx, config) {
      ...
      Object.defineProperty(this, 'sessions', {
        ...
        get() {
          const decorated = this._decoratedNamespaces(['sessions', 'sessions.branches', 'sessions.channels', 'sessions.views', 'sessions.activity'], this._sessionSurface)
          // sessions.planMode: the sub-surface is caller-bound (its write
          // member derives the owner from the calling plugin fiber), so it is
          // materialized per access on top of the cached decorated surface —
          // the same pattern the caller-bound members of `agents`/`events`
          // use. `_sessionPlanModeSurface` always exists, so the namespace
          // never disappears.
          const planModeSlot = this._sessionPlanModeSlot ? this._sessionPlanModeSlot.surface : this._sessionPlanModeSurface
          if (!planModeSlot || !decorated || typeof decorated !== 'object') return decorated
          const callerCtx = this?.ctx ?? this
          const compose = (member) => (member && typeof member === 'object' ? composeSurface(decorated, { planMode: member }) : decorated)
          try {
            return compose(planModeSlot.surfaceFor(callerCtx))
          } catch {
            ...
            try {
              return compose(createDisabledSessionPlanModeApi(() => this._active()).surfaceFor(callerCtx))
            } catch {
              // a broken plan-mode member degrades alone
              return decorated
            }
          }
        },
    ...
    _installScopedPromptContribution(spec, callerCtx, ownerKey) {
      ...
      const handle = Object.freeze({
        ...
        dispose() {
          try {
            const removal = registry.removeScoped(recordToken, ownerId)
