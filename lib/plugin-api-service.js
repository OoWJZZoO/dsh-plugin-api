...
export function createPluginApiService({ apiVersion, registry, coreActive = false, logWriter = writeGuardLog, featureNotice = featureFailNotice, toolAbortedErrorFactory, mcpCatalogProvider = null, attachmentsProvider = null, routePolicyProvider = null } = {}) {
  ...
  return class PluginApiService extends Service {
    ...
    /**
     * Rebuild the public `profile` namespace surface from the two owner
     * slots. The inspection slot is a stable createFeatureSlot surface; the
     * mutation owner is a lazy provider (Task: mutation mounter) that may be
     * absent — its members stay typed-disabled until the owner mounts,
     * matching the namespace shape of the disabled surface.
     */
    /**
