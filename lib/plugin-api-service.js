...
export function createPluginApiService({ apiVersion, registry, coreActive = false, logWriter = writeGuardLog, featureNotice = featureFailNotice, toolAbortedErrorFactory, mcpCatalogProvider = null, attachmentsProvider = null, routePolicyProvider = null } = {}) {
  ...
  return class PluginApiService extends Service {
    ...
    constructor(ctx, config) {
      ...
      Object.defineProperty(this, 'workspaces', {
        ...
        get() {
          ...
          return this._decoratedNamespaces(['workspaces'], Object.freeze({
            transactions: slot ? this._workspacesForCaller(slot.surface, callerCtx) : this._workspaceTransactionsSurface,
          }))
        },
      })
      ...
      Object.defineProperty(this, 'security', {
        ...
        get() {
          ...
          return this._decoratedNamespaces(['security'], slot ? this._securityForCaller(slot.surface, callerCtx) : this._securitySurface)
        },
      })
    ...
    _storageForCaller(callerCtx) {
      ...
      this._storageCallerViewCache ??= new Map()
      const key = callerIdentityOf(callerCtx)
      const cached = this._storageCallerViewCache.get(key)
      if (cached) return cached
      const surface = slot.surface
      const view = Object.freeze({ ...surface, open: (...args) => surface.open(...args, callerCtx) })
      this._storageCallerViewCache.set(key, view)
      return view
    }

    _workspacesForCaller(surface, callerCtx) {
      this._workspaceTransactionsViewCache ??= new Map()
      const key = callerIdentityOf(callerCtx)
      const cached = this._workspaceTransactionsViewCache.get(key)
      if (cached) return cached
      const view = Object.freeze({ ...surface, prepare: (...args) => surface.prepare(...args, callerCtx) })
      this._workspaceTransactionsViewCache.set(key, view)
      return view
    }

    _securityForCaller(surface, callerCtx) {
      const owner = callerIdentityOf(callerCtx)
      this._securityCallerViewCache ??= new Map()
      const cached = this._securityCallerViewCache.get(owner)
      if (cached) return cached
      ...
      this._securityCallerViewCache.set(owner, view)
      return view
    }
