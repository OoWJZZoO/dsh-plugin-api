 ...
 */
function createContributeEntry(getBase, resolveOwnerId = (spec) => spec?.ownerId ?? 'caller') {
  let sequence = 0
...
export function createPluginApiService({ apiVersion, registry, coreActive = false, logWriter = writeGuardLog, featureNotice = featureFailNotice, toolAbortedErrorFactory, mcpCatalogProvider = null, attachmentsProvider = null, routePolicyProvider = null } = {}) {
  ...
  return class PluginApiService extends Service {
    ...
    _promptsSurface() {
      ...
      const cached = this._promptsSurfaceCache
      if (cached && cached.ownerKey === ownerKey) return cached.surface
      ...
      const surface = composeSurface(base, { provenance, contribute })
      this._promptsSurfaceCache = { ownerKey, surface }
      return surface
    }
