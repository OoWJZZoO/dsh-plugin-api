 ...
 * delegates to the engine's own disposer. A surface that exposes no visibility
 * registration (or a disabled one that refuses on read) is published as it is.
 */
function bindExecutionVisibility(executions, callerCtx) {
  let visibility
  try {
  ...
  return Object.freeze({
    ...
    register: (spec) => {
      if (spec === null || typeof spec !== 'object') {
        throw new TypeError('visibility policy must be an object')
      }
      ...
      return createResourceHandle({
        ...
        ownerId,
        generation: `${ownerId}:visibility:${++generation}`,
        revoke: () => release() === true,
      })
    },
  })
    ...
    constructor(ctx, config) {
      ...
      Object.defineProperty(this, 'executions', {
        ...
        get: () => {
          ...
          // The visibility registration carries the derived caller identity and
          // the standard resource handle, like the rest of the registration
          // plane; the engine keeps the policy record itself.
          const bound = bindExecutionVisibility(executions, callerCtx)
        ...
        const workspaceTransactionStatus = () => {
          try {
            ...
            if (typeof api.availability === 'function') {
              const probe = api.availability()
              const status = probe?.status
              if (status === 'active' || status === 'degraded' || status === 'unavailable') {
                // The domain detail rides along: the facade normalizes the
                // status, it never trims what the owner reported.
                return Object.freeze({ ...probe, status })
              }
              if (typeof probe?.active === 'boolean') {
                return Object.freeze({ ...probe, status: probe.active ? 'active' : 'unavailable' })
              }
            }
          } catch {
