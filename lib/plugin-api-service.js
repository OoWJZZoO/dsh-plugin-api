...
function createToolsApi(resolveTools, discoverySurface, scopedHelpers) {
  ...
  return Object.freeze({
    ...
    // It is passed through verbatim; wrapping it into a resource handle would
    // manufacture a lifecycle the official contract does not have.
...
function createConditionalAttachmentsSurface(active, pipelineProvider) {
  ...
  const invoke = (owner, method, args, { operation = true } = {}) => {
    ...
    try {
      ...
      if (result && typeof result.then === 'function') {
        return operation ? result.catch(() => ATTACHMENT_UNAVAILABLE) : result
      }
      return result
    } catch (error) {
...
function createFeatureSlot({ active, feature, api, methods, isCurrent, callerAware }) {
  const record = { current: true, api: null, surface: null, callerCtx: undefined }
  const assertAvailable = () => {
    if (!active()) throw new PluginApiInactiveError()
  ...
  for (const name of methods) {
    surface[name] = (...args) => {
      assertAvailable()
      // Caller-aware members receive the caller context as their trailing
      // argument, so an owner identity is derived from the caller rather than
      // demanded from the caller. The context is stamped per surface access.
      const forwarded = Array.isArray(callerAware) && callerAware.includes(name)
        ? [...args, record.callerCtx]
        : args
      return Reflect.apply(api[name], api, forwarded)
    }
  }
  record.api = api
  ...
  return class PluginApiService extends Service {
    ...
    constructor(ctx, config) {
      ...
      Object.defineProperty(this, 'llm', {
        ...
        get() {
          ...
          if (this._llmRequestSlot) this._llmRequestSlot.callerCtx = callerCtx
          if (this._llmAdmissionSlot) this._llmAdmissionSlot.callerCtx = callerCtx
      ...
      Object.defineProperty(this, 'executions', {
        ...
        get: () => {
          ...
          if (false) {
            console.error('EXEC GETTER recovery keys:', Object.keys(recovery).join(','))
          }
          return this._decoratedNamespaces(['executions', 'executions.recovery', 'executions.recovery.checkpoints'], composeSurface(executions, {
            recovery,
          }))
        },
      })
      ...
      Object.defineProperty(this, 'storage', {
        ...
        configurable: false,
        get: () => {
          // The storage owner label is derived from the calling plugin, so the
          // caller context is stamped per surface access.
          if (this._storageSlot) this._storageSlot.callerCtx = this?.ctx ?? this
          return this._decoratedNamespaces(['storage'], this._storageSlot ? this._storageSlot.surface : this._storageSurface)
        },
      })
      ...
      Object.defineProperty(this, 'workspaces', {
        ...
        configurable: false,
        get: () => {
          // The transaction owner is derived from the calling plugin, so the
          // caller context is stamped per surface access.
          if (this._workspaceTransactionsSlot) this._workspaceTransactionsSlot.callerCtx = this?.ctx ?? this
          return this._decoratedNamespaces(['workspaces'], Object.freeze({
      ...
      Object.defineProperty(this, 'security', {
        ...
        configurable: false,
        get: () => {
          // The registration owner is derived from the calling plugin, so the
          // caller context is stamped per surface access.
          if (this._securitySlot) this._securitySlot.callerCtx = this?.ctx ?? this
          return this._decoratedNamespaces(['security'], this._securitySlot ? this._securitySlot.surface : this._securitySurface)
        },
      })
      ...
      Object.defineProperty(this, 'tools', {
        ...
        get() {
          ...
          if (this._toolsDiscoverySlot) this._toolsDiscoverySlot.callerCtx = this?.ctx ?? this
          let surface
      ...
      Object.defineProperties(this, {
        ...
        diagnostics: { enumerable: true, configurable: false, get() {
          // Stamp the caller context on the slot before handing the surface
          // out, so a registration made through it derives the caller's owner
          // identity instead of asking the caller to declare one.
          if (this._diagnosticsSlot) this._diagnosticsSlot.callerCtx = this?.ctx ?? this
          return this._decoratedNamespaces(['diagnostics'], this._diagnosticsSurface)
        } },
    ...
    _installLlmSlot(name, api) {
      ...
      const record = createFeatureSlot({
        ...
        // The registry derives the registration owner from the caller, so the
        // caller binding is forwarded as the trailing argument.
        callerAware: methods,
      })
      this[field] = record
      return record
    ...
    _installScopedPromptContribution(spec, callerCtx, ownerKey) {
      ...
      if (typeof spec.kind !== 'string' || !(spec.kind in SCOPED_PROMPT_KINDS)) {
        return Object.freeze({ ok: false, code: 'unsupported', reason: `prompts.contribute scoped kind "${spec.kind}" is unsupported` })
      }
    ...
    _assignFeature(name, api) {
      ...
      if (name === 'workspaceTransactions') {
        ...
        const surface = Object.freeze({
          ...
          // receives the stamped caller context as its trailing argument.
          prepare: (...args) => {
            fail()
            return Reflect.apply(api.prepare, api, [...args, record.callerCtx])
          },
      ...
      if (name === 'security') {
        ...
        const wrapRegistration = (fn) => (...args) => {
          fail()
          return Reflect.apply(fn, api, [callerIdentityOf(record.callerCtx), ...args])
        }
        const surface = Object.freeze({
      ...
      if (name === 'toolDiscovery') {
        ...
        // The catalog registration derives its owner from the caller, so the
        // caller context stamped by the namespace access is forwarded as the
        // trailing argument; the surrounding read and lifecycle members keep
        // their exact argument lists.
        const invokeNested = (parent, method, { callerBound = false } = {}) => (...args) => {
          fail()
          const forwarded = callerBound ? [...args, record.callerCtx] : args
          return Reflect.apply(api[parent][method], api[parent], forwarded)
        }
        const surface = Object.freeze({
          catalog: Object.freeze({ register: invokeNested('catalog', 'register', { callerBound: true }) }),
          list: (...args) => {
            fail()
      ...
      if (name === 'storage') {
        ...
        const surface = Object.freeze({
          ...
          open: (...args) => {
            fail()
            return Reflect.apply(api.open, api, [...args, record.callerCtx])
          },
          availability: invoke('availability'),
        })
        record.surface = surface
