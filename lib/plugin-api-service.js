...
function createToolsApi(resolveTools, routeOf, toolAbortedErrorFactory) {
  ...
  return Object.freeze({
    ...
    // (requirements 1.4) — it only invokes the stable injected factory.
    toolAbortedError() {
 ...
 */
export function createPluginApiService({ apiVersion, registry, coreActive = false, logWriter = writeGuardLog, featureNotice = featureFailNotice, toolAbortedErrorFactory } = {}) {
  ...
  return class PluginApiService extends Service {
    ...
    constructor(ctx, config) {
      ...
      // degraded so construction can never throw (requirements 3.2/3.3).
