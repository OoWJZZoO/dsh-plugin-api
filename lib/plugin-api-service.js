...
/**
 * Present the route-policy health evidence ingest through the facade's
 * mutation idiom.
 *
 * The owner verb is `health.observe`, but this facade reserves `observe` for
 * projection subscriptions: every other `observe` answers the four-member
 * handle, so reusing the name here would hand callers a bare evidence record
 * instead. The public member is `health.report`, and it answers the mutation
 * idiom's frozen discriminated result carrying the written evidence as
 * `entry`.
 */
const healthReportMember = (ingest) => (...args) => {
  const entry = ingest(...args)
  return Object.freeze({ ok: true, code: 'committed', commitState: 'committed', entry })
}
...
function createConditionalRoutePolicySurface(active, provider) {
  ...
  const health = Object.freeze({
    // Internal forwarding face: the raw owner answer. The public surface
    // presents it as the mutation result (see `healthReportMember`).
    report: invoke('health', 'observe'),
  ...
  const api = {
    ...
    durable: Object.freeze({
      ...
      observe(subject) {
        ...
        // standard projection handle, and listeners attach through it. A facade
        // that does not provide the forwarding target answers typed disabled —
        // never a bare TypeError through the public observation entry.
        if (typeof facade.observeDurable !== 'function') {
          throw new PluginApiFeatureDisabledError('sessions.durable', 'the durable facade does not provide the observation entry')
        }
  ...
  return class PluginApiService extends Service {
    ...
    _publishRoutingSurface() {
      ...
      this._routingSurface = Object.freeze({
        ...
        health: Object.freeze({
          report: healthReportMember(invokeRoutePolicy('health', 'report')),
