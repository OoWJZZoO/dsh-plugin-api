...
function createContributeEntry(getBase, resolveOwnerId = (spec) => spec?.ownerId ?? 'caller') {
  ...
  return (spec) => {
    ...
    if (!route || typeof base?.[route.member] !== 'function') {
      ...
      // (contribution never throws through the caller).
      const backingActive = base === null || base === undefined || base.isActive !== false
      return Object.freeze({
        ok: false,
        code: backingActive ? 'unsupported' : 'unavailable',
        reason: backingActive
