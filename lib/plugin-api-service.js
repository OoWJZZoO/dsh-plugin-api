...
function createDisabledToolsDiscoveryApi(active) {
  ...
  return Object.freeze({
    ...
    activate: fail,
    deactivate: fail,
    audit: Object.freeze({ query: fail, list: fail }),
    get availability() {
      fail()
    },
  })
...
function createToolsApi(resolveTools, toolAbortedErrorFactory, discoverySurface) {
  const tools = () => {
    let service
    try {
...
function createDisabledAgentApi(active) {
  ...
  const unavailable = {
    ...
    enter: failAgentExtension('providers.enter'),
    announce: failAgentExtension('providers.announce'),
    setFactory: failAgentExtension('providers.setFactory'),
    providersRegister: failAgentExtension('providers.register'),
  }
  return Object.freeze({
    isActive: false,
    get: fail,
    ...
    providers: Object.freeze({
      ...
      register: unavailable.providersRegister,
    }),
...
function createDisabledDiagnosticsApi(active) {
  ...
  return Object.freeze({ register: fail, get: fail, onChange: fail, observe: fail })
}
...
const SKILLS_ACTIVATION_SURFACE_METHODS = ['registerDescriptor', 'registerSkill', 'activate', 'deactivate']
...
function createDisabledSkillsActivationApi(active) {
  ...
  surface.policy = Object.freeze({ registerMinimalCatalogUpdate: fail, register: fail })
...
function createDisabledSessionChannelApi(active) {
  ...
  return Object.freeze({
    ...
    redaction: Object.freeze({ registerProfile: fail, register: fail }),
...
function createDisabledRoutePolicyApi(active) {
  ...
  return Object.freeze({
    ...
    circuit: Object.freeze({ status: fail, inspect: fail }),
    observe: fail,
...
function createConditionalLlmAdaptersSurface(active, provider, registryGate) {
  ...
  return Object.freeze({
    ...
    register(definition, callerCtx) {
      return this.decorate(definition, callerCtx)
    },
    snapshot() {
      const facet = gates()
 ...
 */
function createContributeEntry(getBase) {
  let sequence = 0
  ...
  return (spec) => {
    ...
    if (!route || typeof base?.[route.member] !== 'function') {
      return Object.freeze({ ok: false, code: 'unsupported', reason: `prompts.contribute kind "${spec.kind}" is unsupported` })
    }
    const id = spec.id ?? spec.name ?? spec.section?.id ?? '<anonymous>'
    const ownerId = spec.ownerId ?? 'caller'
  ...
  return class PluginApiService extends Service {
    ...
    constructor(ctx, config) {
      ...
      Object.defineProperty(this, 'llm', {
        ...
        get() {
          ...
          return this._decoratedNamespaces(['llm', 'llm.routing'], composeSurface(base, {
            ...
            // Provider/model directory registrations ride the official
            // directory handles (official binding preserves official identity).
            providers: Object.freeze({
              register: (...args) => Reflect.apply(base.registerConfigurableProviders, base, args),
            }),
            models: Object.freeze({
              register: (...args) => Reflect.apply(base.registerModelDiscovery, base, args),
            }),
          }))
        },
      })
      ...
      Object.defineProperty(this, 'sessions', {
        enumerable: true,
        configurable: false,
        get: () => this._decoratedNamespaces(['sessions', 'sessions.branches', 'sessions.channels'], this._sessionSurface),
      })
      // skills activation facade
      ...
      this._promptsSurfaceCache = null
      ...
      const resolveFeatureStatus = (feature) => {
        ...
        return this._registry?.isActive?.(feature) === true
      }
      ...
      Object.defineProperties(this, {
        prompts: { enumerable: true, configurable: false, get: () => this._decoratedNamespaces(['prompts', 'prompts.provenance'], this._promptsSurface()) },
    ...
    _publishSettingsSurface() {
      ...
      this._settingsSurface = composeSurface(this._settingsBaseSurface, {
        ...
        contribute: settingsRemote.contribute ?? settingsRemote.remote,
        inspect: (...args) => {
          const base = this._settingsBaseSurface
          if (!base || typeof base.describe !== 'function') {
            throw new PluginApiFeatureDisabledError('settings', 'settings owner is unavailable')
          }
          const view = Reflect.apply(base.describe, base, args)
          return Array.isArray(view) ? Object.freeze(view) : Object.freeze(view ?? {})
        },
      })
    }
    ...
    _promptsSurface() {
      ...
      // passthrough leaves when mounted. The composed view is rebuilt when
      // any composition input changes, so the published object identity is
      // stable between remounts.
      if (this._promptsSurfaceCache) return this._promptsSurfaceCache
      const base = this._systemPromptSurface
      const provenance = this._contextSlot ? this._contextSlot.surface : this._contextSurface
      this._promptsSurfaceCache = composeSurface(base, {
        provenance,
        contribute: createContributeEntry(() => this._systemPromptSurface),
      })
      return this._promptsSurfaceCache
    }
    ...
    _publishRoutingSurface() {
      ...
      const routePolicy = () => {
        ...
        if (!flat || typeof flat.policies?.register !== 'function') {
          throw new PluginApiFeatureDisabledError('routePolicy', 'route policy replacement is not active')
        }
        return flat
      }
      const invokeRoutePolicy = (path, method) => (...args) => {
        ...
        if (!target || typeof target[method] !== 'function') {
          throw new PluginApiFeatureDisabledError('routePolicy', `route policy member "${path}.${method}" is unavailable`)
        }
        return Reflect.apply(target[method], target, args)
      }
      this._routingSurface = Object.freeze({
        ...
        health: Object.freeze({
          ...
          registerCircuitPolicy: invokeRoutePolicy('health', 'registerCircuitPolicy'),
          circuitPolicy: Object.freeze({ register: invokeRoutePolicy('health', 'registerCircuitPolicy') }),
          registerProbe: invokeRoutePolicy('health', 'registerProbe'),
          probe: Object.freeze({ register: invokeRoutePolicy('health', 'registerProbe') }),
          startProbe: invokeRoutePolicy('health', 'startProbe'),
          completeProbe: invokeRoutePolicy('health', 'completeProbe'),
          get: invokeRoutePolicy('health', 'get'),
          history: invokeRoutePolicy('health', 'history'),
        }),
        circuit: Object.freeze({
          status: invokeRoutePolicy('circuit', 'status'),
    ...
    _assignFeature(name, api) {
      ...
      if (name === 'diagnostics') {
        ...
        this._diagnosticsSlot = createFeatureSlot({
          ...
          feature: 'diagnostics',
          api: diagnosticsApi,
          methods: ['register', 'get', 'onChange', 'observe'],
      ...
      if (name === 'recovery') {
        ...
        const surface = Object.freeze({
          ...
          capability: Object.freeze({
            declare: invokeNested('capability', 'declare'),
            register: invokeNested('capability', 'declare'),
          }),
      ...
      if (name === 'security') {
        ...
        const surface = Object.freeze({
          ...
          audit: Object.freeze({
            query: wrap(api.audit.query),
            list: wrap(api.audit.query),
          }),
          get availability() {
            fail()
            return api.availability
      ...
      if (name === 'toolDiscovery') {
        ...
        const surface = Object.freeze({
          ...
          audit: Object.freeze({
            query: invokeNested('audit', 'query'),
            list: invokeNested('audit', 'list'),
          }),
          get availability() {
            fail()
            return api.availability()
      ...
      if (name === 'skillsActivation') {
        ...
        surface.register = (spec) => {
          ...
          if (spec && typeof spec === 'object' && 'skill' in spec && typeof api.registerSkill === 'function') {
      ...
      if (name === 'storage') {
        ...
        const surface = Object.freeze({
          get isActive() { fail(); return api.isActive },
          open: invoke('open'),
          availability: invoke('availability'),
        })
        record.surface = surface
