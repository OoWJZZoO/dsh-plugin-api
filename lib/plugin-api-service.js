...
function createConditionalAttachmentsSurface(active, pipelineProvider) {
  ...
  const invoke = (owner, method, args, { operation = true } = {}) => {
    ...
    try {
      ...
      // A registration failure is the caller's own input or identity problem:
      // it must reach the caller as the typed error the registration raised,
      // not be folded into an availability verdict about the pipeline. Only
      // operation-class calls degrade to the unavailable result.
