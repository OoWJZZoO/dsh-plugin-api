import { PluginApiError, PluginApiInactiveError, PluginApiFeatureDisabledError } from './errors.js'
...
export function createScopedAgentContributionRegistry({
  ...
  return Object.freeze({
    ...
    registerScope(spec, callerCtx) {
      gates()
      // Resource-registry registration: every failure is a typed throw, never a
      // discriminated result (one rule source per idiom).
      if (!isPlainObject(spec)) {
        throw new PluginApiError('PLUGIN_API_SCOPE_SPEC_INVALID', 'agents.scopes.register requires a spec object')
      }
      const targetId = targetRefOf(spec.agent)
      if (targetId === null) {
        throw new PluginApiError('PLUGIN_API_SCOPE_SPEC_INVALID', 'spec.agent must be a session id string or an agent reference with an id')
      }
      const ownerId = resolveOwnerId?.(callerCtx)
      if (!isNonEmptyString(ownerId)) {
        throw new PluginApiError('PLUGIN_API_SCOPE_OWNER_UNAVAILABLE', 'scope owner is unavailable')
      }
      const agents = officialAgents()
      if (!agents) {
        throw new PluginApiError('PLUGIN_API_SCOPE_BACKING_UNAVAILABLE', 'the official agents backing is unavailable')
      }
      let live = false
      try {
        live = agents.get(targetId) !== undefined
      } catch (error) {
        throw new PluginApiError('PLUGIN_API_SCOPE_TARGET_UNRESOLVED', `target resolution failed: ${String(error?.message ?? error)}`)
      }
      if (!live) {
        throw new PluginApiError('PLUGIN_API_SCOPE_TARGET_UNRESOLVED', `target agent "${targetId}" cannot be resolved (nonexistent or disposed)`)
      }
      ...
      const handle = Object.freeze({
        ...
        dispose() {
          if (!disposeHandleRecord()) {
            return staleResult('the scope handle is already released')
          }
