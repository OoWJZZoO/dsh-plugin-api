 ...
 * identity, `agent/disposed` marks records dormant (the official scoped
 * registrations die with the old agent fiber regardless). Every internal
...
const SCOPED_PROMPT_KINDS = Object.freeze({
...
export function createScopedAgentContributionRegistry({
  ...
  const purgeOwner = (ownerId) => {
    for (const [targetId, byTarget] of [...records]) {
      for (const record of [...byTarget.values()]) {
        if (record.ownerId === ownerId) removeRecord(record)
      }
  ...
  return Object.freeze({
    ...
    registerScope(spec, callerCtx) {
      ...
      const handle = Object.freeze({
        ...
        dispose() {
          if (handleRecord.disposed) return Object.freeze({ status: 'stale', reason: 'scope handle already disposed' })
          handleRecord.disposed = true
          // Identity-bound: only the contributions installed through this
          // handle are removed; other owners and other handles survive.
          const byTarget = records.get(targetId)
          if (byTarget) {
            for (const record of [...byTarget.values()]) {
    ...
    inspection() {
      let count = 0
      for (const byTarget of records.values()) count += byTarget.size
      return Object.freeze({
        scopes: scopes.size,
        records: count,
