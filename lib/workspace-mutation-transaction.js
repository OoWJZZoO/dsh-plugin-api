 ...
 * (`pluginApi.workspaces.transactions`).
 *
 ...
 * fencing) and the read-only `pluginApi.executions.recovery` vocabulary, plus optional
...
export function createWorkspaceMutationTransaction({ ctx, coordination, recovery, logger, now, idFactory } = {}) {
  ...
  const invalid = (reasonValue, { transactionId, action, observed } = {}) => deepFreeze({
    ok: false,
    code: 'invalid-input',
    reason: reasonValue,
  ...
  const failed = (code, { transactionId, action, reason, observed, recoveryClassification } = {}) => deepFreeze({
    ok: false,
    code,
    ...(transactionId ? { transactionId } : {}),
    action,
  ...
  const succeeded = (code, payload = {}) => deepFreeze({
    ok: true,
    code,
    ...(COMMIT_STATES[code] ? { commitState: COMMIT_STATES[code] } : {}),
    ...payload,
    observedAt: observedAt(),
  })
  ...
  const classifyFailure = (error) => {
    if (!isObject(recovery) || typeof recovery.evaluate !== 'function') return undefined
    try {
      const result = classifyRecoveryFailure(error)
  ...
  // Facade-owned recovery authority consumption (recovery/transaction path).
  // An interrupted commit consults the shared private authority once per
  // transaction window before continuing through recovery. The domain's
  // documented outcome (recovering state, continued only through the recovery
  // operation) stays authoritative for every converged action: an interrupted
  // commit has no safe automatic retry or rollback besides the recovery path,
  // and recovery never becomes a hidden mutation or generic rollback disposer.
  const consultTransactionRecovery = async ({ transactionId, revision, cause }) => {
    if (!isObject(recovery) || !isObject(recovery._internal)
      || typeof recovery._internal.decide !== 'function'
      || typeof recovery._internal.commit !== 'function') return null
    const generation = boundedString(String(revision), 120)
    const executionId = boundedString(`transaction:${transactionId}`, 120)
    let result
    try {
      result = await recovery._internal.decide({
        ownerId: 'facade-transaction-owner',
        generation,
        path: 'recovery/transaction',
        scope: 'workspace',
        decisionWindowId: boundedString(`transaction:${transactionId}:${revision}`, 120),
        execution: { executionId, active: true, cancellable: false },
        failure: { code: 'commit-interrupted', message: boundedString(cause, 160) },
        attemptsRemaining: 0,
      })
    } catch {
      return null
    ...
    try {
      await recovery._internal.commit(decision, {
        operation: { ownerId: 'facade-transaction-owner', generation, executionId },
      })
    } catch {
      // a duplicate or stale window keeps its first consumption; the
      // recovering state is unchanged either way
    }
    return {
  ...
  async function prepare(input = {}, callerCtx) {
    if (disposed) return inactive('prepare')
    const transactionId = normalizeTransactionId(input.transactionId)
    if (!transactionId.ok) return invalid(transactionId.reason, { action: 'prepare' })
    const workspace = normalizeWorkspace(input.workspace)
    if (!workspace.ok) return invalid(workspace.reason, { transactionId: transactionId.value, action: 'prepare' })
    // The transaction owner is the calling plugin's identity, derived from the
    // caller context; a caller-supplied ownerId is ignored rather than trusted.
    const ownerId = normalizeOwnerId(callerIdentityOf(callerCtx))
    if (!ownerId.ok) return invalid(ownerId.reason, { transactionId: transactionId.value, action: 'prepare' })
    const intent = normalizeIntent(input.intent)
    if (!intent.ok) return invalid(intent.reason, { transactionId: transactionId.value, action: 'prepare' })
    const resources = normalizeResourceSet(input.resources)
    if (!resources.ok) return invalid(resources.reason, { transactionId: transactionId.value, action: 'prepare' })
    const lease = normalizeFencingEvidence(input.lease)
    if (!lease.ok) return invalid(lease.reason, { transactionId: transactionId.value, action: 'prepare' })
    const provenance = normalizeProvenance(input.provenance)
    if (!provenance.ok) return invalid(provenance.reason, { transactionId: transactionId.value, action: 'prepare' })
  ...
  async function record(transactionId, input = {}) {
    ...
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'record' })
    const mutation = normalizeMutationRecord(input)
    if (!mutation.ok) return invalid(mutation.reason, { transactionId: normalizedId.value, action: 'record' })
  ...
  async function preview(transactionId, options = {}) {
    ...
    const normalizedId = normalizeTransactionId(transactionId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'preview' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { transactionId: normalizedId.value, action: 'preview', reason: found.reason })
    }
    const recordValue = found.record
  ...
  async function commit(transactionId, options = {}) {
    ...
    const normalizedId = normalizeTransactionId(transactionId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'commit' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { transactionId: normalizedId.value, action: 'commit', reason: found.reason })
    }
    const recordValue = found.record
  ...
  async function rollback(transactionId, options = {}) {
    ...
    const normalizedId = normalizeTransactionId(transactionId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'rollback' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { transactionId: normalizedId.value, action: 'rollback', reason: found.reason })
    }
    const recordValue = found.record
    ...
    if (recordValue.state === 'committing') {
      ...
      const recoveryDecision = await consultTransactionRecovery({
        transactionId: normalizedId.value,
        revision: recordValue.revision,
        cause: 'interrupted commit discovered during rollback',
      })
  ...
  async function recover(transactionId, options = {}) {
    ...
    const normalizedId = normalizeTransactionId(transactionId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'recover' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { transactionId: normalizedId.value, action: 'recover', reason: found.reason })
    }
    const recordValue = found.record
  ...
  async function observe(subject, options = {}) {
    ...
    // The canonical subject is an options object; a bare transaction id stays
    // accepted as the convenience form (the trailing options bag is only read
    // for that legacy shape).
    const isSpec = subject !== null && typeof subject === 'object' && !Array.isArray(subject)
    const spec = isSpec ? subject : {}
    const rawId = isSpec ? spec.transactionId : subject
    const normalizedId = normalizeTransactionId(rawId)
    ...
    const signal = spec.signal !== undefined ? spec.signal : options.signal
    ...
    const currentView = async (live) => {
      if (!live || subscription.disposed) {
        return deepFreeze({ code: 'inactive', transactionId: normalizedId.value, epoch: subscription.epoch, observedAt: observedAt() })
      }
    ...
    const created = createObserverHandle({
      epoch: subscription.epoch,
      current: (live) => currentView(live),
      onDispose: () => {
        subscription.disposed = true
        const listeners = observers.get(normalizedId.value)
        if (listeners) {
    ...
    const onEvent = (event) => {
      ...
      created.publish(deepFreeze({ ...event, epoch: subscription.epoch }))
    }
    ...
    if (subscription.signal) {
      subscription.abortHandler = () => created.handle.dispose()
      try {
        subscription.signal.addEventListener('abort', subscription.abortHandler, { once: true })
      } catch {
    ...
    return Object.freeze({
      ...
      initialState: initialRecord
        ...
        : deepFreeze({ transactionId: normalizedId.value, state: 'unknown' }),
    })
  }
