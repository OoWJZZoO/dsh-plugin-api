 ...
 */
function buildDisabledServicesNamespace(definitions, active) {
  const namespace = {}
...
export function createDisabledServicesNamespace(active) {
  return buildDisabledServicesNamespace(SERVICE_DEFINITIONS, active)
}
 ...
 */
export function createServicesNamespace({
  ctx,
  active = true,
  logger,
  uriHelpers,
  // Internal test/integration seam. Runtime callers always use the static
  // table above; a supplied table exercises the same facade construction.
  definitions = SERVICE_DEFINITIONS,
} = {}) {
  const isActive = normalizeActive(active)
  const definitionTable = Array.isArray(definitions) ? definitions : SERVICE_DEFINITIONS

  if (typeof ctx?.get !== 'function') {
    return buildDisabledServicesNamespace(definitionTable, isActive)
  }

  const namespace = {}
  let availableCount = 0

  for (const def of definitionTable) {
