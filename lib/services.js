 ...
 */
export function buildActiveFacade(def, service, uriHelpers = {}) {
  const facade = { isActive: true }

  for (const member of def.members) {
    if (member.kind === 'method') {
      if (!isCallable(service?.[member.name])) {
        if (member.optional) continue
        // Non-optional method missing from the live official service: honest
        // capability absence, the member is omitted rather than faked.
        continue
      }
      facade[member.name] = (...args) => service[member.name](...args)
    } else if (member.kind === 'getter') {
      if (!memberAvailable(service, member)) {
        if (member.optional) continue
        continue
      }
      ...
      if (!isCallable(uriHelpers?.[member.name])) {
        if (member.optional) continue
        continue
      }
...
/**
 * Build the runtime `pluginApi.services` namespace. Each of the 17 official
 * services is probed independently: an available service gets an active frozen
 * passthrough facade, a missing service gets a per-service disabled facade
 * (`services.<key>`), and the namespace itself is frozen.
 */
export function createServicesNamespace({ ctx, active = true, logger, uriHelpers } = {}) {
  const isActive = normalizeActive(active)

  if (typeof ctx?.get !== 'function') {
    return createDisabledServicesNamespace(isActive)
  }
  ...
  for (const def of SERVICE_DEFINITIONS) {
    ...
    if (service !== undefined) {
      availableCount += 1
      namespace[def.key] = buildActiveFacade(def, service, uriHelpers)
    } else {
