...
const SERVICES_REGISTRY_METADATA = Object.freeze({
  ...
  appExit: Object.freeze({ composition: 'pure', authority: 'official launcher process lifecycle', availability: 'official appExit service present -> active; absent -> typed unavailable', visibility: 'official low-level service' }),
...
/**
 * Resolve one declared member on the live service.
 *
 * A member is either single-segment (`name`) or, when the official surface
 * nests the seam one level deeper, addressed by an explicit `path` (e.g.
 * `['sessions','models']`). The path form exists so a narrow deep seam can be
 * whitelisted without exposing the whole intermediate object to plugins.
 */
function memberTargetOf(service, member) {
  const path = Array.isArray(member.path) ? member.path : [member.name]
  let target = service
...
function memberOwnerOf(service, member) {
  const path = Array.isArray(member.path) ? member.path.slice(0, -1).map((segment) => segment) : []
  let owner = service
  for (const segment of path) {
...
function memberAvailableFor(def, member, service, uriHelpers) {
  if (member.kind === 'method') return isCallable(memberTargetOf(service, member))
...
export function buildActiveFacade(def, service, uriHelpers = {}, active = true, logger = undefined) {
  ...
  for (const member of def.members) {
    if (member.kind === 'method') {
      if (member.optional && !isCallable(memberTargetOf(service, member))) continue
      facade[member.name] = (...args) => {
        if (!normalizeActive(active)()) throw new PluginApiInactiveError()
        // A path-addressed member is invoked on the object that actually owns
        // it, so the official receiver is never silently swapped for the
        // outermost service.
        const owner = memberOwnerOf(service, member)
        const target = memberTargetOf(service, member)
      ...
      Object.defineProperty(facade, member.name, {
        ...
        get() {
          const owner = memberOwnerOf(service, member)
          const target = memberTargetOf(service, member)
          return typeof target === 'function' ? target.bind(owner) : target
        },
      })
 ...
 */
export function createServicesNamespace({ ctx, active = true, logger, uriHelpers, migrateLeaves } = {}) {
  const isActive = normalizeActive(active)

  if (typeof ctx?.get !== 'function') {
    if (migrateLeaves && Object.keys(migrateLeaves).length > 0) {
      return Object.freeze(Object.assign({}, buildDisabledServicesNamespace(isActive), migrateLeaves))
    }
  ...
  if (availableCount === 0 && (!migrateLeaves || Object.keys(migrateLeaves).length === 0)) {
    return buildDisabledServicesNamespace(isActive)
  }

  if (migrateLeaves) {
    for (const [key, leaf] of Object.entries(migrateLeaves)) namespace[key] = leaf
  }
