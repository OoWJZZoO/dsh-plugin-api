 ...
 * @param {(input: {sessionId: string, executionId: string}) => {confidence?: string, activityId?: string}} [options.readActivityCorrelation]
 *   Read-only consultation of the shared activity projection (Requirement 6).
 *   It answers with the projection's own evidence grade; an absent reader or a
 *   throwing reader is treated as `unavailable` (the projection is the
 *   answer's owner, never this authority).
...
export function createRequestAuthority(options = {}) {
   ...
   * projection's own evidence grade (Requirement 6). Reader failures degrade
   * to `unavailable` instead of guessing.
   */
  const resolveCorrelation = (sessionId, executionId) => {
    let answer
    try {
      answer = readActivityCorrelation({ sessionId, executionId })
    } catch (error) {
      log(`activity correlation read failed: ${error?.name ?? 'Error'}`)
      answer = { confidence: 'unavailable' }
    }
    return buildActivityCorrelation({
      executionId,
   ...
   * status-changing attempt fact, so the correlation is exposed as soon as the
   * projection evidences it. An evidenced correlation is stable within the
   * facade lifetime (the projection derives one activity identity per evidence
   * key), so it is not re-read; `unknown` answers keep being retried because
   * evidence may still arrive.
   */
  const refreshActivity = (op, { silent = false } = {}) => {
    if (op.activitySettled === true) return false
    const current = op.status.activity
    const next = resolveCorrelation(op.sessionId, op.executionId)
    if (next.activityId === current.activityId && next.confidence === current.confidence) return false
    setStatus(op, { activity: next })
    if (next.activityId !== null) op.activitySettled = true
  ...
  const commitTerminal = (op, outcome, classification, reason, observedAt) => {
    ...
    refreshActivity(op, { silent: true })
  ...
  const request = async (spec, callerCtx) => {
    ...
    const correlation = resolveCorrelation(value.sessionId, executionId)
    const op = {
      id: operationId,
      ownerId: owner,
      ...
      activitySettled: correlation.activityId !== null,
      observers: new Set(),
