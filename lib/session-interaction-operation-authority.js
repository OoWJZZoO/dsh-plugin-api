...
export function createRequestAuthority(options = {}) {
  ...
  const log = (message) => {
    try {
      logger?.warn?.(`dsh-plugin-api session-interaction-operation: ${message}`)
    } catch {
  ...
  /** Shared cancellation-propagation path used by cancel(), dispose() and signal abort. */
  const requestCancel = (op, input) => {
    ...
    if (resolved !== null && resolved.boundary !== null) {
      ...
      try {
        const result = resolved.boundary.cancelAttempt(op.attemptRef, {
          reason: { message: reason ?? `cancelled (${by})`, by },
    ...
    if (op.attempt === null) {
      ...
      // never picked the work up); the authority adjudicates aborted now.
      commitTerminal(op, 'aborted', 'aborted', reason, isoNow(now))
      ...
      if (op.confirmTimer === undefined && typeof cancelConfirmTimeoutMs === 'number' && cancelConfirmTimeoutMs > 0) {
        op.confirmTimer = timer.setTimeout(() => {
          ...
          commitTerminal(op, 'aborted', 'aborted', reason ?? 'cancel confirmation bound', isoNow(now))
  ...
  const request = async (spec, callerCtx) => {
    ...
    if (value.message !== null) {
      ...
      if (append.ok !== true) {
        ...
        return buildUnavailableOutcome('durable content write failed; request not started')
      }
    }
