...
const AGENT_LOOP_OFFICIAL_PACKAGES = ['@deepseek-ai/dsh-llm', '@deepseek-ai/dsh-agent-loop']
...
export function createDecisionParticipationFeature({ ctx, eventsBus, logger, facadeContract, auxiliaryManifests } = {}) {
  ...
  function agentLoopVersionOk() {
    const manifest = auxiliaryManifests?.agentLoop
    if (!isPlainRecord(manifest) || !isNonEmptyString(manifest.version)) return false
    const parsed = parseFacadeVersion(manifest.version)
    if (!parsed || parsed.runtime !== facadeContract?.runtime || manifest.api !== facadeContract?.api) return false
    // The locked official runtime identity: every official package the
    // replacement row depends on must carry the facade's own runtime identity.
    try {
      return AGENT_LOOP_OFFICIAL_PACKAGES.every((name) => readOfficialPackageVersion(name) === facadeContract?.runtime)
    } catch {
      return false
    }
  }

  function readOfficialPackageVersion(packageName) {
    try {
  ...
  function turnStoppingAvailability() {
    ...
    if (!agentLoopVersionOk()) return 'the agent-loop replacement identity does not match the facade contract'
    let service
    try {
