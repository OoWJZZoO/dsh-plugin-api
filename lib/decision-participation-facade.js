...
function buildDomainMember({ callerCtx, points, resolveAvailability, defaultPoint }) {
  return Object.freeze({
    ...
    admitted() {
      return Object.freeze(Object.keys(points))
    },
    availability() {
      const unavailablePoint = Object.keys(points).find((point) => resolveAvailability(point) !== true)
      return Object.freeze({
        status: unavailablePoint === undefined ? 'active' : 'degraded',
        ...(unavailablePoint === undefined
          ? {}
          : { reason: `the decision point "${unavailablePoint}" is unavailable` }),
      })
    },
  })
}
...
function buildEventsMember({ callerCtx, registries, resolvePointAvailability, activeCatalogHas }) {
  return Object.freeze({
    ...
    availability() {
      const unavailablePoint = UNIFIED_ADMITTED_POINTS.find((name) => resolvePointAvailability(name) !== true)
      return Object.freeze({
        status: unavailablePoint === undefined ? 'active' : 'degraded',
        ...(unavailablePoint === undefined
          ? {}
