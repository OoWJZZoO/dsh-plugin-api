 ...
 * id. A missing or disabled projection, a non-`active` availability probe and
 * a malformed answer all degrade to `unavailable`; a healthy projection that
 * has not evidenced the execution stays `unknown` and is retried on the next
 * read (Requirement 6). The authority never derives an activity identity from
 * event sequences of its own.
 */
function defaultActivityCorrelation(facade) {
  return ({ sessionId, executionId }) => {
    try {
      ...
      // The open record is the common case; history covers an activity that
      // already terminated or was replaced by a newer one.
...
/**
 * Read the correlation one projection snapshot evidences for an execution.
 * Returns `null` when the snapshot is not the matching evidenced record, so
 * the caller keeps looking instead of returning a guessed identity.
 */
