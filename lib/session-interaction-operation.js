...
export function createSessionInteractionOperation(options = {}) {
  ...
  const authority = createRequestAuthority({
    ...
    durableAppend: options.durableAppend ?? defaultDurableAppend(
      facade,
      (sessionId) => service('sessions')?.get?.(sessionId),
      options.readAttachment ?? defaultAttachmentReader(ctx),
    ),
    durableAvailable: options.durableAvailable ?? (options.durableAppend !== undefined ? (() => true) : defaultDurableAvailable(facade)),
    resolveLiveAgent: options.resolveLiveAgent ?? defaultLiveAgentResolver(ctx),
    readAttachment: options.readAttachment ?? defaultAttachmentReader(ctx),
...
function defaultDurableAppend(facade, sessionLookup, readImageOf) {
  return async (sessionId, kind, payload, opts) => {
    try {
      ...
      const declared = Array.isArray(payload?.attachmentRefs) ? payload.attachmentRefs : []
      const attachments = await resolveAttachmentRefs(declared, readImageOf?.(), opts?.signal)
      if (attachments.ok !== true) return { ok: false, reason: attachments.reason }
      const surface = toSurfaceMessage(kind, payload, attachments.refs)
...
/**
 * Default attachment reader: the official attachments authority's own verified
 * read (`readImage`), which re-verifies the reference against stored bytes and
 * answers with the canonical reference. Anything less than that member leaves
 * attachment-bearing requests typed unavailable instead of guessing.
 */
function defaultAttachmentReader(ctx) {
  return () => {
    try {
      const attachments = ctx?.get?.('attachments')
      const readImage = attachments?.readImage
      return typeof readImage === 'function' ? readImage.bind(attachments) : undefined
    } catch {
      return undefined
    }
  }
...
/**
 * Default live-agent resolver for `delivery` (steer/queue): the facade's own
 * `agents` domain hands out the live agent reference, exactly as the parent
 * verification path does. A missing domain or a missing agent refuses the
 * delivery instead of falling back to an append-and-guess path.
 */
function defaultLiveAgentResolver(ctx) {
  return (sessionId) => {
    try {
 ...
 * from the same evidence the projection publishes, and the operation's
 * waiting status follows it. A missing or disabled projection, a
