/**
 * Restore planning — `planRestore(checkpointId)`.
 *
...
export function createRestorePlanner({ store, facts, sources = {}, now, logger } = {}) {
  ...
  const confirmAnchorOf = async (source, kind, anchor) => {
    if (!source || typeof source.confirmAnchor !== 'function') {
      return { ok: false, code: 'unconfirmable', reason: 'owning authority cannot confirm the anchor' }
    }
    try {
      return await source.confirmAnchor(anchor)
    } catch (error) {
      return { ok: false, code: 'unavailable', reason: `anchor confirmation failed: ${error instanceof Error ? error.message : String(error)}` }
    }
  }

  const confirmScopeOf = async (source, kind, resourceId) => {
    if (!source || typeof source.confirmScopeResource !== 'function') {
      return { unknown: true, reason: 'owning authority cannot confirm the scope resource' }
    }
    try {
      return await source.confirmScopeResource(resourceId)
    } catch (error) {
  ...
  const api = {
    ...
    planRestore: async (checkpointId) => {
      ...
      // Plan-time anchor validation against the owning authority: a missing or
      // unconfirmable anchor is marked unavailable/stale with the concrete
      // reason (requirements §2 acceptance 5).
      const anchor = data.source?.anchor ?? {}
      const confirmedAnchor = await confirmAnchorOf(sources[kind], kind, anchor)
      const resourceState = await confirmScopeOf(sources[kind], sliceKind, resourceId)

      const reasons = []
      let { status: sliceStatus, reasons: sliceReasons } = computeSliceRestoreability({
        kind: sliceKind,
        captureStatus,
      ...
      reasons.push(...sliceReasons)
      if (confirmedAnchor.ok !== true) {
        if (sliceStatus === 'restoreable' || sliceStatus === 'partial') sliceStatus = 'unavailable'
        reasons.push(`anchor cannot be confirmed: ${confirmedAnchor.reason ?? 'the captured anchor no longer exists'}`)
      }
      const scopeStatus = resourceState.reachable === true ? 'reachable'
        : resourceState.reachable === false ? 'unavailable'
          : 'unknown'
      if (scopeStatus === 'unavailable') {
        if (sliceStatus === 'restoreable' || sliceStatus === 'partial') sliceStatus = 'unavailable'
        reasons.push(resourceState.reason ?? 'scope resource is gone or unreachable')
      }
      ...
      const slice = deepFreeze({
        ...
        steps: [step],
        overall: sliceStatus,
        scopeResource: deepFreeze({ status: scopeStatus, ...(resourceState.reason ? { reason: boundedString(resourceState.reason) } : {}) }),
