...
export function mountSessionChannelFeature({ ctx, service, logger, featureRegistry } = {}) {
  ...
  const api = Object.defineProperties({
    ...
    // `observe(listener)` hands out the standard observation handle over the
    // same projection. `onChange` stays internal wiring for the handle.
    current: (params) => {
...
function subscribeSessionEvents({ ctx, sessionApi, engine, profiles, logger }) {
  let handle
  try {
    // The merged session observe entry returns the standard projection
    // handle; legacy session surfaces expose on directly (a disposer).
    // Either path observes the official session/event stream.
    const subscribe = typeof sessionApi?.observe === 'function'
      ? (name, listener) => sessionApi.observe(name, listener)
      : (name, listener) => sessionApi.on(name, listener)
    handle = subscribe('session/event', (payload) => {
      try {
