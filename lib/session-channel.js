...
/** Mount-scoped epoch: one per channel engine, so a handle from an earlier
 * mount is distinguishable from the live one. */
let channelMountSequence = 0
...
export function mountSessionChannelFeature({ ctx, service, logger, featureRegistry } = {}) {
  ...
  if (
    !sessionApi
    || (typeof sessionApi.observe !== 'function' && typeof sessionApi.on !== 'function')
    || (typeof sessionApi.views?.events !== 'function' && typeof sessionApi.events !== 'function')
    || !eventsApi
    || typeof eventsApi.observe !== 'function'
  ) {
    return null
  }
  ...
  const epoch = `channels-${++channelMountSequence}`
  ...
  const sessionEventDisposer = subscribeSessionEvents({ ctx, sessionApi, engine, profiles, logger })
  ...
  const api = Object.defineProperties({
    ...
    // Projection face (read-only): `current()` answers the one-shot snapshot,
    // `observe()` hands out the standard observation handle over the same
    // projection (listeners attach through the handle). `onChange` stays
    // internal wiring for the handle.
    current: (params) => {
    ...
    observe: () => {
      ...
      const created = createObserverHandle({
        epoch,
        current: (live) => (live ? projection.observe() : degradedProjection()),
        onDispose: () => {
          try {
            unsubscribe()
          } catch {
            // projection teardown never throws back through the release path
          }
        },
        reportError: (error) => reportDiagnostic({ stage: 'projection-listener', error }),
      })
      unsubscribe = projection.onChange(() => {
        created.publish(created.handle.current())
      })
...
/**
 * The read face of a released observation handle: the same projection fields,
 * an explicit unknown connection state, and the reason — never a stale copy of
 * the last live snapshot.
 */
function degradedProjection() {
  return Object.freeze({
    ...
    reason: 'the channel observation is no longer active',
  })
}
 ...
 * @returns {(() => void) | null}
 */
function subscribeSessionEvents({ ctx, sessionApi, engine, profiles, logger }) {
  let handle
  try {
    ...
    // The merged session observe entry answers the standard projection handle
    // and the listener attaches through it; legacy session surfaces expose
    // `on` directly and answer a disposer. Either path observes the shared
    // official session/event feed (no private scope: every projection
    // subscriber shares the one feed).
    if (typeof sessionApi?.observe === 'function') {
      handle = sessionApi.observe({ name: 'session/event' })
      handle?.subscribe?.(listener)
    } else {
      handle = sessionApi.on('session/event', listener)
    }
  } catch (error) {
  ...
  return typeof handle === 'function'
    ...
    : () => {
        try {
          ...
          // disposal must never throw into the feature cleanup path
        }
        return true
      }
}
