 ...
 *
 * Subscription and production are separate rights: observing an event grants
 * no dispatch right. Each catalog entry declares its producer
 * (`entry.producer`, see `./events-catalog.js`); the public dispatch members
 * derive the caller identity from the dispatch receiver and refuse every
 * caller that is not one of the declared producers with a frozen
 * `{ ok: false, code: 'denied', reason }` before anything is dispatched.
 * Entries without a producer declaration are fail-closed. Official plugins
 * dispatching through their own native `ctx.emit` never pass through this
 * bus and stay unaffected.
 */
...
// Participation entries take every priority tier except `monitor`, which stays
// reserved for the read-only observe feed (an observer never gains decision
// power).
export const PARTICIPATION_PRIORITIES = Object.freeze(EVENT_PRIORITIES.filter((priority) => priority !== 'monitor'))
 ...
 *   used both for custom event definitions and for the dispatch producer decision; a caller context without
 *   derivable identity falls back to the root token.
 */
export function createEventsBus({ ctx, catalog, rSlices = [], logger = { warn() {}, error() {} }, resolveOwnerId = () => undefined } = {}) {
  ...
  function createWrappedListener(entry) {
    ...
    return function wrapped(...args) {
      ...
      if (isWaterfall) {
        if (entry.participation) return runParticipationWaterfall.call(this, entry, args, maybeNext)
  ...
  /**
   * Value-aware waterfall path for participation entries (decision
   * participants installed through `registerParticipation`).
   *
   * Unlike the plain listener waterfall (whose success path forwards the
   * listener's return value directly), a participation slot classifies its
   * return value against the point's typed decision vocabulary:
   * - `undefined` → no decision: the chain continues with its current input;
   * - a classified decision → that value becomes the chain result (the guarded
   *   `next` is not called — the cordis "no next vetoes the chain" semantics,
   *   so a reject/deny can never be overwritten downstream); a point that
   *   declares `denialErrorOf` turns its decision into a deliberate typed
   *   error thrown from this slot (the official deny channel for fs intents);
   * - a malformed value → bounded diagnostic + no decision (chain continues);
   * - a throw / rejection → containment + no decision.
   */
  function runParticipationWaterfall(entry, args, next) {
    ...
    const applyValue = (value) => {
      if (value === undefined) return continueIfNeeded()
      let verdict
      try {
        verdict = participation.classify(value)
      } catch {
        verdict = 'malformed'
      }
      if (verdict === 'decision') {
        if (typeof participation.denialErrorOf === 'function') {
          // Deliberate decision path (not a failure): the classified decision
          // is converted into the point's typed denial error and propagates
          // through the official dispatch to the producer.
          throw participation.denialErrorOf(value)
        }
        return value
      }
      if (verdict === 'malformed') {
        contain(new Error(`participation decision at "${entry.name}" was malformed and treated as no decision`))
      }
      return continueIfNeeded()
    }
    ...
    let returned
    try {
      returned = entry.listener.apply(this, callArgs)
    } catch (error) {
      contain(error)
      return continueIfNeeded()
    }
    ...
    return applyValue(returned)
  }

  /**
   * Install one facade entry for a cataloged event and return its
   * identity-bound disposer. Shared by the public `subscribe` path and the
   * internal participation path (`registerParticipation`).
   */
  function installEntry(name, entry) {
    const state = stateOf(name)
  ...
  /**
   * Internal participation installation path (decision-participation
   * substrate). Not a public entry: only the domain decision registries call
   * it, so there is no unconstrained "participate at any event" surface.
   *
   * Validates that the point exists in the currently active catalog with a
   * waterfall dispatch (decision semantics admission is owned by the domain
   * registries' enumerated point sets), rejects `monitor`, and installs a
   * value-aware participation entry (see `runParticipationWaterfall`).
   *
   * @param {string} name
   * @param {{ listener: Function, classify: (value: unknown) => 'undecided' | 'decision' | 'malformed', denialErrorOf?: (value: unknown) => Error, priority?: string, scope?: unknown }} spec
   * @returns {() => boolean} identity-bound disposer
   */
  function registerParticipation(name, spec) {
    const activeMeta = catalogAccessor()[name]
    if (!activeMeta) {
      throw new PluginApiError(
        'PLUGIN_API_PARTICIPATION_UNAVAILABLE',
        `dsh-plugin-api events: participation point "${String(name)}" is not part of the currently active event vocabulary`,
      )
    }
    if (activeMeta.mode !== 'waterfall') {
      throw new PluginApiError(
        'PLUGIN_API_PARTICIPATION_UNSUPPORTED',
        `dsh-plugin-api events: participation point "${String(name)}" is not a waterfall decision dispatch`,
      )
    }
    if (spec === null || typeof spec !== 'object' || typeof spec.listener !== 'function' || typeof spec.classify !== 'function') {
      throw new PluginApiError(
        'PLUGIN_API_PARTICIPATION_INVALID',
        `dsh-plugin-api events: participation at "${String(name)}" requires a listener and a decision classifier`,
      )
    }
    ...
    return installEntry(name, {
      ...
      participation: {
        ...
        ...(typeof spec.denialErrorOf === 'function' ? { denialErrorOf: spec.denialErrorOf } : {}),
      },
    })
  }
   ...
   *
   * The subject is a single argument: `{ name, scope?, priority? }` in the
   * canonical options form, or the bare event name as the convenience form.
   * The answer is the frozen discriminated envelope `{ ok, code, handle }`:
   * `code: 'observed'` for a canonical catalog name, `code: 'untyped'` (plus
   * a reason) for a name outside the catalog — the existing untyped
   * passthrough channel stays open and is expressed truthfully instead of
   * being dressed up as a catalog hit.
   *
   * The handle exposes `current()`, `subscribe(listener)`, `dispose()` and
   * `epoch`. One monitor-priority feed per event and scope backs every
   * handle; each handle fans out to its own listeners with per-listener
   * containment, so a throwing listener never affects its peers.
   *
   * @param {{ name: string, scope?: unknown, priority?: string }|string} subject
   */
  function observe(subject) {
    ...
    const opts = spec
    const typed = catalogEntryOf(name) !== undefined
    const state = stateOf(name)
    ...
    const handle = Object.freeze({
      ...
      subscribe(listener) {
        // A non-function listener subscribes nothing and never throws through
        // the caller, exactly like a released handle.
    ...
    return Object.freeze({
      ok: true,
      code: typed ? 'observed' : 'untyped',
      handle,
      ...(typed
        ? {}
        : { reason: 'the event name is not part of the canonical catalog; the observation rides the untyped passthrough channel' }),
    })
  }

  /**
   * Resolve the dispatch caller's owner identity.
   *
   * `caller` is the receiver of the public dispatch member (`this`), whose
   * `ctx` the platform tracker binds to the consuming plugin context; a
   * receiver without a derivable context falls back to the root token. Both
   * the receiver lookup and the resolver are containment-hardened: dispatch
   * must never throw through the caller because of identity resolution.
   *
   * @param {unknown} caller
   * @returns {string} derived owner identity, or the root token when unresolvable
   */
  function callerOwnerId(caller) {
    let callerCtx
    try {
      callerCtx = caller?.ctx ?? caller
    } catch {
      callerCtx = undefined
    }
    let ownerId
    ...
    return typeof ownerId === 'string' && ownerId.length > 0 ? ownerId : FALLBACK_OWNER
  }

  /**
   * Producer decision for one dispatch.
   *
   * A canonical event belongs to the producer declared on its catalog entry
   * (`entry.producer`): the caller's derived owner identity must be one of the
   * declared owners. A facade-produced event is dispatchable by the facade's
   * own production identity; an official-produced event is dispatched natively
   * by the official runtime and is never produced through the facade bus; an
   * entry without a producer declaration is fail-closed for every caller.
   *
   * @returns {object | null} a frozen typed denial, or `null` when the caller may dispatch
   */
  function producerDenial(name, meta, caller) {
    const denied = (reason) => Object.freeze({ ok: false, code: 'denied', reason })
    try {
      const producer = meta.producer
      if (producer === null || typeof producer !== 'object' || typeof producer.kind !== 'string') {
        return denied(`event ${String(name)} declares no producer and cannot be dispatched through the facade`)
      }
      const owners = producer.owners
      if (!Array.isArray(owners) || owners.length === 0) {
        return denied(
          producer.kind === 'official'
            ? `event ${String(name)} is produced by the official runtime and cannot be dispatched through the facade`
            : `event ${String(name)} declares no dispatch owner and cannot be dispatched through the facade`,
        )
      }
      const ownerId = callerOwnerId(caller)
      if (!owners.includes(ownerId)) {
        return denied(`caller ${ownerId} is not a declared producer of event ${String(name)}`)
      }
      return null
    } catch (error) {
      return denied(`producer check for event ${String(name)} failed: ${String(error?.message ?? error)}`)
    }
  }
   ...
   *
   * Before anything is dispatched the caller passes the producer decision: a
   * caller that is not one of the event's declared producers receives a frozen
   * `{ ok: false, code: 'denied', reason }` and nothing reaches `ctx[mode]`.
   *
   * @param {unknown} caller - the dispatch member's receiver (its `ctx` carries the caller identity)
   * @param {'emit' | 'serial' | 'parallel' | 'bail' | 'waterfall'} mode
   * @param {string} name
   * @param {unknown[]} args
   */
  function dispatch(caller, mode, name, args) {
    const meta = catalogEntryOf(name)
    if (!meta) {
      return Object.freeze({
        ok: false,
    ...
    const denial = producerDenial(name, meta, caller)
    if (denial !== null) return denial
    // Async dispatch modes (serial/parallel) resolve the discriminated
  ...
  function define(spec) {
    ...
    const handle = Object.freeze({
      ...
      dispose() {
        ...
        return staleResult('the publisher was already released or superseded')
      },
    })
    return handle
  }
  ...
  const bus = {
    ...
    emit(name, ...args) {
      return dispatch(this, 'emit', name, args)
    },

    serial(name, ...args) {
      return dispatch(this, 'serial', name, args)
    },

    parallel(name, ...args) {
      return dispatch(this, 'parallel', name, args)
    },

    bail(name, ...args) {
      return dispatch(this, 'bail', name, args)
    },

    waterfall(name, ...args) {
      return dispatch(this, 'waterfall', name, args)
    },
  ...
  // The participation substrate is an internal installation path reachable
  // only by the host decision registries; it never shows in the walked public
  // surface (internalize, same mode as `dispose`).
  Object.defineProperty(bus, 'registerParticipation', { value: registerParticipation, enumerable: false })
