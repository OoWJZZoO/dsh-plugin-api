...
import { deepFreezeExceptSignal, freezeByPolicy } from './deep-freeze.js'
import { PluginApiEventPriorityError, PluginApiFeatureDisabledError } from './errors.js'
 ...
 */
export function createEventsBus({ ctx, catalog, logger = { warn() {}, error() {} }, featureRegistry } = {}) {
  ...
  function createWrappedListener(entry) {
    ...
    return function wrapped(...args) {
      ...
      const freezeArg = meta.freeze === 'except-signal' ? deepFreezeExceptSignal : (arg) => freezeByPolicy(arg, meta.freeze)
      for (const arg of args) freezeArg(arg)
