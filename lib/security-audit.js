 ...
 * gap accounting and fail-closed behavior (gaps stay visible). `channel`
 * distinguishes automatic enforcement from cooperative invocation and is
 * required on every record.
 */
export function normalizeAuditRecord({ auditId, at, kind, ownerIds, policyIds, summary, outcome, generation, channel } = {}) {
  ...
  const channelValue = channel === 'automatic' || channel === 'cooperative' ? channel : 'cooperative'
