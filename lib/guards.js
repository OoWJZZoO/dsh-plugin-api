...
import { normalizeVersion, parseFacadeVersion, satisfiesContract } from './version.js'
...
export function runCoreGuard(ctx, { apiVersion, facadeVersion, runtimeVersion } = {}) {
  ...
  probe('runtime version', 'installed DSH runtime does not match the runtime the facade was built for', () => {
    const parsed = parseFacadeVersion(facadeVersion)
    if (!parsed) return false
    // Both sides are full versions: compare their normalized major.minor.
    return satisfiesContract(normalizeVersion(parsed.runtime), runtimeVersion)
  })
...
export function runFeatureGuard(featureName, ctx, deps = {}) {
  ...
  if (featureName === 'llm/admission') {
    ...
    probe('capability services', 'none of the 19 official capability services is available', () => {
      if (typeof ctx?.get !== 'function') return false
  ...
  } else if (featureName === 'llm/request') { // B-class compat translation: fail-closed
    ...
    probe('agents service', 'execRoute cannot resolve the official agents service', () => {
