...
export function runFeatureGuard(featureName, ctx, deps = {}) {
  ...
  if (featureName === 'llm/admission') {
  ...
  } else if (featureName === 'settingsRemote') { // mandatory B-class host publication
    ...
    probe('settings service', 'settingsRemote requires the official settings read/write service', () => {
      ...
      return settings != null && typeof settings.get === 'function' && typeof settings.mutate === 'function'
    })
  ...
  } else if (featureName === 'typert') { // mandatory C8 official registry seam
