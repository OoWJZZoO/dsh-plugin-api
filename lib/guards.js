...
export function runFeatureGuard(featureName, ctx, deps = {}) {
  ...
  } else if (featureName === 'usage') { // durable ledger + projections: only ctx.on is a hard substrate
    probe('ctx.on', 'usage cannot register provider-confirmed intake listeners', () => typeof ctx?.on === 'function')
