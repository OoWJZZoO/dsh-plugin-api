...
export function runFeatureGuard(featureName, ctx, deps = {}) {
  ...
  if (featureName === 'llm/admission') {
    ...
    // (design rule) rather than environment probes.
...
} else if (featureName === 'execution') { // host observation: failures degrade per source at mount
    probe('ctx.on', 'execution cannot register source observation listeners', () => typeof ctx?.on === 'function')
  ...
  } else if (featureName === 'recovery') { // explicit host policy surface over the shared event substrate
    probe('ctx.on', 'recovery cannot verify the shared host event substrate', () => typeof ctx?.on === 'function')
  } else if (featureName === 'storage') { // thin owner-scoped private binding over the official storage domain facility
    probe('ctx.get', 'storage cannot resolve the optional official storage facility', () => {
      return typeof ctx?.get === 'function'
    })
  } else if (featureName === 'coordination') { // host coordination facade over optional public services
    probe('ctx.get', 'coordination cannot resolve optional official services for capability probing', () => {
      return typeof ctx?.get === 'function'
    })
} else if (featureName === 'workspaceTransactions') { // host transaction facade: fs/tool evidence intake listeners need the shared event substrate
    probe('ctx.on', 'workspaceTransactions cannot register evidence intake listeners', () => typeof ctx?.on === 'function')
  } else if (featureName === 'tasks') { // host task aggregation facade: source bridges need the shared event substrate
    probe('ctx.on', 'tasks cannot register source observation listeners', () => typeof ctx?.on === 'function')
  } else if (featureName === 'profile') { // host profile projection + remote mutation: loader entries are the mandatory read substrate
    probe('ctx.get', 'profile cannot resolve official services for the runtime view', () => {
      return typeof ctx?.get === 'function'
    })
    probe('loader.entries', 'profile runtime view requires the loader entries surface', () => {
      if (typeof ctx?.get !== 'function') return false
      const loader = ctx.get('loader')
      return typeof loader?.entries === 'function' || typeof ctx?.loader?.entries === 'function'
    })
  } else if (featureName === 'security') { // security-policy: ctx.on is the mandatory seam substrate
    probe('ctx.on', 'security cannot register decision-point listeners', () => typeof ctx?.on === 'function')

    // tool-discovery
  } else if (featureName === 'toolDiscovery') { // host exposure over the system-prompt and tools seams
    probe('systemPrompt.tools', 'tool discovery cannot register its exposure provider', () => {
      return typeof ctx?.get === 'function' && typeof ctx.get('systemPrompt')?.tools === 'function'
    })
    probe('systemPrompt.section', 'tool discovery cannot register its hint section', () => {
      return typeof ctx?.get === 'function' && typeof ctx.get('systemPrompt')?.section === 'function'
    })
    probe('tools service', 'tool discovery cannot resolve the official tools service', () => {
      return typeof ctx?.get === 'function' && typeof ctx.get('tools') === 'object'
    })
    // tool-discovery

    // skills activation facade
  } else if (featureName === 'skillsActivation') { // facade entry needs to resolve the replacement service lazily
    probe('ctx.get', 'skills activation cannot resolve official services for marker probing', () => {
      return typeof ctx?.get === 'function'
    })
    // skills activation facade

    // session-branch facade
  } else if (featureName === 'sessionBranch') { // facade entry needs to resolve the sessions service lazily
    probe('ctx.get', 'session branch cannot resolve official services for marker probing', () => {
      return typeof ctx?.get === 'function'
    })
    // session-branch facade

    // context-provenance
  } else if (featureName === 'context') { // composition facade: mandatory seams fail closed;
    // soft sources (attachments/compaction/toolDiscovery/skillsActivation/evidence slice) are
    // probe-free here and degrade per-source at mount time (design failure paths).
    probe('systemPrompt.service', 'context cannot compose without the official systemPrompt service', () => {
      return typeof ctx?.get === 'function' && typeof ctx.get('systemPrompt')?.section === 'function'
        && typeof ctx.get('systemPrompt')?.context === 'function'
        && typeof ctx.get('systemPrompt')?.variable === 'function'
        && typeof ctx.get('systemPrompt')?.tools === 'function'
    ...
    probe('sessions service', 'context session-surface honesty checks cannot resolve the official sessions service', () => {
      return typeof ctx?.get === 'function' && typeof ctx.get('sessions')?.get === 'function'
        && typeof ctx.get('sessions')?.list === 'function'
    })
    // context-provenance

} else if (featureName === 'llmAdapters') { // host-projected decoration surface: soft probes
    probe('ctx.get', 'cannot resolve services', () => typeof ctx?.get === 'function')
    probe('loader.entries', 'loader composition is unavailable', () => typeof ctx?.loader?.entries === 'function')
    probe('llmAdapters.composition', 'the adapter decoration replacement composition is not active', () => {
      try {
        ...
        for (const entry of ctx.loader.entries()) {
          ...
          if ((options.id === 'plugin-api-llm' || options.name === '@deepseek-ai/dsh-plugin-api-llm') && !disabled) activeReplacement += 1
          if ((options.id === 'llm' || options.name === '@deepseek-ai/dsh-llm') && !disabled) officialEnabled = true
        }
        return activeReplacement === 1 && !officialEnabled
      } catch {
        return false
      }
    })

  } else if (featureName === 'sessionChannel') { // B facade channel control/projection: official sessions service is the read substrate
    probe('sessions service', 'sessionChannel cursor/replay requires the official sessions service', () => {
      return typeof ctx?.get === 'function' && typeof ctx.get('sessions')?.get === 'function'
        && typeof ctx.get('sessions')?.list === 'function'
    })

  } else if (featureName === 'sessionActivity') { // host read-only projection: the shared event substrate is the mandatory seam
    probe('ctx.on', 'session activity cannot subscribe to official and shared facts', () => {
      return typeof ctx?.on === 'function'
    })
    probe('ctx.get', 'session activity cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    // The durable joiner and the shared attempt-facts slice are optional
    // inputs: their absence degrades the projection per source at mount time
    // (reconstructed/unknown evidence), it never disables the feature guard.

  } else if (featureName === 'sessionPlanMode') { // host controlled plan-mode switch face over the official plan-mode service and the session fact stream
    probe('ctx.get', 'plan mode control cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('ctx.on', 'plan mode observation requires the session fact stream', () => {
      return typeof ctx?.on === 'function'
    })
    probe('planMode', 'plan mode control requires the official plan-mode service with its get/set contract', () => {
      let service
      try {
        service = ctx?.get?.('planMode')
      } catch {
        return false
      }
      return Boolean(service) && typeof service.get === 'function' && typeof service.set === 'function'
    })

  } else if (featureName === 'sessionPermissionPresets') { // host controlled permission-preset selection face over the official permission-presets service and the session fact stream
    probe('ctx.get', 'permission preset control cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('ctx.on', 'permission preset observation requires the session fact stream', () => {
      return typeof ctx?.on === 'function'
    })
    probe('permissionPresets', 'permission preset control requires the official permission-presets service with its current/set contract', () => {
      let service
      try {
      ...
      return Boolean(service) && typeof service.current === 'function' && typeof service.set === 'function'
    })

  } else if (featureName === 'credentials') { // host controlled credential write face over the official credential provider and its fact stream
    probe('ctx.get', 'credential mutation cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('ctx.on', 'credential revision tracking requires the official fact stream', () => {
      return typeof ctx?.on === 'function'
    })
    probe('credentials', 'credential mutation requires the official credentials service with its set/unset/describe contract', () => {
      let service
      try {
      ...
      return Boolean(service)
        ...
        && typeof service.unset === 'function'
        && typeof service.describe === 'function'
    })

  } else if (featureName === 'sessionCompaction') { // facade entry resolves the compaction replacement lazily through its marker gate
    probe('ctx.get', 'the compaction operation cannot resolve official services for marker probing', () => {
      return typeof ctx?.get === 'function'
    })

  } else if (featureName === 'workflows') { // host operation facade over the official workflow engine seam
    probe('ctx.get', 'workflow start cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('workflowEngine', 'workflow start requires the official engine with its start member', () => {
      let service
      try {
        service = ctx?.get?.('workflowEngine')
      } catch {
        return false
      }
      return Boolean(service) && typeof service === 'object' && typeof service.start === 'function' && typeof service.then !== 'function'
    })
    probe('agents', 'workflow start verifies the parent against the official agent registry', () => {
      let agents
      try {
      ...
      return Boolean(agents) && typeof agents.get === 'function'
    })

  } else if (featureName === 'sessionInteractions') { // pending-interaction facade over the durable approval pair and the official answerer waterfall
    probe('ctx.get', 'pending interactions cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('ctx.on', 'the approval answerer requires the official event substrate', () => {
      return typeof ctx?.on === 'function'
    })
    probe('sessions', 'the pending view folds the official session log', () => {
      let sessions
      try {
      ...
      return Boolean(sessions) && typeof sessions.get === 'function'
    })

  } else if (featureName === 'sessionSelection') { // coordinated model/effort mutation over the audited apiProxy selection seam
    probe('ctx.get', 'selection cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('apiProxy', 'selection requires the official apiProxy with its nested sessions.models read seam', () => {
      let apiProxy
      try {
      ...
      return Boolean(apiProxy) && typeof apiProxy === 'object' && typeof apiProxy.sessions?.models === 'function'
    })

  } else if (featureName === 'sessionInteraction') { // host operation facade: request authority operates over the shared boundary and sessions
    probe('ctx.get', 'session interaction cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    // The shared loop boundary slice, durable session log and the official
    // sessions service are lazy/optional inputs: absence reports typed
    // unavailable/degraded per member at call time, never a guard failure.

  } else if (featureName === 'attention') { // host attention hub: dependency-free memory hub over the shared event substrate
    probe('ctx.get', 'attention cannot resolve official services', () => {
      return typeof ctx?.get === 'function'
    })
    probe('ctx.on', 'attention forwarding requires the shared event substrate', () => {
      return typeof ctx?.on === 'function'
    })

  } else if (featureName === 'checkpoints') { // host checkpoint/restore projection: optional facility faces degrade per member
    probe('ctx.get', 'checkpoints cannot resolve official services and authority faces', () => {
      return typeof ctx?.get === 'function'
    })
    // Capture authorities (branches/transactions/workspace snapshot), the
    // durable record backend and the shared loop slice are per-member inputs:
    // absence degrades typed per member (create unavailable / plan blocked),
    // never a guard failure.

  } else if (featureName === 'decisionParticipation') { // domain decision registries over the events bus participation substrate
    probe('ctx.on', 'decision participation cannot install bus participation entries', () => {
      return typeof ctx?.on === 'function'
    })
    probe('ctx.waterfall', 'decision participation cannot dispatch value-aware waterfalls', () => {
      return typeof ctx?.waterfall === 'function'
    })
    // The live participation substrate (events bus registerParticipation) only
    // exists after the events feature mounts, so it is re-checked at mount
    // time; absence there degrades the feature with a warn, never a guard
    // failure.

  } else if (featureName === 'scopedAgentContributions') { // scoped agent contributions: target-dimension installs over official scoped layers
    probe('ctx.get', 'scoped contributions cannot resolve the official agents registry', () => {
      return typeof ctx?.get === 'function'
    })
    probe('ctx.on', 'scoped contributions lifecycle handling requires the shared event substrate', () => {
      return typeof ctx?.on === 'function'
    })
    // The live agents registry and per-agent contexts are re-resolved per
    // install; absence there degrades typed per call, never a guard failure.
