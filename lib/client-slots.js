...
export function createClientSlots({ ctx, slots, active = true, notifyChanged } = {}) {
  ...
  return Object.freeze({
     ...
     * registers nothing.
     */
    declaration(key) { assertActive(isActive); validateKey(key); return declarationOf(key) },
    list(key) {
      assertActive(isActive)
      validateKey(key)
  ...
  function declarationOf(key) {
    ...
    let epoch
    let snapshot
    try { if (hasSpec) spec = slots.spec(key) } catch { spec = undefined }
    ...
    let status
    if (hasSpec) status = spec === undefined ? 'missing' : 'declared'
