...
/** The subject is definitely absent. */
const MISSING = deepFreeze({ ok: false, code: 'missing' })
/** A malformed argument is not a missing subject: the two stay apart. */
const invalidInput = (reason) => deepFreeze({ ok: false, code: 'invalid-input', reason })
...
export function createSessionActivityView({ store, unavailableSources } = {}) {
  ...
  function current(sessionId, options = {}) {
    if (!isNonEmptyString(sessionId)) return invalidInput('a session id is required')
  ...
  function get(activityId, options = {}) {
    if (!isNonEmptyString(activityId)) return invalidInput('an activity id is required')
