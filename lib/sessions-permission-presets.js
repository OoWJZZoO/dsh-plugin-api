...
export function createPermissionPresetAuthority({
  ...
  const observe = (subject) => {
    gates()
    // The canonical subject is `{ session }` (the same field the namespace's
    // `select` verb uses); a bare session stays accepted as the convenience form.
    const session = subject !== null && typeof subject === 'object' && !Array.isArray(subject) && 'session' in subject
      ? subject.session
      : subject
