  ...
  const face = Object.freeze({
    ...
    observe(filters) {
      ...
      const handle = Object.freeze({
        ...
        subscribe(listener) {
          // A non-function listener subscribes nothing and never throws — the
          // same rule the runtime-absent kernel handle keeps, so both attention
          // states answer one subscription contract.
    ...
    contribute(spec) {
      ...
      // The outer result is frozen: the shape already carries the discriminated
      // result plus the { id, ownerId, seq, dispose() } handle.
