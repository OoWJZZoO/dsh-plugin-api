 ...
 * mutate what the facade exposes as read-only model metadata.
 */
...
export function createLlmApi({ llm, deepFreeze }) {
  return {
    ...
    async modelInfo(provider, model, signal) {
      const info = await llm.resolveModelInfo(provider, model, ...(signal === undefined ? [] : [signal]))
     ...
     * @returns {import('@deepseek-ai/dsh-llm').AdapterRegistrationHandle}
     */
    ...
    /**
     * Declare configurable provider routes.
     *
     * @param {readonly import('@deepseek-ai/dsh-llm').LlmConfigurableProvider[]} entries
     * @returns {import('@deepseek-ai/dsh-llm').DirectoryRegistrationHandle}
     */
    ...
    /**
     * Offer model discovery for a settings namespace.
     *
     * @param {string} settingsNs
     * @param {(request: import('@deepseek-ai/dsh-llm').LlmModelDiscoveryRequest) => Promise<readonly import('@deepseek-ai/dsh-llm').LlmDiscoveredModel[]>} discover
     * @returns {() => void}
     */
