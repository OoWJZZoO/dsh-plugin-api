...
const DEGRADED_ORDER = { active: 0, degraded: 1, unavailable: 2 }

function worstStatus(statuses) {
  let worst = 'active'
  for (const status of statuses) {
    if (status === 'degraded' && DEGRADED_ORDER[worst] < 1) worst = 'degraded'
    else if (status === 'unavailable') worst = 'unavailable'
  }
  return worst
}
 ...
 *   disabledReason?: string,  // capability-disabled reason (P2)
