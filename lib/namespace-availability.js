 ...
 *
 * A namespace surface that already provides its own availability keeps its
 * bounded domain details; only the `status` vocabulary is normalized to
 * `active | degraded | unavailable`. The `services.*` passthrough
 * namespaces are exempt: official passthrough surfaces acquire no facade
...
export function createNamespaceAvailability({ resolveFeatureStatus }) {
  ...
  /** Keep at most sixteen bounded domain detail keys, never the status. */
  const boundedDetails = (raw) => {
   ...
   * back to the backing features. Bounded domain details always survive.
   */
  const availabilityOf = (capabilityPath, existing) => {
    const normalized = (raw, resolved) => {
      if (isPlainRecord(raw)) {
        if (AVAILABILITY_STATUSES.includes(raw.status)) {
          return Object.freeze({ status: raw.status, ...boundedDetails(raw) })
        }
        if (typeof raw.active === 'boolean') {
          return Object.freeze({ status: raw.active ? 'active' : 'unavailable', ...boundedDetails(raw) })
        }
      }
      if (resolved && typeof existing === 'function') {
        return Object.freeze({ status: 'active', ...boundedDetails(raw) })
      }
      return Object.freeze({ status: statusOf(capabilityPath), ...boundedDetails(raw) })
    }
    ...
    if (isPlainRecord(existing)) {
      ...
      // expose it as bounded detail under the standard status vocabulary.
  ...
  const decorate = (surface, names, cache = new WeakMap()) => {
    ...
    for (const name of names) {
      ...
      result = mergeSurface(result, { [last]: decorated })
    }
    return result
  }
