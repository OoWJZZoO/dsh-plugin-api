...
export function createMutationEvidenceIntake({ ctx, logger, now } = {}) {
  ...
  if (typeof ctx?.on === 'function') {
    for (const eventName of ['fs/write-intent', 'fs/edit-intent']) {
      let detach = () => false
      try {
        // The official fs intent dispatches carry three arguments:
        // (target, exec, continuation). The trailing continuation is the last
        // function-valued argument — forwarding it keeps the official
        // waterfall chain alive for later listeners (decision participation);
        // a missing continuation falls through to the old two-argument shape.
        detach = ctx.on(eventName, (payload, second, third) => {
          try {
            if (isObject(payload)) {
              recordObservation({
                kind: eventName,
          ...
          const forward = typeof third === 'function' ? third : typeof second === 'function' ? second : undefined
