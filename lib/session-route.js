...
export function createSessionRouteOwner({ sessions, eventsApi, ctx, logger, coreActive = () => true } = {}) {
  ...
  function detachEntry(entry) {
    ...
    // A detach released by the owner (session disposal, owner epoch end) must
    // also release the caller-facing observation handle, so a subscription
    // that can no longer deliver answers honestly instead of going silent.
  ...
  function addNative(name, listener) {
    ...
    if (typeof eventsApi?.observe === 'function') {
      // The event-face entry answers the discriminated envelope; the feed is
      // its handle (a legacy owner answering a handle directly stays accepted).
      const observed = eventsApi.observe(name)
  ...
  /**
   * Read the handle's own view of the target route.
   *
   * `current()` is the read face of the bound target; a released handle or a
   * target that is no longer provably live answers a typed unavailable view
   * rather than a silently dead stream.
   */
  function handleView(session, liveHandle) {
    if (liveHandle !== true) return unavailableRouteView('the observation handle is released')
    let route
    try {
      route = currentRoute(session)
    } catch {
      return unavailableRouteView('the target session is not live')
    }
    if (route === undefined) return unavailableRouteView('the session route is not currently known')
    return route
  }

  function unavailableRouteView(reason) {
  ...
  const api = {
    ...
    /**
     * Subscribe one listener to the committed route of one live session.
     *
     * The returned value is the standard projection handle: the listener is
     * subscribed through the handle, `subscribe` is additive, and `dispose`
     * detaches exactly this observation (idempotent, never throwing through
     * the caller).
     *
     * @param {object} session the live target session.
     * @param {(route: object) => unknown} [listener] optional route-change
     *   listener; listeners normally attach through the returned handle.
     * @returns {Readonly<object>} frozen `{ current, subscribe, dispose, epoch }`.
     */
    on(session, listener) {
      assertAvailable()
      live(session)
      if (listener !== undefined && typeof listener !== 'function') throw invalidListener()
      let publish = () => {}
      const entry = addEntry(session, (route) => publish(route), false)
      const created = createObserverHandle({
        epoch: `epoch:${++epochSeq}`,
        current: (liveHandle) => handleView(session, liveHandle),
        onDispose: () => detachEntry(entry),
        reportError: () => report(logger, diagnostics, 'routing-listener-failed'),
      })
      publish = created.publish
      entry.release = created.handle.dispose
      if (typeof listener === 'function') created.handle.subscribe(listener)
