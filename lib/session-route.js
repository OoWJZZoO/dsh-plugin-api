...
export function createSessionRouteOwner({ sessions, eventsApi, ctx, logger, coreActive = () => true } = {}) {
  ...
  function addNative(name, listener) {
    const on = typeof eventsApi?.on === 'function' ? eventsApi.on.bind(eventsApi) : ctx?.on?.bind(ctx)
    if (typeof on !== 'function') throw new TypeError('sessionRoute requires an event registration surface')
    const disposer = on(name, listener)
    if (typeof disposer !== 'function') throw new TypeError(`sessionRoute ${name} registration did not return a disposer`)
  ...
  function assertAvailable() {
    ...
    if (!current || disposed) throw new PluginApiFeatureDisabledError('sessionRoute')
  }
  ...
  const api = {
        ...
        return new Promise((resolve, reject) => {
          queueMicrotask(() => {
            if (!current || disposed) {
              ...
              reject(coreIsActive
                ? new PluginApiFeatureDisabledError('sessionRoute')
    ...
    dispose() {
      ...
      let coreIsActive = false
      try { coreIsActive = Boolean(coreActive()) } catch {}
      const reason = coreIsActive ? new PluginApiFeatureDisabledError('sessionRoute') : new PluginApiInactiveError()
