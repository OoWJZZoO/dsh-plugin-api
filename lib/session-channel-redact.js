...
/** Profile registrations carry no caller context here; the root token is the honest owner. */
...
export function createRedactionProfiles({ maxProfiles = 64, maxFieldsPerProfile = 128 } = {}) {
  ...
  /**
   * Register (or replace) a named profile and answer with the standard
   * resource handle. Invalid declarations are typed throws — never a wider
   * default exposure, and never a silently inert registration.
   */
  const registerProfile = (spec, owner) => {
    // A missing or non-object spec is an illegal input: normalize it first so
    // the failure is the typed SPEC_INVALID below, never a bare destructuring
    // TypeError.
    const { id, allowlist } = spec !== null && typeof spec === 'object' ? spec : {}
    if (
      ...
      throw new PluginApiError('PLUGIN_API_CHANNEL_REDACTION_SPEC_INVALID', 'a redaction profile needs a non-empty id and a bounded allowlist of field names')
    }
    // The identity comes from the caller (the facade passes the derived owner,
    // or the root token when the caller cannot be traced): a profile another
    // owner holds is refused typed instead of being taken silently, and the
    // same owner replacing its own profile keeps working.
    ...
    if (previous !== undefined && previous.ownerId !== ownerId) {
      throw new PluginApiError('PLUGIN_API_CHANNEL_REDACTION_OWNER_CONFLICT', `the redaction profile "${id}" is owned by another plugin`)
    }
    const generation = `${ownerId}:${id}:${++registrationSequence}`
    profiles.set(id, { allowlist: new Set(allowlist), ownerId, generation })
    return createResourceHandle({
      id,
      ownerId,
      generation,
      revoke: () => (profiles.get(id)?.generation === generation ? profiles.delete(id) : false),
    })
  }
  ...
  const combinedAllowlist = () => {
    const union = new Set()
    for (const { allowlist } of profiles.values()) {
      for (const f of allowlist) union.add(f)
    }
    return [...union]
  }
