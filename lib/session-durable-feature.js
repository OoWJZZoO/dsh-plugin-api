 ...
 */
import { DURABLE_EVENT_DESCRIPTORS, DURABLE_EVENT_TYPES, isDurableEventType } from './session-durable-catalog.js'
...
export function createDurableObservationHub({ eventsApi, logger } = {}) {
  ...
  if (typeof eventsApi?.observe === 'function') {
    try {
      // The event-face observation entry answers the discriminated envelope;
      // the feed is its handle. A legacy owner that still answers a handle
      // directly stays accepted.
      const observed = eventsApi.observe('session/event')
      const feed = observed?.handle ?? observed
      feed.subscribe((payload) => {
        const [session, event] = Array.isArray(payload) ? payload : [payload]
  ...
  function dispatch(session, event) {
    ...
    // Ordinary baseline session events are outside the finite durable vocabulary;
  ...
  return {
    ...
    disposeEpoch(epoch) {
      if (!epoch || epoch.state === 'closed') return false
      epoch.state = 'closed'
      epoch.entries.clear()
      try { epoch.owner?.close?.() } catch { warn(logger, 'dsh-plugin-api session durable: epoch cleanup failed') }
      return true
    ...
    releaseNative() {
      if (nativeReleased) return false
      nativeReleased = true
      const dispose = nativeDisposer
      nativeDisposer = null
      try { dispose?.() } catch { warn(logger, 'dsh-plugin-api session durable: native hub disposal failed') }
      return true
    },
    close() {
      if (closed) return false
      const epoch = this.closeState()
      this.disposeEpoch(epoch)
      this.releaseNative()
      return true
    },
...
/**
 * Creates the active durable observation observer facade for one durable epoch.
 *
...
export function createSessionDurableApi({ Session, sessions, eventsApi, contracts, owner, reset, hub } = {}) {
  ...
  const subscribe = (once, targetSession, kind, listener) => {
    ...
    if (typeof eventsApi?.observe !== 'function') return () => false
    ...
    const feed = observed?.handle ?? observed
    feed.subscribe((payload) => {
      const [publishedSession, event] = Array.isArray(payload) ? payload : [payload]
      // The wrapper's returned listener promise flows back into the
      // projection feed so async rejections reach its containment.
      return wrapper(publishedSession, event)
    })
    dispose = owner.own(() => {
      try {
        feed.dispose()
      } catch {
        // disposal must never throw into the owner's cleanup path
      }
      return true
    })
    return dispose
  ...
  /**
   * Observe one audited durable record stream.
   *
   * The returned value is the standard projection handle: `current()` answers
   * the latest delivered frozen durable record (a degraded view before the
   * first delivery or after release), and listeners attach through
   * `handle.subscribe`, never as an argument.
   *
   * @param {{ targetSession: object, kind: string }} [subject] the canonical
   *   options subject.
   * @returns {Readonly<object>} frozen `{ current, subscribe, dispose, epoch }`.
   */
  const observeDurable = (subject) => {
    const spec = subject !== null && typeof subject === 'object' && !Array.isArray(subject) ? subject : {}
    const targetSession = spec.targetSession
    const kind = spec.kind
    let lastDelivered = null
    let off = () => {}
    const created = createObserverHandle({
      epoch: `epoch:${++observeSeq}`,
      current: (live) => {
        if (!live) {
          return Object.freeze({ code: 'inactive', kind, reason: 'the durable observation is no longer active' })
        }
        if (lastDelivered === null) {
          return Object.freeze({ code: 'unavailable', kind, reason: 'no durable record has been delivered yet' })
        }
      ...
      onDispose: () => {
        try {
          ...
          // durable teardown never throws back through the release path
        }
      },
      reportError: () => {},
    })
    off = subscribe(false, targetSession, kind, (event) => {
      const frozen = Object.freeze({ ...event })
      lastDelivered = frozen
      created.publish(frozen)
    })
...
/**
 * Build the durable facade the service-side active api drives.
 *
 * The facade is the composition seam between this module and
 * `createActiveSessionDurableApi`: every member that api forwards through must
 * be listed here, so the factory (not an inline literal in the plugin entry)
 * is the single place that can drift. `observeDurable` is the observation
 * entry's forwarding target and must stay present.
 */
export function createSessionDurableFacade({ activeApi, Session, sessions, contracts, logger } = {}) {
  return {
    ...
    observeDurable: activeApi?.observeDurable,
...
function safelyReset(reset, reason, kind, seq) {
  try {
    ...
    // observer contract breach must never escape the session lifecycle contained callback
  }
}
...
/**
 * Validates the finite finite append append contract without writing to the session.
 *
