...
/**
 * Registration failures of the execution visibility registry are typed: the
 * public member is a `policy` registration entry (`typed-throw`), so an
 * illegal spec never escapes as a bare TypeError.
 */
class ExecutionVisibilityRegistrationError extends PluginApiError {
  constructor(message) {
    super('EXECUTION_VISIBILITY_REGISTRATION_INVALID', message)
  }
}
...
export function createExecutionVisibility() {
  ...
  function register(spec) {
    if (!isObject(spec)) throw new ExecutionVisibilityRegistrationError('visibility policy must be an object')
    const { id, ownerId, generation, filter } = spec
    if (!isNonEmptyString(id)) throw new ExecutionVisibilityRegistrationError('visibility policy id is required')
    if (!isNonEmptyString(ownerId)) throw new ExecutionVisibilityRegistrationError('visibility policy ownerId is required')
    if (typeof filter !== 'function') throw new ExecutionVisibilityRegistrationError('visibility policy filter must be a pure function')
    if (byId.has(id)) throw new ExecutionVisibilityRegistrationError(`duplicate visibility policy id "${id}"`)
    const token = {}
