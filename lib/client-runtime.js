...
import { createContributionHandle, createObserverHandle, createPendingContributionHandle } from './contract-kernel.js'
...
function createClientCapabilitiesApi({ isActive, resolveAvailability, resolveMemberPath }) {
  ...
  // inventory; the client accepts its namespace roots and their members at any
  // depth, resolved segment by segment on the live surface, so a consumer asks
  // about `slots.contribute` rather than being told the only addressable names
  // are fourteen roots. A member path is admitted only when the member really
  // exists on that namespace's live surface — an unknown member is answered as
  // unknown instead of inheriting a status it does not own.
  ...
  try {
    ...
    const buildClientAttentionFace = () => {
      ...
      if (!runtime || typeof runtime.current !== 'function') {
        ...
        // The degraded path answers the same observation contract as the
        // installed path: a kernel handle whose release is discriminating and
        // whose subscriptions are no-ops, so callers keep one code path.
