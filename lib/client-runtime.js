    ...
    const buildClient = (callerCtx) => {
      ...
      const capabilities = createClientCapabilitiesApi({
        ...
        resolveAvailability: (capability) => {
          ...
          // Only faces that publish a self-describing (zero-argument)
          // availability probe may answer here: several members expose an
          // `availability(id, owner)` face query with a different meaning, so
          // duck-typing the name would report the wrong status.
          ...
          return member === undefined ? 'unavailable' : 'active'
        },
      })
