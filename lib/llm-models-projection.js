...
export function createLlmModelsProjection({ active, llmProvider, facadeRoutes }) {
   ...
   * fields — declared entries only add fields the official whitelist drops
   * (like a declared context window) and append declared-only model ids.
   */
  const mergeDeclared = (officialModels, declaredRow) => {
    ...
    for (const official of officialModels) {
      ...
      for (const [key, value] of Object.entries(extras)) {
        if (!(key in enriched)) enriched[key] = value
      }
