...
export function createClientSettingsRemote({ remoteContribution, remote, codec, logger } = {}) {
  if (!remoteContribution || typeof remoteContribution.mountRemote !== 'function' || !codec || typeof codec.validate !== 'function') {
    return Object.freeze({ isActive: false, async contribute() { return degraded('remote-unavailable') }, dispose() {} })
  }
  ...
  async function contribute(contribution, { namespace, render } = {}) {
    ...
    record.promise = (async () => {
      try {
        ...
        if (record.disposed) {
          // Disposed while the remote mount owner mount was pending: clean up the committed
  ...
  return Object.freeze({ isActive: true, contribute, dispose() { return Promise.all([...owners.entries()].map(([key, record]) => disposeRecord(key, record))) } })
}
