...
export class PluginApiCapabilityUnavailableError extends PluginApiError {
  constructor(capability, options = {}) {
    const reason = options.reason === undefined ? 'the capability is not available in this composition' : options.reason
...
/** Base class for adapter-decoration registration failures on the facade face. */
export class LlmAdaptersError extends PluginApiError {
  constructor(code, message, options) {
    super(code, message, options)
  }
}

/** The decoration definition failed validation. */
export class LlmAdaptersValidationError extends LlmAdaptersError {
  constructor(message) {
    super('LLM_ADAPTERS_VALIDATION', `dsh-plugin-api llm.adapters: ${message}`)
  }
}

/** A same-owner decoration id conflicts with an existing registration. */
export class LlmAdaptersConflictError extends LlmAdaptersError {
  constructor(message) {
    super('LLM_ADAPTERS_CONFLICT', `dsh-plugin-api llm.adapters: ${message}`)
  }
}

/** The decoration owner cannot be resolved or the replacement is unavailable. */
export class LlmAdaptersUnavailableError extends LlmAdaptersError {
  constructor(message) {
    super('LLM_ADAPTERS_UNAVAILABLE', `dsh-plugin-api llm.adapters: ${message}`)
  }
}

/** Another replacement owner already claims the decoration resource. */
export class LlmAdaptersOwnerConflictError extends LlmAdaptersError {
  constructor(message) {
    super('LLM_ADAPTERS_OWNER_CONFLICT', `dsh-plugin-api llm.adapters: ${message}`)
  }
}
