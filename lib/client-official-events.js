...
export function createClientOfficialEvents(options = {}) {
  ...
  /**
   * Observation entry: a frozen discriminated result carrying the standard
   * observation handle, plus a typed result for a name outside the catalog
   * (never a bare throw, and the catalog itself is queryable). The subject is
   * the same single argument the host entry takes: `{ name }` in the canonical
   * options form, or the bare event name as the convenience form.
   */
  api.observe = (subject) => {
    ...
    return Object.freeze({ ok: true, code: 'observed', handle: record.api.observe() })
  }
...
export function createClientOfficialEvent({
  ...
  const observerEntries = new Set()
  let lastEvent = null

  /**
   * Standard observation handle for this event. The caller subscribes through
   * `handle.subscribe(listener)`, reads the last delivered payload through
   * `handle.current()`, and releases through `handle.dispose()`; the internal
   * bookkeeping never leaves this module.
   */
  function observe() {
    assertAvailable()
    let entry
    const created = createObserverHandle({
      ...
      reportError: (error) => safeLog(logger, 'dsh-plugin-api client event observer failed', error),
    })
    entry = { handle: created.handle, publish: created.publish }
    observerEntries.add(entry)
    try {
      // An observation handle is a subscription in its own right: the native
      // source is bound as soon as the handle exists, not on first listener.
  ...
  function disposeRecord(owner, superseded) {
    ...
    for (const entry of [...observerEntries]) {
      try {
        ...
        // teardown must never throw through the record disposal
      }
    }
...
export function createDisabledClientOfficialEvents(active = true, reason = 'official client event source is unavailable') {
  ...
  for (const name of CLIENT_EVENT_NAMES) {
    ...
    failures[name] = () => {
      if (!isRootActive()) throw new PluginApiInactiveError()
      throw new PluginApiFeatureDisabledError(`client.${key}`, reason)
    }
    leaves[key] = Object.freeze({ isActive: false, on: failures[name], dispose() { return false } })
  }
  const on = (name) => {
    if (!failures[name]) throw new TypeError(`unsupported client event ${String(name)}`)
    return failures[name]()
  }
  const observe = (name) => on(name)
  const dispose = () => false
  const api = Object.freeze({ isActive: false, ...leaves, on, observe, dispose })
  return Object.freeze({ isActive: false, api, events: api, on, observe, dispose })
}
