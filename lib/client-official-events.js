...
export function createClientOfficialEvent({
  ...
  const isAvailable = () => {
    ...
    return isValidSource(readSource(resolve))
  }
  ...
  const assertAvailable = () => {
    ...
    const current = readSource(resolve)
    if (!isValidSource(current)) {
      state.enabled = false
  ...
  function ensureNativeSubscription() {
    ...
    const native = subscribeNative(current, name, dispatch)
...
function subscribeNative(source, name, dispatch) {
  if (typeof source === 'function') return source(name, dispatch)
  if (typeof source?.on === 'function') return Reflect.apply(source.on, source, [name, dispatch])
  if (typeof source?.$on === 'function') return Reflect.apply(source.$on, source, [name, dispatch])
  if (typeof source?.addEventListener === 'function') return Reflect.apply(source.addEventListener, source, [name, dispatch])
...
function safelyDispose(disposer) {
