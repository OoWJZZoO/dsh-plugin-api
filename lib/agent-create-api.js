...
const MEMBER_PATHS = Object.freeze({
  ...
  resume: 'resume',
  register: 'register',
  enter: 'providers.enter',
  announce: 'providers.announce',
  setFactory: 'providers.setFactory',
})
...
function freezeAvailability(state) {
  return Object.freeze({
    ...
    providers: Object.freeze({
      ...
      register: state.enter && state.announce && state.setFactory,
    }),
  })
}
...
export function createAgentCreateExtension({
  ...
  const unavailable = (member) => {
    ...
    throw new PluginApiFeatureDisabledError(
      'agents',
      `agents extension member "${MEMBER_PATHS[member]}" is unavailable`,
    )
  }
  ...
  const createView = (consumerCtx) => ({
    ...
    providers: {
      ...
      // The merged provider registration entry: enter/announce/setFactory
      // converge on one registration verb (the official handle is preserved).
      register(spec, owner) {
