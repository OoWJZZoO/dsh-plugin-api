 ...
 * machinery shared by the generic `pluginApi.remotes.publish` (host-remote)
...
export function publishRemotePublication({
  ...
  // Step 6 — commit the owner record and return a stable disposer. The
  // generation is an opaque token minted here: it identifies the current
  // occupier of the service-key slot, so the owning facade handle can report
  // stale disposers without ever retracting a later publication.
  const record = {
    serviceKey,
    service,
    methods: effectiveMethods,
    binding,
    generation: `${serviceKey}:${++publicationGeneration}`,
    disposer: null,
    disposed: false,
  }
  const disposer = () => {
