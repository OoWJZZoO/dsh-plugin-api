 ...
 * and the ST4 `pluginApi.settings.remote` bridge. Every feature gets its own
 * per-context record map keyed by `ownerName`, so `settingsRemote` and `remote`
 * keep fully independent owner semantics (AC 5.6).
 *
...
export function publishRemotePublication({
  ...
  // Step 1 — owner idempotence short-circuit (AC 1.5): same key + reference-equal
  ...
  // Step 2 — read-only conflict probe (AC 4.1b): the official service registry
  ...
  // checks, so a doomed publication never mutates the caller's object (M3).
  // Safety net: even when the caller passes `rehome:false` (host-remote/ST4
  ...
  // leak into every unrelated plain object (AC 2.4 / 3.7).
  let effectiveMethods = methods
  if (rehome || isPlainObject(service)) {
    if (isRehomedService(service)) {
      // D2b recovery: an already-re-homed object no longer exposes its methods
  ...
  let unregister
  try {
    // Step 3 — binding (AC 2.1). Idempotent: an already-consistent binding from
    // a failed prior attempt (D2b recovery) is reused instead of redefined on a
    ...
    // (AC 2.2 / 2.4). Markers live on the dedicated prototype only.
    ...
    // Step 5 — register through the official provider boundary (AC 2.3).
    ...
    // Roll back only this call's completed registration (AC 4.4).
    if (typeof unregister === 'function') {
      try {
        unregister()
      } catch (rollbackError) {
 ...
 * markers off `Object.prototype` / any shared/foreign prototype (AC 2.4 / 3.7).
 *
...
export function rehomeService({ service, methods }) {
  ...
  try {
    ...
    // typed error (AC 3.1 "无法安全 re-home → typed validation error"), never
...
/** Own callable members of a service's dedicated prototype (D2b recovery). */
...
/** JSON-safe boundary predicates / helpers (shared, mirror ST4 semantics). */
