...
/** Client→host method tokens carried by the pending-interaction route. */
export const INTERACTIONS_CLIENT_METHODS = Object.freeze([
...
/** Client→host method tokens carried by the selection route. */
export const SELECTION_CLIENT_METHODS = Object.freeze([
...
/**
 * Build the host route handler for the pending-interaction route.
 *
 * One member per client token; every answer is projected value-only, so a
 * callable or private object can never ride the carrier. An unknown token is
 * refused typed instead of being guessed onto a member.
 */
export function createInteractionsRouteHandler({ interactions, ctx } = {}) {
  return async (args, signal) => {
    const method = typeof args?.method === 'string' ? args.method : ''
    const payload = isObject(args?.payload) ? args.payload : {}
    if (method === 'sessions.interactions.list') {
      return projectWireOutcome(await interactions.list(payload))
    }
    if (method === 'sessions.interactions.get') {
      return projectWireOutcome(await interactions.get(payload))
    }
    if (method === 'sessions.interactions.respond') {
      const input = signal === undefined ? payload : { ...payload, signal }
      return projectWireOutcome(await interactions.respond(input, ctx))
    }
    if (method === 'sessions.interactions.availability') {
      return projectWireOutcome(interactions.availability())
    }
...
/**
 * Build the host route handler for the selection route.
 *
 * `set` is a coordinated mutation: the caller's observed snapshot rides along
 * as `expected` and the authority decides the commit eligibility, so a client
 * can never write a value it did not observe.
 */
export function createSelectionRouteHandler({ selection, ctx } = {}) {
  return async (args, signal) => {
    const method = typeof args?.method === 'string' ? args.method : ''
    const payload = isObject(args?.payload) ? args.payload : {}
    if (method === 'sessions.selection.get') {
      return projectWireOutcome(await selection.get(payload, ctx))
    }
    if (method === 'sessions.selection.set') {
      const input = signal === undefined ? payload : { ...payload, signal }
      return projectWireOutcome(await selection.set(input, ctx))
    }
    if (method === 'sessions.selection.availability') {
      return projectWireOutcome(selection.availability())
    }
    return { ok: false, code: 'unsupported', reason: `unsupported client method '${method}'` }
  }
}
