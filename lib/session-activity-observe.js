...
export function createSessionActivityObserverHub({ store, render } = {}) {
  ...
  function observe(options = {}) {
    ...
    const observer = {
      ...
      sessionId: isNonEmptyString(opts.sessionId) ? opts.sessionId : undefined,
    ...
    observer.current = () => {
      ...
      if (!observer.sessionId) return deepFreezeFallback()
...
function deepFreezeFallback() {
  return Object.freeze({ snapshot: undefined, sessionQueue: null, gap: null })
}