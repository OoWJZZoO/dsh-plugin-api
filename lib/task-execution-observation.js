...
export function createTaskExecutionObservation({
  ...
  const consultSettlementRecovery = async ({ taskId, attemptId, outcome, reason, revision }) => {
    ...
    try {
      result = await authority.decide({
        ...
        capability: { operationId: 'task-settlement', scopeOwner: 'facade-task-owner', scopeGeneration: generation },
   ...
   *
   * The declared namespace status aggregates its registered source parts: an
   * unavailable source makes the namespace degraded, and the degrading part is
   * named in the reason instead of being hidden behind `active`.
   */
  const mountAvailability = () => {
    const view = availabilitySnapshot()
    const degradedSource = Object.entries(view.sources ?? {})
      .find(([, value]) => value === 'unavailable')
    if (degradedSource === undefined) return view
    return deepFreeze({
      ...view,
      status: 'degraded',
      reason: `the ${degradedSource[0]} source is unavailable`,
    })
  }

  const inactive = (action) => deepFreeze({
    ok: false,
    code: 'inactive',
    action,
  ...
  const invalid = (reasonValue, { taskId, action } = {}) => deepFreeze({
    ok: false,
    code: 'invalid-input',
    reason: reasonValue,
    ...(taskId ? { taskId } : {}),
  ...
  const failed = (code, { taskId, action, reason, observed, attemptId, recovery } = {}) => deepFreeze({
    ok: false,
    code,
    ...(taskId ? { taskId } : {}),
    ...(attemptId ? { attemptId } : {}),
  ...
  /**
   * Read one task record with the shared absence vocabulary.
   *
   * The registry is the task authority: an answer without failure evidence is
   * its authoritative "no such record" answer, so the caller can know the
   * record is absent (`missing`). An answer that reports a backend failure, an
   * indeterminate lookup, or a record outside the caller's declared workspace
   * scope means existence cannot be determined (`unavailable`).
   */
  const readRecord = async (taskId, audience) => {
    if (disposed) return { ok: false, code: 'unavailable', reason: 'the task observation is disposed' }
    let found
    try {
      found = await registry.read(taskId)
    } catch {
      return { ok: false, code: 'unavailable', reason: 'the task registry could not be read' }
    }
    if (!found?.ok) {
      const code = read(found, 'code')
      const evidence = read(found, 'reason')
      // An answer that proves absence — an explicit `missing`, or a lookup miss
      // carrying no failure evidence — answers `missing`; anything else means
      // existence cannot be determined.
      const absent = code === 'missing' || (code === 'unavailable' && evidence === undefined)
      return absent
        ? { ok: false, code: 'missing', reason: 'the task is not registered' }
        : { ok: false, code: 'unavailable', reason: 'the task registry could not be read' }
    }
    if (scopeDenies(found.record, audience)) {
      return { ok: false, code: 'unavailable', reason: 'the task is outside the requested workspace scope' }
    }
  ...
  async function register(input = {}) {
    ...
    // Illegal specs stay inside the discriminated contract: a missing or
    // non-object spec answers `invalid-input` like every other bad field,
    // never a rejected promise carrying a bare TypeError.
    if (input === null || typeof input !== 'object' || Array.isArray(input)) {
      return invalid('task registration requires an object spec', { action: 'register' })
    }
    const taskId = normalizeTaskId(input.taskId)
    if (!taskId.ok) return invalid(taskId.reason, { action: 'register' })
    const ownerId = normalizeOwnerId(input.ownerId)
    if (!ownerId.ok) return invalid(ownerId.reason, { taskId: taskId.value, action: 'register' })
    const scope = normalizeScope(input.scope)
    if (!scope.ok) return invalid(scope.reason, { taskId: taskId.value, action: 'register' })
    const intent = normalizeIntent(input.intent)
    if (!intent.ok) return invalid(intent.reason, { taskId: taskId.value, action: 'register' })
    const provenance = normalizeProvenance(input.provenance)
    if (!provenance.ok) return invalid(provenance.reason, { taskId: taskId.value, action: 'register' })
  ...
  async function start(taskId, options = {}) {
    ...
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'start' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { taskId: normalizedId.value, action: 'start', reason: found.reason })
    }
  ...
  async function claim(taskId, options = {}) {
    ...
    const normalizedId = normalizeTaskId(taskId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'claim' })
    const ownerId = normalizeOwnerId(options.ownerId)
    if (!ownerId.ok) return invalid(ownerId.reason, { taskId: normalizedId.value, action: 'claim' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { taskId: normalizedId.value, action: 'claim', reason: found.reason })
    }
    const record = found.record
  ...
  async function reassign(taskId, options = {}) {
    ...
    const normalizedId = normalizeTaskId(taskId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'reassign' })
    const ownerId = normalizeOwnerId(options.ownerId)
    if (!ownerId.ok) return invalid(ownerId.reason, { taskId: normalizedId.value, action: 'reassign' })
    const expectedGeneration = boundedString(options.expectedGeneration, 120)
    if (!expectedGeneration) {
      return invalid('reassign requires the expected generation of the prior attempt', { taskId: normalizedId.value, action: 'reassign' })
    }
    const reason = boundedString(options.reason, 120)
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { taskId: normalizedId.value, action: 'reassign', reason: found.reason })
    }
    const record = found.record
  ...
  async function settle(taskId, options = {}) {
    ...
    const normalizedId = normalizeTaskId(taskId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'settle' })
    const attemptId = boundedString(options.attemptId, 120)
    if (!attemptId) return invalid('settle requires the attempt identity', { taskId: normalizedId.value, action: 'settle' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { taskId: normalizedId.value, action: 'settle', reason: found.reason })
    }
    const record = found.record
  ...
  async function attach(taskId, options = {}) {
    ...
    const normalizedId = normalizeTaskId(taskId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'attach' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { taskId: normalizedId.value, action: 'attach', reason: found.reason })
    }
    const record = found.record
    if (record.state !== 'registered' && record.state !== 'active') {
      return failed('conflict', {
  ...
  async function get(taskId, options = {}) {
    ...
    const normalizedId = normalizeTaskId(taskId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'get' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      // Absence is a typed answer: `missing` when the registry proved the
      // record is not there, `unavailable` when it cannot be determined.
      return failed(found.code, { taskId: normalizedId.value, action: 'get', reason: found.reason })
    }
  ...
  /**
   * Observe one task identity.
   *
   * @param {{ taskId: string, audience?: object|string, signal?: AbortSignal }|string} subject
   *   the canonical options subject; a bare task id stays accepted as the
   *   convenience form.
   * @returns {Promise<Readonly<{ current: () => Promise<object>,
   *   subscribe: (listener: Function) => Function, dispose: () => object,
   *   epoch: string, taskId: string, initialState: object }>>} the frozen
   *   projection handle; subscribing after release is a no-op and its
   *   `dispose()` answers the shared discriminated result.
   */
  async function observe(subject, options = {}) {
    if (disposed) return inactive('observe')
    const isSpec = subject !== null && typeof subject === 'object' && !Array.isArray(subject)
    const spec = isSpec ? subject : {}
    const rawId = isSpec ? spec.taskId : subject
    const normalizedId = normalizeTaskId(rawId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'observe' })
    const audience = normalizeAudience(spec.audience !== undefined ? spec.audience : options.audience)

    let initialRecord = null
    const found = await readRecord(normalizedId.value, audience)
    if (found.ok) initialRecord = found.record
    const taskIdValue = normalizedId.value

    // Owner-private record: the mutable fields never leave this closure. The
    // caller only receives the frozen projection handle.
    const epoch = `epoch:${++epochSeq}`
    const signal = options.signal
    let abortHandler = null
    const detach = () => {
      const listeners = observers.get(taskIdValue)
      if (!listeners) return
      listeners.delete(onEvent)
      if (listeners.size === 0) observers.delete(taskIdValue)
    }
    const detachAbort = () => {
      if (!signal || !abortHandler) return
      try {
    ...
    const currentView = async (liveHandle) => {
      if (disposed || liveHandle !== true) {
        return deepFreeze({ code: 'inactive', taskId: taskIdValue, epoch, observedAt: observedAt() })
      }
      const currentFound = await readRecord(taskIdValue, audience)
      if (!currentFound.ok) {
        return deepFreeze({
          code: currentFound.code,
          taskId: taskIdValue,
          epoch,
          observedAt: observedAt(),
          resync: true,
    ...
    const created = createObserverHandle({
      epoch,
      current: (liveHandle) => currentView(liveHandle),
      onDispose: () => {
        detach()
        detachAbort()
        observationHandles.delete(created.handle)
      },
      // A listener that throws (or rejects) only degrades that listener.
      reportError: () => {},
    })
    function onEvent(event) {
      created.publish(deepFreeze({ ...event, epoch }))
    }
    let listenerSet = observers.get(taskIdValue)
    if (!listenerSet) {
      listenerSet = new Set()
      observers.set(taskIdValue, listenerSet)
    }
    listenerSet.add(onEvent)
    observationHandles.add(created.handle)
    ...
    return Object.freeze({
      ...
      initialState: initialRecord
        ...
        : deepFreeze({ taskId: taskIdValue, state: 'unknown' }),
    })
  }
  ...
  async function history(taskId, options = {}) {
    ...
    const normalizedId = normalizeTaskId(taskId)
    if (!normalizedId.ok) return invalid(normalizedId.reason, { action: 'history' })
    const audience = normalizeAudience(options.audience)
    const found = await readRecord(normalizedId.value, audience)
    if (!found.ok) {
      return failed(found.code, { taskId: normalizedId.value, action: 'history', reason: found.reason })
    }
    const record = found.record
