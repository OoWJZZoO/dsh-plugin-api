 ...
 * the public `pluginApi.execution`, `pluginApi.recovery`,
 * `pluginApi.coordination`, `pluginApi.diagnostics`, and
 * `pluginApi.workspaceTransactions` surfaces plus public jobs/workflows/
...
export function createTaskExecutionObservation({
  ...
  const failed = (code, { taskId, operation, reason, observed, attemptId } = {}) => deepFreeze({
    ok: false,
    code,
    ...(taskId ? { taskId } : {}),
    ...(attemptId ? { attemptId } : {}),
  ...
  async function settle(taskId, options = {}) {
    ...
    if (isObject(recovery) && typeof recovery.classify === 'function') {
      try {
        const classified = recovery.classify({
          kind: outcome,
          reason: options.reason,
        })
        if (classified?.ok && classified.class) {
    ...
    const settledAttempts = writtenSettling.record.attempts.map((item) => {
      ...
      return {
        ...
        provenance: [...item.provenance, ...safety, ...sourceProvenance],
      }
    })
  ...
  async function observe(taskId, options = {}) {
    ...
    subscription.current = async () => {
      if (subscription.disposed) {
        return deepFreeze({ code: 'inactive', taskId: normalizedId.value, epoch: subscription.epoch, observedAt: observedAt(), availability: setEpoch() })
      }
    ...
    return deepFreeze({
      ...
      initialState: initialRecord
        ...
        : deepFreeze({ taskId: normalizedId.value, state: 'unknown', availability: setEpoch('no record') }),
    })
  }
