 ...
 *
 * D5 refactor (recorded deviation): the generic binding / marking / registration
 * / rollback / disposer machinery now delegates to the shared core
 * `remote-publication.js` (owner name `settingsRemote`), so ST4 and the generic
 * `pluginApi.remote.publish` share one binding/rollback implementation with fully
...
export function createSettingsRemoteApi({ ctx, settings, protocol, active = true, logger } = {}) {
  ...
  // ST4 publishes through `ownerName:'settingsRemote'` so its owner semantics are
  // fully independent from `pluginApi.remote.publish` (AC 5.6).
  ...
  function remote(namespace, serviceKey = namespace) {
    ...
    // Namespace-aware idempotence / conflict (AC 1.5 / 1.6): the settings face
 ...
 * registration are the shared core's responsibility (D5).
 */
function createSettingsService({ settings, protocol, namespace }) {
  ...
  // service prototype instead (shared requirement AC 2.4 / design D2).
  ...
  // factory returns — D5 delegation, design §3.1 step 3).
