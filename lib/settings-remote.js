...
export function createSettingsRemoteApi({ ctx, settings, protocol, active = true, logger } = {}) {
  ...
  function remote(namespace, serviceKey = namespace) {
    ...
    const disposer = () => {
      ...
      // Cordis reflect.provide() disposers delete by key, not by service
      // identity. A delayed cleanup must never retract a later provider.
      const published = readPublishedService(ctx, serviceKey)
      if (published !== undefined && published !== record.service) return false
      try {
        return unregister()
      } catch (error) {
...
function readPublishedService(ctx, serviceKey) {
  try {
    return typeof ctx?.get === 'function' ? ctx.get(serviceKey) : undefined
  } catch {
    // An unavailable probe is not evidence that this epoch still owns the
    // service. Skipping unregister is the fail-safe outcome.
    return undefined
  }
}
