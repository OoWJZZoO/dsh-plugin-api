...
import { createContributionHandle } from './contract-kernel.js'

/** The settings remote face carries no caller context; the root token is the honest owner. */
const SETTINGS_REMOTE_OWNER = 'root'
...
export function createSettingsRemoteApi({ ctx, settings, protocol, active = true, logger } = {}) {
  ...
  /**
   * Contribution entry: a frozen discriminated result carrying the standard
   * contribution handle. The synchronous official mount / lease / rebinding
   * work inside `remote` is unchanged; only the outer presentation moves to the
   * contribution idiom, where an unavailable environment is a code and not a
   * throw.
   */
  function contribute(namespace, serviceKey = namespace) {
    let disposer
    try {
      disposer = remote(namespace, serviceKey)
    } catch (error) {
      if (error instanceof PluginApiInactiveError) {
        return Object.freeze({ ok: false, code: 'inactive', reason: String(error?.message ?? error) })
      }
      // A same-key conflict carries the key it collided on and is a stable
      // conflict code; a malformed namespace or service key stays invalid input.
      if (error instanceof PluginApiRemoteError && error.serviceKey !== undefined) {
        return Object.freeze({ ok: false, code: 'conflict', reason: String(error?.message ?? error) })
      }
      return Object.freeze({ ok: false, code: 'invalid-input', reason: String(error?.message ?? error) })
    }
    if (typeof disposer !== 'function') {
      return Object.freeze({ ok: false, code: 'unavailable', reason: 'the settings remote mount did not return a disposer' })
    }
    const id = `${namespace}:${serviceKey}`
    return Object.freeze({
      ok: true,
      code: 'contributed',
      handle: createContributionHandle({
        id,
        ownerId: SETTINGS_REMOTE_OWNER,
        seq: ++contributionSequence,
        revoke: () => {
          disposer()
          ownedDisposers.delete(disposer)
          return true
        },
      }),
    })
