...
function createRemoteService({ settings, protocol, namespace, serviceKey }) {
  const service = {
    get() {
      ...
      assertJsonValue(value, 'settings snapshot')
      return value
    },
...
function assertNamespace(settings, namespace) {
  if (typeof settings.scope === 'function') {
    try {
      settings.scope(namespace)
      return
    } catch (error) {
      if (error instanceof PluginApiSettingsNamespaceError) throw error
      throw new PluginApiSettingsNamespaceError(namespace)
    }
  }
  if (typeof settings.describe === 'function') {
    const descriptors = settings.describe({})
    if (!Array.isArray(descriptors) || !descriptors.some((descriptor) => descriptor?.ns === namespace)) {
      throw new PluginApiSettingsNamespaceError(namespace)
    }
    return
  }
  if (typeof settings.get !== 'function' || settings.get(namespace) === undefined) {
    throw new PluginApiSettingsNamespaceError(namespace)
  }
}
...
function validatePrimitives(ctx, settings, protocol) {
  ...
  if (settings == null || typeof settings.get !== 'function' || typeof settings.mutate !== 'function') return 'official settings service is unavailable or malformed'
  if (
