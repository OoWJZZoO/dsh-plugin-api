...
export function resolveLeafProvider(record, rootState, callerCtx) {
  ...
  let check
  try {
    check = validateStaticMembers(provider, record.descriptor.members)
  } catch {
    // A throwing member read during re-validation is an invalid member; the
    // official method invocation below still propagates its own raw result.
    check = { ok: false, reason: CLIENT_REASONS.invalidMember }
  }
  if (!check.ok) {
...
function acceptImport(rootState, record, namespace) {
  ...
  try {
    ...
    check = validateStaticMembers(provider, descriptor.members)
  } catch {
    // A throwing member on the initial provider is treated as an invalid
    // member: the contract cannot be verified, so the leaf must settle
    // atomically instead of hanging in pending.
    check = { ok: false, reason: CLIENT_REASONS.invalidMember }
  }
