...
const definitions = [
  {
    ...
    ctxService: 'appExit',
    members: [
      // The official launcher publishes the process-exit seam; a deployment
      // that mounts no launcher simply does not carry it, so the member is
      // optional and the facade stays honest about the absence.
      { kind: 'method', name: 'exit', optional: true },
    ],
  },
  {
    key: 'agentLoop',
  ...
  },
  {
    ...
    members: [
      ...
      // The per-session model selection seam is reached through the official
      // sessions bus. Only these two narrow operations are whitelisted — the
      // whole `sessions` object (every mux operation) is deliberately never
      // exposed — and both are optional so an installation whose apiProxy does
      // not carry them keeps `downloads`/`respond` untouched.
