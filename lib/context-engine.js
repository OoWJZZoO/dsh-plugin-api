...
/**
 * Fixed decision-priority vocabulary shared with the policy idiom.
 * Unknown priorities are registration errors, never a silent default.
 */
export const PROVENANCE_POLICY_PRIORITIES = Object.freeze([
  'lowest',
  'low',
  'normal',
  'high',
...
const PROVENANCE_POLICY_PRIORITY_RANK = new Map(
  PROVENANCE_POLICY_PRIORITIES.map((name, index) => [name, index]),
)

/** Typed registration error: invalid input, unavailable engine, owner conflict. */
export class ProvenancePolicyRegistrationError extends PluginApiError {
...
export function createContextEngine({
  ...
  const policies = [] // { id, ownerId, generation, name, priority, decide, seq }
  const policiesBySlot = new Map() // `${ownerId}\u0000${id}` -> policy entry (latest registration)
  const policiesById = new Map() // id -> Set(ownerId) for cross-owner conflict detection
  const observers = new Map() // seq -> { seq, created, lastDelivered }
  ...
  function policyOrder() {
    return [...policies].sort((a, b) =>
      (PROVENANCE_POLICY_PRIORITY_RANK.get(b.priority) ?? PROVENANCE_POLICY_PRIORITY_RANK.get('normal'))
      - (PROVENANCE_POLICY_PRIORITY_RANK.get(a.priority) ?? PROVENANCE_POLICY_PRIORITY_RANK.get('normal'))
      || a.seq - b.seq)
  }
  ...
  function notifyObservers(payload) {
    ...
    for (const record of [...observers.values()]) {
      record.lastDelivered = snapshot
      record.created.publish(snapshot)
    }
  }
  ...
  function compose(request, options = {}) {
    ...
    if (policyOrdered.length > 0) {
      ...
      for (const policy of policyOrdered) {
        let result
        try {
          result = policy.decide(frozenInput, context)
        } catch (error) {
  ...
  /** The degraded read face of a released or not-yet-delivered observation. */
  const releasedFrame = (reason) => frozenResult({ ok: false, code: 'INACTIVE', reason })

  /**
   * Subscribe to provenance notifications.
   *
   * The returned value is the standard projection handle: `current()` answers
   * the latest delivered frozen notification frame (a degraded frame before
   * the first delivery, after release, or once the engine is disposed), and
   * listeners are attached through `handle.subscribe`, never as an argument.
   *
   * @returns {Readonly<object>} frozen `{ current, subscribe, dispose, epoch }`.
   */
  function observe() {
    if (disposed) {
      const released = createObserverHandle({
        epoch: `epoch:${++observerSeq}`,
        current: () => releasedFrame('the provenance observation is no longer active'),
      })
      return released.handle
    }
    const seq = ++observerSeq
    const record = { seq, created: null, lastDelivered: null }
    const created = createObserverHandle({
      epoch: `epoch:${seq}`,
      current: (live) => {
        if (!live || disposed) return releasedFrame('the provenance observation is no longer active')
        if (record.lastDelivered === null) {
          return releasedFrame('no provenance notification has been delivered yet')
        }
        return record.lastDelivered
      },
      onDispose: () => {
        observers.delete(seq)
      },
      reportError: (error) => report(`<observer:${seq}>`, `context observer listener failed: ${error?.message ?? error}`),
    })
    record.created = created
    observers.set(seq, record)
    return created.handle
  }

  /**
   * Register one provenance compose policy and answer with the standard policy
   * handle `{ id, ownerId, generation, dispose() }`.
   *
   * The spec carries `id`, an optional `priority` from the fixed vocabulary
   * (`lowest | low | normal | high | highest | monitor`) and the pure
   * `decide(nodes, context)` decision callback. The owner is derived from the
   * caller's context (root token when the caller is untraceable) and the
   * generation is minted here, so a caller declares neither. Same owner + same
   * id is latest-wins; the same id under a different owner is a typed owner
   * conflict.
   *
   * Registration failures are typed throws — never a discriminated result.
   *
   * @param {{ id: string, priority?: string, decide: Function, name?: string }} spec
   * @param {object} [callerCtx] facade-forwarded caller binding.
   * @returns {Readonly<{ id: string, ownerId: string, generation: string, dispose: () => object }>}
   * @throws {ProvenancePolicyRegistrationError}
   */
  function registerPolicy(spec, callerCtx) {
    if (disposed) {
      throw new ProvenancePolicyRegistrationError(
        'PROVENANCE_POLICY_UNAVAILABLE',
        'context provenance engine is disposed',
      )
    }
    if (spec === null || typeof spec !== 'object' || Array.isArray(spec)) {
      throw new ProvenancePolicyRegistrationError(
        'PROVENANCE_POLICY_INVALID',
        'provenance policy registration requires a spec object with id, priority, and decide',
      )
    }
    ...
    if (id.length === 0) {
      throw new ProvenancePolicyRegistrationError(
        'PROVENANCE_POLICY_INVALID',
        'provenance policy id must be a non-empty string',
      )
    }
    const priority = spec.priority === undefined ? 'normal' : spec.priority
    if (!PROVENANCE_POLICY_PRIORITY_RANK.has(priority)) {
      throw new ProvenancePolicyRegistrationError(
        'PROVENANCE_POLICY_INVALID',
        `provenance policy priority ${JSON.stringify(spec.priority)} is invalid; ` +
          `expected one of ${PROVENANCE_POLICY_PRIORITIES.join(' | ')}`,
      )
    }
    if (typeof spec.decide !== 'function') {
      throw new ProvenancePolicyRegistrationError(
        'PROVENANCE_POLICY_INVALID',
        'provenance policy decide must be a function',
      )
    }
    const ownerId = callerIdentityOf(callerCtx)
    const ownerPolicies = policiesById.get(id)
    if (ownerPolicies && ownerPolicies.size > 0 && !ownerPolicies.has(ownerId)) {
      throw new ProvenancePolicyRegistrationError(
        'PROVENANCE_POLICY_OWNER_CONFLICT',
        `provenance policy id "${id}" is already registered by another owner`,
      )
    }
    const name = typeof spec.name === 'string' && spec.name.length > 0 ? spec.name : id
    const previous = policiesBySlot.get(`${ownerId}\u0000${id}`)
    if (previous) removePolicy(previous)
    const entry = Object.freeze({
      id,
      ownerId,
      generation: `${ownerId}:${++policySeq}`,
      name,
      priority,
      decide: spec.decide,
      seq: ++policySeq,
    ...
    policies.push(entry)
    policiesBySlot.set(`${ownerId}\u0000${id}`, entry)
    const currentOwners = policiesById.get(id)
    if (currentOwners) currentOwners.add(ownerId)
    else policiesById.set(id, new Set([ownerId]))
    return createResourceHandle({
      id,
      ownerId,
      generation: entry.generation,
      revoke: () => removePolicy(entry),
    })
  }

  function removePolicy(entry) {
    const index = policies.indexOf(entry)
    if (index < 0) return false
    policies.splice(index, 1)
    const slotKey = `${entry.ownerId}\u0000${entry.id}`
    if (policiesBySlot.get(slotKey) === entry) {
      policiesBySlot.delete(slotKey)
      const owners = policiesById.get(entry.id)
      if (owners) {
        owners.delete(entry.ownerId)
        if (owners.size === 0) policiesById.delete(entry.id)
      }
    }
    return true
  }
