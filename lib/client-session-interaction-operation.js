...
function validRequestOutcome(value) {
  ...
  if (value.ok === true) {
    if (value.code === 'accepted') {
      // A queued delivery carries a cancellable queued reference instead of a
      // live operation (the message waits in the official inbox); a steered or
      // ordinary acceptance carries the live operation. Both are accepted
      // shapes, anything else is malformed.
      if (value.delivery === 'queue') {
        return isPlainObject(value.queuedRef) && typeof value.queuedRef.id === 'string'
      }
