...
/**
 * Spec normalization and four-action discrimination (Req 7.5): a spec that
 * belongs to another entry is rejected with a typed validation naming the
 * correct entry, never accepted and misinterpreted.
 */
...
export function createLlmAdapterRegistration({ active, llmProvider, ownerOf, facadeOwnerIds }) {
  ...
  const disposeEntry = (entry, handleGeneration) => {
    ...
    if (live !== entry || (handleGeneration !== undefined && live.generation !== handleGeneration)) {
      return Object.freeze({ status: 'stale', reason: `route "${entry.id}" is already revoked or superseded` })
    }
    ...
    return Object.freeze({
      ...
      dispose() {
        try {
          ...
          return Object.freeze({ status: 'unavailable', reason: error?.message ?? 'registration disposal failed' })
        }
      },
    })
  }
