 ...
 *
 * `opts.kind` performs the per-client audience trim that must happen host-side
 * before an item enters any forwarded payload: when the item's audience does
 * not include the given client kind, no payload is produced at all (the item
 * never leaves the host boundary for that stream).
 *
 * @param {object} item
 * @param {object} [opts]
 * @param {string} [opts.kind] - the client kind the payload is trimmed for.
 * @returns {object | null} frozen public item payload, or null on failure.
 */
export function buildRedactedItemPayload(item, opts = {}) {
  try {
    if (opts.kind !== undefined && opts.kind !== null && opts.kind !== 'all') {
      if (item.audience !== 'all') {
        const kinds = Array.isArray(item.audience) ? item.audience : []
        if (!kinds.includes(opts.kind)) return null
      }
    }
