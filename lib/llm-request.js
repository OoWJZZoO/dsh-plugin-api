/**
 * L4 synchronous compatibility transform owner — pluginApi.llm.request.
 *
 * This module owns the raw `llm/stream` listener, the L4 transform registry,
 * operation records, terminal image classification, the L2 image-policy phase
 ...
 * docs/specs/plugin-api-semantic-hooks-m2 (A1); `llm/admission` never installs
 ...
 * (task 5.1): both enter this scope before resolving target capability so that
 * an active admission gateway overlay is never observed by owner classification.
 * The gateway resolver wrapper (llm-request-m2 task 4) consults
...
/**
 * Feature-private pipeline handle shared with the admission feature (task 4):
 * the admission mounter injects its L2 policy registry source; the request
...
/**
 * Install the L4 compatibility transform owner.
 *
 * @param {object} deps
...
export function mountLlmRequestFeature({
  ...
  function evaluatePolicyMatches(policies, context, record) {
    ...
    for (const entry of policies) {
      ...
      try {
        ...
        reportOnce(record, `policy-match:${entry.id}`, warn, error)
        continue
      }
      if (matched === true) {
        selected.push(entry)
        ...
        reportOnce(record, `policy-match:${entry.id}`, warn, new Error('match must return a synchronous boolean'))
      }
    }
    return selected
  }
  ...
  /** Run the frozen L2 policy list for non-native terminal image input. */
  ...
  /** Synchronous L4 phase; returns the working candidate and replacement flag. */
  function runL4Phase(record, options) {
  ...
  function runOperation(record, options, next) {
    if (!isObject(options) || !Array.isArray(options?.messages)) {
      ...
      // (semantic-hooks A2.5): pass the official operation through unchanged.
      settle(record)
      return next()
    }
    ...
    // needs no translation and can pass through unchanged (A2.5 default).
    ...
    const { candidate, replaced } = runL4Phase(record, options)
  ...
  /** Async extension for terminal image input: capability then L2 phase. */
  async function runImageOperation(record, candidate, next) {
    ...
    if (!isNativeImageCapability(capability)) {
      ...
      // A successful L2 phase always eliminates image input, so its terminal
  ...
  function listener(options, next) {
    if (isObject(options)) {
      ...
      if (mark && mark.owner === OWNER && mark.epoch === currentEpoch && mark.record.status === 'running') {
        // Our own marked candidate: bypass L4/L2/assertions/effects, continue
