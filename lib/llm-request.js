 ...
 * returns the standard policy handle `{ id, ownerId, generation, dispose() }`:
 * the owner is derived from the caller's context (root token when the caller is
 * untraceable) and the generation is minted here, so a caller never declares
 * either. Same owner + same id is latest-wins; the same id under a different
 * owner is a typed owner conflict. `dispose()` answers with the shared
 * discriminated result (`revoked` / `stale`) and removes only the exact
 * registration token, so a newer registration with the same id survives.
 * `snapshot()` returns entries ordered by priority then successful registration
 * sequence.
 */
export function createTransformRegistry() {
  ...
  function register(spec, callerCtx) {
    ...
    if (owners && owners.size > 0 && !owners.has(ownerId)) {
      throw new LlmRequestTransformRegistrationError(
        `transform id "${data.id}" is already registered by another owner`,
      )
    }
    const token = {}
...
export function mountLlmRequestFeature({
  ...
  const api = {
    // The trailing caller context is the facade-forwarded caller binding used
    // to derive the registration owner; without it the shared root token binds
    // the registration.
    transform: (spec, callerCtx) => registry.register(spec, callerCtx),
  }
