...
export function createSessionApi({ ctx, sessions, eventsApi, dshSession, logger } = {}) {
  ...
  return {
    ...
    // The standard projection subscription entry: on/once merge into one
    // subscription surface (the official once variant stays reachable through
    // the underlying event naming until the cutover removes the old paths).
...
export function mountSessionFeature({ ctx, service, logger, featureRegistry } = {}) {
  ...
  // `append` keeps the leaf's source-absent behavior because the official
  // runtime exposes append only on Session instances.
