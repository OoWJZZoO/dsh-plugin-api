...
export function createSessionApi({ ctx, sessions, eventsApi, dshSession, logger } = {}) {
  ...
  return {
    ...
    observe(subject) {
      // The event-face entry answers the discriminated envelope; the session
      // observe entry keeps answering the projection handle itself. The
      // subject is the same single argument every other observation entry
      // takes — `{ name, ...options }` in the canonical options form (the
      // former trailing options ride the subject), or the bare event name as
      // the convenience form. A listener is never an argument here: it
      // attaches through the returned handle's `subscribe`.
      const spec = subject !== null && typeof subject === 'object' && !Array.isArray(subject)
        ? subject
        : { name: subject }
      const observed = eventsApi.observe(spec)
      return observed?.handle ?? observed
    },
