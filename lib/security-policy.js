 ...
 *   to the point default, tolerates missing provenance
 ...
 *
 * Point defaults (the outcome a point falls back to when nothing was
 * consulted and when a single policy is degraded):
 * - approval-context / tool-before / model-request-before = ask (fall through
 *   to the official flow unchanged);
 * - redaction = noop;
 * - egress = allow. The egress face is a denylist: an outbound target is
 *   blocked only by an explicit deny policy, so an empty registry reproduces
 *   the official outbound behavior exactly. A degraded (throwing or malformed)
 *   egress policy therefore also falls back to allow, matching the empty
 *   registry baseline rather than inventing a deny.
 */
...
/** Face validator: egress target match + allowance decision (denylist default). */
...
export function createSecurityRegistry({ validateSpec, point, now = Date.now, rng, onPolicyError } = {}) {
  ...
  const register = (ownerId, spec) => {
    ...
    return { id: policyId, ownerId: owner, generation, dispose }
  }
   ...
   * consulted (seams keep the official flow unchanged; the egress face then
 * falls back to its allow default, i.e. the official outbound behavior).
   */
