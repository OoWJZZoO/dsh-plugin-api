 ...
 *
 * The control handle is an operation handle: `dispose()` is the operation-class
 * disposer — it answers `requested` when this call handed the teardown to the
 * official run and `stale` afterwards. Disposing never adjudicates the
 * terminal: the official run result stays the single terminal source.
 */
...
export function createWorkflowRunHandle({ run, ownerId, subscribeFeed }) {
  ...
  return createOperationHandle({
    ...
    requestStop: () => {
      ...
      try {
        const teardown = run.dispose()
        // The official teardown is asynchronous; nothing waits on it here, so a
        // rejection must not surface as an unhandled rejection.
        if (teardown != null && typeof teardown.then === 'function') teardown.then(undefined, () => {})
      } catch {
        // the official run teardown is best-effort
      }
      return true
    },
  })
