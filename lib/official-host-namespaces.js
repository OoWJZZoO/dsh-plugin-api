...
function createOfficialLlmLeaf(input = {}, secondary) {
  ...
  const lifecycle = createLifecycle(input, input.feature ?? 'llm')
...
function createOfficialAgentLeaf(input = {}, secondary) {
  input = normalizeInput(input, secondary)
  const lifecycle = createLifecycle(input, input.feature ?? 'agent')
  ...
  Object.defineProperty(api, 'options', {
    ...
    get() {
      ...
      if (snapshot === undefined) throw new PluginApiFeatureDisabledError('agents')
      return snapshot
    },
  })
  return finishLeaf(api, lifecycle)
...
function createSessionMethodMember(lifecycle, store, input, name) {
  ...
  const feature = input.feature ?? 'session'
...
function createOfficialSessionLeaf(input = {}, secondary) {
  input = normalizeInput(input, secondary)
  const lifecycle = createLifecycle(input, input.feature ?? 'session')
...
function createOfficialSystemPromptLeaf(input = {}, secondary) {
  input = normalizeInput(input, secondary)
  const lifecycle = createLifecycle(input, input.feature ?? 'systemPrompt')
...
function createOfficialSettingsLeaf(input = {}, secondary) {
  input = normalizeInput(input, secondary)
  const lifecycle = createLifecycle(input, input.feature ?? 'settings')
  const api = {}
...
/**
 * Build the migrate-target passthrough faces (`services.<key>`) for the
 * registry migrate rows: audited official members ride under the services
 * namespace. Each face reuses the official leaf builders (member whitelist
 * plus lifecycle), reporting `services.<key>` feature codes; a missing
 * backing service degrades per member with typed unavailable errors and
 * never disables the namespace.
 *
 * @param {{ ctx?: object, active?: boolean|(() => boolean), llmPublicExports?: object,
 *           dshSession?: object, toolAbortedError?: () => unknown, recoveryAdapters?: object }} options
 */
export function createServicesMigrateLeaves({
  ctx,
  active = () => true,
  llmPublicExports,
  dshSession,
  ...
  const feature = (key) => `services.${key}`
  const service = (name) => {
    try {
  ...
  const leaves = {}
  leaves.llm = createOfficialLlmLeaf({
    llm: service('llm'),
    publicExports: llmPublicExports,
    active,
    feature: feature('llm'),
  }).api
  leaves.agents = createOfficialAgentLeaf({
    agents: service('agents'),
    active,
    feature: feature('agents'),
  }).api
  leaves.sessions = createOfficialSessionLeaf({
    sessions: service('sessions'),
    session: dshSession,
    active,
    feature: feature('sessions'),
  }).api
  const settingsLeaf = createOfficialSettingsLeaf({
    ctx,
    active,
    feature: feature('settings'),
  }).api
  // services.settings carries exactly the three registry migrate rows; the
  // root-retained update/replace/mutate members are not passthrough members.
  leaves.settings = Object.freeze(Object.defineProperties(
    Object.create(null),
    {
      writable: Object.getOwnPropertyDescriptor(settingsLeaf, 'writable'),
      prepareDocument: Object.getOwnPropertyDescriptor(settingsLeaf, 'prepareDocument'),
      get: Object.getOwnPropertyDescriptor(settingsLeaf, 'get'),
    },
  ))
  leaves.prompts = createOfficialSystemPromptLeaf({
    systemPrompt: service('systemPrompt'),
    active,
    feature: feature('prompts'),
  }).api
  const toolsLifecycle = createLifecycle({ active }, 'services.tools')
  let toolsServicePresent = false
  try {
    toolsServicePresent = typeof ctx?.get === 'function' && ctx.get('tools') != null
  } catch {
    toolsServicePresent = false
  }
  leaves.tools = Object.freeze({
    toolAbortedError() {
      toolsLifecycle.assertAvailable()
      if (!toolsServicePresent) throw new PluginApiFeatureDisabledError('services.tools')
      return toolAbortedError()
    },
  })
  leaves.recovery = Object.freeze({
    fromAgentRequestError: recoveryAdapters?.fromAgentRequestError,
    fromToolResult: recoveryAdapters?.fromToolResult,
    unsupported: recoveryAdapters?.unsupported,
  })
  return leaves
}
