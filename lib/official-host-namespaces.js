...
function createAgentOptionsSnapshot(input, registry) {
  const explicit = hasOwn(input, 'agentOptions')
    ? input.agentOptions
    : hasOwn(input, 'optionsSnapshot')
      ? input.optionsSnapshot
      : undefined

  let raw = explicit
  if (raw === undefined && hasOwn(input, 'agent')) raw = input.agent
  ...
  if (raw !== undefined && raw !== null && typeof raw === 'object' && !hasOwn(input, 'agentOptions') && !hasOwn(input, 'optionsSnapshot')) {
    raw = readMember(raw, 'options')
  }
...
function createSessionMethodMember(lifecycle, store, input, name) {
  ...
  return function targetSessionMember(...args) {
    lifecycle.assertAvailable()
    const storeMethod = readMember(store, name)
    if (typeof storeMethod === 'function') return Reflect.apply(storeMethod, store, args)
    const targetMethod = readMember(fixedTarget, name)
    if (typeof targetMethod === 'function') return Reflect.apply(targetMethod, fixedTarget, args)
    const target = args[0]
    const method = readMember(target, name)
    if (typeof method !== 'function') throw new PluginApiFeatureDisabledError('session')
    return Reflect.apply(method, target, args.slice(1))
  }
}
