/**
 * image admission private scoped image admission gateway — pluginApi.llm.admission runtime.
 *
 ...
 *: AsyncLocalStorage alone cannot prove
 ...
 * confinement it shares the relaxation (the documented relaxation covers the
 ...
 *   the image admission policy registry owned by the admission feature
...
export function installLlmAdmissionGateway({ llm, apiProxy, policySource, agents, logger = {} } = {}) {
  ...
  // Diagnostics must be inert: a missing or throwing logger never changes the
  // admission outcome (semantic-hooks compat diagnostic contract).
  const safeWarn = (message) => {
    try {
      warn(message)
    } catch {
      // a throwing logger is ignored
    }
  }
  ...
  function enterRpcScope(kind, request, original, isActive) {
    ...
    const selected = selectMatchingPolicies(policies, context, (category, error) => {
      safeWarn(`dsh-plugin-api: llm/admission scope ${category}: ${error?.message ?? error}`)
    })
