...
import { createObserverHandle, releasedResult, staleResult } from './contract-kernel.js'
...
/**
 * Face lifecycle states mapped onto the public three-value availability
 * vocabulary. A pending face is `degraded` — the face exists but has not
 * settled — and its own token is kept as the caller-facing `reason`.
 */
const FACE_AVAILABILITY_STATUS = Object.freeze({
  available: 'active',
  degraded: 'degraded',
  pending: 'degraded',
...
/** Owner token used when a client caller identity cannot be resolved. */
const CLIENT_ROOT_OWNER = 'root'
let tokenSequence = 0
let identitySequence = 0
const moduleInvalidationFallbacks = new WeakMap()

/**
 * Resolve the registering client plugin's identity for ownership binding.
 *
 * The identity comes from the caller fiber's loader entry (row name = package
 * identity) and never from a caller-supplied string; an untraceable caller
 * binds to the root token, which is registered as such. This mirrors the host
 * facade's caller derivation on the browser side, where the Cordis tracing
 * proxy rebinds `ctx` to the consuming context.
 */
function callerOwnerOf(callerCtx) {
  try {
    ...
    // An unreadable caller chain falls through to the root token.
  }
...
function createLifecycleController({ active, logger, services, ctx, resolveEvidence, publishDiagnostic } = {}) {
  ...
  /**
   * Standard observation handle over the face set.
   *
   * `current()` reads the same frozen face list as `list()`, `subscribe()` adds
   * one listener that receives change and rebind notifications, and `epoch` is
   * the change epoch captured when the observation is created. The handle is
   * frozen and carries no internal listener table.
   */
  function observeFaces() {
    ...
    const created = createObserverHandle({
      ...
      reportError: (error) => log('dsh-plugin-api client lifecycle observer failed', error),
    })
    try {
      ...
      // The observation never half-binds: a refused subscription releases the
      // handle and rethrows the typed refusal.
  ...
  function forCaller(callerCtx) {
    ...
    const callerApi = Object.freeze({
      ...
      availability: availabilityDescriptor,
      observe() {
        // Caller-bound observation: the handle is the standard one, and the
        // caller fiber's unload releases it exactly as it releases a face.
  ...
  /**
   * Register one lifecycle face.
   *
   * `ownerId` is the binding supplied by the calling layer: the caller-bound
   * face passes the registering plugin's *derived* identity (root when
   * untraceable) and a caller-declared `input.ownerId` is ignored, while the
   * ownerless internal face — which has no caller to derive from — keeps the
   * declared owner as its explicit binding.
   */
  ...
  /**
   * Namespace-level availability: the public three-value vocabulary every
   * namespace answers with. A face query would collide with this member name,
   * so per-face state is read through the handle's own `availability()`
   * extension, which answers the full face snapshot.
   */
  ...
  /** The handle's domain extension member: the full face snapshot. */
  ...
  function readSettingsEvidence(face, base) {
    ...
    if (!services.settingsScope
      || (typeof services.settingsScope.scope !== 'function' && typeof services.settingsScope.bind !== 'function')) {
  ...
  function ensureSettingsEvidence(face) {
    ...
    let scope
    try {
      // The composition may hand in either the official service or the client
      // facade's scope binding; both answer the same spec shape.
      const bindScope = typeof services.settingsScope.scope === 'function'
        ? services.settingsScope.scope
        : services.settingsScope.bind
...
function isSettingsScope(value) {
  // Accepts either the official settings scope service (`bind`) or the client
  // facade's scope binding (`scope`), which is what the composition passes in.
  try {
    return serviceIsActive(value) && value != null
      && (typeof value.scope === 'function' || typeof value.bind === 'function')
  } catch { return false }
}
...
function validateRegistration(input) {
  // Registration failures are typed: the public member is a resourceRegistry
  // entry, so an illegal declaration never escapes as a bare TypeError.
  ...
  if (input.ownerId !== undefined && (typeof input.ownerId !== 'string' || input.ownerId.length === 0)) {
  ...
  if (input.require !== undefined && (!input.require || typeof input.require !== 'object' || Array.isArray(input.require))) throw invalid('require must be an object')
  const require = input.require ?? {}
  if (require.contractVersion !== undefined && typeof require.contractVersion !== 'string') throw invalid('contractVersion must be a string when provided')
