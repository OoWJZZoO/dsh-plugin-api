...
function createLifecycleController({ active, logger, services, ctx, resolveEvidence, publishDiagnostic } = {}) {
  ...
  const api = Object.freeze({
    ...
    registerFace,
    availability,
    onChange(listener) { return subscribe(state.listeners, listener, isActive, log, 'lifecycle change listener') },
    onRebind(listener) { return subscribe(state.rebindListeners, listener, isActive, log, 'lifecycle rebind listener') },
    scan() {
