 ...
 * domain composition and thin-storage rules.
 *
 ...
 * - handle `dispose()` closes the official domain and never deletes data;
 *   `purge()` is the explicit deletion operation (deletes every record
 *   across the domain's tables). The handle is an operation handle whose
 *   destruction verb is `dispose()`; no legacy `close()` alias exists.
 */
...
/** Official storage keeps records across restarts; the domain detail reports it. */
export const STORAGE_DURABILITY = 'durable'

let bindingEpoch = 0

/**
 * Derive the owner-local unit label from the calling plugin's identity.
 *
 * The label is a stable, lowercased projection of the loader entry the caller
 * runs under, so two plugins can never collide on a unit name by construction
 * and a caller cannot claim another plugin's namespace by passing a string.
 *
 * @param {object | undefined} callerCtx - the caller context stamped by the surface.
 * @returns {string} a label matching {@link STORAGE_UNIT_NAME_RE}.
 */
function unitOwnerOf(callerCtx) {
  const raw = String(callerIdentityOf(callerCtx) ?? 'root').toLowerCase()
  const normalized = raw.replace(/[^a-z0-9_]+/g, '_').replace(/^_+|_+$/g, '')
  const seeded = normalized.length > 0 ? normalized : 'root'
  return /^[a-z]/.test(seeded) ? seeded : `o_${seeded}`
}
 ...
 */
function validateOpen(input, owner) {
  const bad = (reason) => ({ ok: false, reason })
  if (!isObject(input)) return bad('open input must be an object')
  const { scope, schema, version, name, tables, global } = input
...
export function createDisabledStorageApi(active = () => false, reason = 'official storage domain facility is unavailable') {
  ...
  // The disabled face carries the same detail field set as the active face
  // (including its epoch), so a probe reads one shape across both states.
  ...
  return Object.freeze({
    ...
    availability: () => Object.freeze({
      status: 'unavailable',
      reason,
      scope: Object.freeze([...STORAGE_SCOPES]),
...
export function createStorageBinding({ ctx, active = true, logger } = {}) {
  ...
  const open = async (input = {}, callerCtx) => {
    assertActive(isActive)
    // The owner label comes from the calling plugin, never from the input: a
    // caller-supplied `owner` is ignored rather than trusted.
    const validated = validateOpen(input, unitOwnerOf(callerCtx))
    if (!validated.ok) {
    ...
    const dispose = async () => {
      ...
      try {
        ...
        return Object.freeze({ ok: false, code: 'unavailable', reason: `storage dispose failed: ${error instanceof Error ? error.message : String(error)}` })
      }
      return Object.freeze({ ok: true, code: 'disposed', envelope: validated.envelope })
    }
    const purge = async () => {
      assertActive(isActive)
      try {
    ...
    return Object.freeze({
      ...
      code: 'opened',
      envelope: validated.envelope,
      handle: Object.freeze({ dispose, purge, domain }),
    })
  }
