...
/**
 * Internal cross-package seam: the host attention hub publishes its whole-hub
 * redacted snapshot seed under this symbol (the same family as the client-side
 * `dsh-plugin-api.attention.runtime` key). The seed bypasses the caller scope
 * by design — it is the host-side forwarding source — so it must never be a
 * public member; the api-remotes replacement resolves it through this key.
 */
export const ATTENTION_SNAPSHOT_SEED_KEY = Symbol.for('dsh-plugin-api.attention.snapshot-seed')
