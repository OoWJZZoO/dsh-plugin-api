 ...
 * One authority commits exactly one terminal
  ...
  const api = {
    ...
    restore: async (checkpointId, { plan, signal } = {}, caller = {}) => {
      ...
      if (!stopped.ok) {
        ...
        return deepFreeze({ ok: false, code: stopped.code === 'aborted' ? 'aborted' : 'denied', reason: stopped.reason, operation: api.handleFor(operation.id), observedAt: clock().toISOString() })
      }
      ...
      // Staging is awaited end to end, so the terminal is normally already
      // committed here: the success code answers `completed`, and `accepted`
      // remains the honest code for a result whose terminal is not yet
      // decidable. The terminal itself is always read through
      // `operation.status()`, never faked onto the result.
      const code = operation.terminal === null ? 'accepted' : 'completed'
      return deepFreeze({ ok: true, code, operation: api.handleFor(operation.id), observedAt: clock().toISOString() })
    },
      ...
      return createOperationHandle({
        ...
        extensions: {
          ...
          observe: (listener) => {
            ...
            let subscribed = true
            try {
              // First delivery is the current state; later deliveries are the
              // operation's own stage/terminal notifications.
              listener(api.statusOf(operationId).operation)
            } catch {
              // immediate delivery containment: one broken listener never breaks the rest
            }
            return () => {
        ...
        requestStop: () => {
          // The stop request is the only thing dispose does: it never forges a
          // terminal, and a settled operation has nothing left to stop.
          if (operation.terminal !== null) return false
          try {
