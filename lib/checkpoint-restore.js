...
export function createRestoreAuthority({ store, planner, facts, stepAdapters = {}, coordination, ownerOf, idFactory, now, waitForTerminal, audit, logger } = {}) {
  ...
  const runSteps = async ({ operation, plan, fence, signal }) => {
    ...
    for (const slice of plan.slices ?? []) {
      for (const step of slice.steps ?? []) {
        if (signal?.aborted) {
          return { ok: false, code: 'aborted', reason: 'cancelled before the next step', stoppedAt: slice.steps.length - done.length }
        }
        ...
        // after stop-then-restore (Requirement 10 AC4): the branch authority
  ...
  const api = {
     ...
     * preconditions before any step executes (Requirement 6 AC2).
     */
