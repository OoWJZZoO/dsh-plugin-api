...
export function createToolDiscoveryEngine(options = {}) {
  ...
  /**
   * Register one catalog entry and answer with the standard resourceRegistry
   * handle `{ id, ownerId, generation, dispose() }`.
   *
   * The entry owner is the caller identity derived from the caller's context
   * (root token when the caller is untraceable): the caller never declares it.
   * The generation is minted here and identifies this occupancy of the entry
   * slot, so a stale disposer can never remove a newer registration.
   *
   * @param {object} spec catalog descriptor spec (`id`, `summary`,
   *   `capabilities`, `activate`, optional `sourceKind` / `toolNames`).
   ...
   * @throws {ToolDiscoveryRegistrationError | ToolDiscoveryEntryConflictError}
   */
  function register(spec, callerCtx) {
    assertActive()
    const owner = callerIdentityOf(callerCtx)
    const normalized = normalizeDescriptorSpec(spec !== null && typeof spec === 'object' ? { ...spec, owner } : spec)
    if (!normalized.ok) {
      reportDiagnostics(owner, `catalog registration rejected: ${normalized.detail}`)
      throw new ToolDiscoveryRegistrationError(normalized.detail)
    }
    const { id, summary, capabilities, sourceKind, toolNames, activate: activateCallback } = normalized.value
    if (entries.has(id)) {
    ...
    return createResourceHandle({
      ...
      ownerId: owner,
      generation: record.generation,
      revoke: makeEntryDisposer(record),
    })
  }
  ...
  function availability(extras = {}) {
    if (!isActive() || tornDown) {
      return deepFreeze({ status: 'unavailable', reason: 'tool discovery is not active' })
    }
    let failed = 0
    let offline = 0
    for (const record of entries.values()) {
    ...
    if (failed > 0 || offline > 0) {
      return deepFreeze({ status: 'degraded', reason: `${failed} failed, ${offline} offline tool entries` })
    }
    return deepFreeze({ status: 'active', ...extras })
  }
