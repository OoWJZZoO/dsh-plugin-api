...
export const CAPABILITY_DESCRIPTORS = Object.freeze([
  ...
  entry('llm.adapters', 'register', ['llm', 'llmAdapters']),
  entry('llm.routing', 'read', ['execRoute', 'sessionRoute', 'routePolicy']),
  entry('agents', 'execute', ['agent']),
  entry('agents.scopes', 'register', ['agent', 'scopedAgentContributions']),
  ...
  entry('sessions.activity', 'read', ['sessionActivity']),
  entry('sessions.planMode', 'mutate', ['sessionPlanMode']),
  entry('sessions.permissionPresets', 'mutate', ['sessionPermissionPresets']),
  entry('credentials', 'mutate', ['credentials']),
  entry('sessions.compaction', 'execute', ['sessionCompaction']),
  entry('sessions.interactions', 'execute', ['sessionInteractions']),
  entry('sessions.selection', 'mutate', ['sessionSelection']),
  entry('workflows', 'execute', ['workflows']),
  entry('sessions.request', 'execute', ['sessionInteraction']),
  entry('sessions.cancel', 'execute', ['sessionInteraction']),
  entry('executions.recovery.checkpoints', 'mutate', ['checkpoints']),
  entry('attention', 'mutate', ['attention']),
  entry('tools', 'register', ['tools']),
  entry('tools.discovery', 'register', ['toolDiscovery']),
  entry('skills', 'register', ['skillsActivation']),
  entry('skills.activation', 'register', ['skillsActivation']),
  ...
  entry('attachments.projection', 'read', ['attachments']),
  entry('mcp', 'read', ['mcp']),
  entry('tasks', 'read', ['tasks']),
  entry('coordination', 'mutate', ['coordination']),
  entry('storage', 'mutate', ['storage']),
  entry('workspaces', 'mutate', ['workspaceTransactions']),
