/**
 * Vendored fork of `@deepseek-ai/dsh-compaction-basic@0.1.0-rc.6` `lib/index.js`
 * (origin: `/usr/lib/node_modules/@deepseek-ai/dsh/node_modules/@deepseek-ai/dsh-compaction-basic/lib/index.js`).
 * MIT License, (c) DeepSeek. This file is the unmodified baseline for the R-class
 * replacement bundle `@deepseek-ai/dsh-plugin-api-compaction-events`. Every R-class
 * semantic increment is marked inline with `// R-class patch:`.
 */
...
* @param trigger - canonical compaction trigger (`pressure`, `context-overflow`,
*   `manual`, or `direct`) carried into every compaction/* payload.
* @param options - bracket owner, stability rule, and optional durability checkpoint.
* @param signal - optional summarization cancellation signal.
* @returns the successful durable compaction result, or `COMPACTION_REJECTED`
*   when a `compaction/request` listener vetoed the transaction.
*/
// R-class patch: sentinel returned when a compaction/request listener vetoes
// the transaction (requirements 5.4, design C4/C5).
const COMPACTION_REJECTED = Symbol("dsh-plugin-api.compaction-events-r1.rejected");

// R-class patch: the sole contract symbol between this auxiliary package and
// the main facade (design D5). Registered on every forked provider instance so
// the main package's catalog guard can detect the replacement without importing
// this package (requirements 1.5 / design C7).
export const COMPACTION_EVENTS_ACTIVE_SYMBOL = Symbol.for("dsh-plugin-api.compaction-events-r1.active");

function safeWarn(ctx, message) {
	try {
		ctx.logger?.warn?.(message);
	} catch { /* logging must never change compaction results */ }
}

// R-class patch: contain every observer dispatch so a listener failure can
// never affect the compaction transaction or the official caller (5.12).
function emitSafe(ctx, name, payload) {
	try {
		ctx.emit(name, payload);
	} catch (error) {
		safeWarn(ctx, `compaction observation listener failed for ${name}`);
	}
}

// R-class patch: policy waterfall (5.2-5.7). Listener throw/reject is contained
// and treated as no decision (5.7); decision interpretation lives in the pure
// event-contract module.
async function dispatchCompactionRequest(ctx, payload) {
	try {
		const raw = await ctx.waterfall("compaction/request", payload, () => undefined);
		return decideCompactionRequest(raw);
	} catch (error) {
		safeWarn(ctx, "compaction/request listener failed; treated as no decision");
		return { kind: DECISION.proceed };
	}
}

// R-class patch: revalidate a replace-range decision against the official
// range-selection invariants (5.5). Any failure falls back to the originally
// selected range (5.6). `expectedOwner` is the open-turn identity captured
// before the asynchronous policy decision.
function revalidateReplacementRange(session, candidateStart, candidateEnd, expectedOwner) {
	try {
		const selection = validateSurfaceRegion(session, candidateStart, candidateEnd);
		const state = inspectCompactionEntryState(session.events);
		if (expectedOwner === null) {
			if (state.openTurn !== null) return null;
		} else {
			if (state.openTurn === null || state.openTurn !== expectedOwner) return null;
		}
		assertNoActiveCompaction(session, "compaction/request replace-range revalidation");
...
function buildObservationRange(selection) {
	return { start: selection.start, end: selection.end };
}

// R-class patch: completed payload WITHOUT the summary body (design D7).
function buildCompletionResult(result) {
	return {
		compactionId: result.compactionId,
		shadowedRange: { start: result.shadowedRange.start, end: result.shadowedRange.end },
		...
		...(result.sourceCommandId === void 0 ? {} : { sourceCommandId: result.sourceCommandId })
	};
}

async function compactSurfaceRegion(dependencies, session, start, end, agent, trigger, options, signal) {
	if (options.owner === null) signal?.throwIfAborted();
	let selection = validateSurfaceRegion(session, start, end);
	...
	// R-class patch: policy waterfall (5.2-5.7) before any durable marker.
	const requestPayload = freezeCompactionPayload({
		agent,
		session,
		trigger,
		range: buildObservationRange(selection),
		...(options.sourceCommandId === void 0 ? {} : { sourceCommandId: options.sourceCommandId })
	}, ["range"]);
	const decision = await dispatchCompactionRequest(dependencies.ctx, requestPayload);
	if (decision.kind === DECISION.reject) {
		const skippedPayload = freezeCompactionPayload({
			agent,
			session,
			trigger,
			range: buildObservationRange(selection),
	...
	if (decision.kind === DECISION.replaceRange) {
		...
		if (revalidated !== null) {
			selection = validateSurfaceRegion(session, revalidated.start, revalidated.end);
		} else {
			safeWarn(dependencies.ctx, "compaction/request replace-range revalidation failed; using the originally selected range");
		}
	}

	const startEvent = session.append("compaction/start", lifecycle);
	const assertStable = options.stability === "whole-surface" ? assertWholeSurfaceUnchanged : assertSelectedSpanStable;
	// R-class patch: started once before the first summarizer call (5.8).
	let startedEmitted = false;
	let failedEmitted = false;
	emitSafe(dependencies.ctx, "compaction/started", freezeCompactionPayload({
		agent,
		session,
		trigger,
		range: buildObservationRange(selection),
	...
	try {
		...
		// R-class patch: completed once after the durable commit (5.9).
		emitSafe(dependencies.ctx, "compaction/completed", freezeCompactionPayload({
			agent,
			session,
			trigger,
			range: buildObservationRange(selection),
			result: buildCompletionResult(result)
		}, ["range", "result"]));
	} catch (error) {
		failure = {
			error,
		...
		// R-class patch: failed exactly once for any post-start failure (5.10).
		if (startedEmitted && !failedEmitted) {
			failedEmitted = true;
			emitSafe(dependencies.ctx, "compaction/failed", freezeCompactionPayload({
				agent,
				session,
				trigger,
				range: buildObservationRange(selection),
				failure: buildRedactedFailure({ stage: failure.stage, error: failure.error })
			}, ["range", "failure"]));
		}
	}
	...
	// R-class patch: flush/durability-checkpoint failure after a successful
	...
	if (flushFailure !== void 0) {
		if (startedEmitted && !failedEmitted) {
			...
			emitSafe(dependencies.ctx, "compaction/failed", freezeCompactionPayload({
				...
				trigger,
				range: buildObservationRange(selection),
				failure: buildRedactedFailure({ stage: "commit", error: flushFailure })
			}, ["range", "failure"]));
		}
...
var BasicCompactionEngine = class extends CompactionEngine {
	...
	constructor(ctx, config = {}) {
		...
		// this replacement without importing this package (design C3/D5).
		Object.defineProperty(this, COMPACTION_EVENTS_ACTIVE_SYMBOL, { value: true });
	...
	*/
	async compactIfNeeded(agent, trigger, signal) {
		...
		for (let attempt = 0; attempt <= spec.compactionRetries; attempt += 1) {
			...
			// R-class patch: a veto cancels the pressure loop immediately (5.4);
	...
	*/
	// R-class patch: single internal routing point for all four triggers
	// (design C4.1). `manual` keeps the official idle-session bracket owner,
	// selected-span stability and durability checkpoint; the other triggers
	// keep the automatic whole-surface bracket.
	async compactRegionInternal(start, end, agent, trigger, signal, sourceCommandId) {
		if (trigger === COMPACTION_TRIGGERS.manual) {
			return compactSurfaceRegion(this.regionDependencies(), agent.session, start, end, agent, trigger, {
				owner: null,
