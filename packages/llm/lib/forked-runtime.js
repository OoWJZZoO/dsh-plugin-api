/**
 * Faithful fork of `@deepseek-ai/dsh-llm@0.1.0-rc.6` `lib/index.js` (1407
 * lines, installed source) for the replacement bundle
 * `@deepseek-ai/dsh-plugin-api-llm`.
 *
 * The fork preserves every official export and member and adds the adapter
 * decoration lifecycle at the real adapter boundary. The complete difference
 * register is maintained in `OFFICIAL_AUDIT_BASELINE.md` next to this file;
 * no member is renamed, removed, or reinterpreted.
 */
import { createRequire } from "node:module";
import { Service } from "@deepseek-ai/cordis";
import z from "@deepseek-ai/schemastery";
import { MAX_TIMER_DELAY_MS } from "@deepseek-ai/dsh-timeout";
import { runDecorationStream } from './decoration-chain.js';
...
*/
// Attribution identity must stay byte-identical to the official package on the
// wire, so the version is read from the installed official package manifest —
// never from this replacement's own manifest (its full unique version carries
// the API-protocol suffix and is not a wire attribution value).
const { version } = createRequire(import.meta.url)("@deepseek-ai/dsh-llm/package.json");
/**
...
var LlmRuntime = class extends Service {
	...
	/** Decoration hook surface attached by the replacement registry; undefined when decoration is inactive. */
	_decoration = void 0;
	constructor(ctx) {
		super(ctx, "llm");
	}
	...
	*/
	registerAdapter(providers, adapter) {
		...
		const ownership = /* @__PURE__ */ {};
		let released = false;
		const dispose = this.ctx.effect(function* () {
			if (providers.length === 0) throw new LlmError("an adapter must register at least one provider", "INVALID_ADAPTER");
			this.commitRoutes(owned, this.prepareRoutes(providers, adapter, owned), ownership);
			yield () => {
				released = true;
				const providersSnapshot = [...owned];
				for (const provider of owned) this.adapters.delete(provider);
				owned.clear();
				this.emitAdaptersUpdated();
				this._decoration?.onAdapterRoutesDisposed?.(providersSnapshot, ownership);
			};
		...
		handle.replace = (next) => {
			...
			this.commitRoutes(owned, this.prepareRoutes(next, adapter, owned), ownership);
		};
		return handle;
	}
	/**
	...
	*/
	commitRoutes(owned, registrations, ownership) {
		for (const provider of owned) this.adapters.delete(provider);
		owned.clear();
		...
		this.emitAdaptersUpdated();
		this._decoration?.onAdapterRoutesCommitted?.(owned, registrations, ownership);
	}
	/**
	...
	registerConfigurableProviders(entries) {
		...
		*/
		const commit = (candidates) => {
			...
			held = detached;
			this.emitAdaptersUpdated();
			this._decoration?.onDirectoryCommitted?.(detached.map((entry) => ({ ...entry, settingsPath: [...entry.settingsPath] })));
		};
		const dispose = this.ctx.effect(function* () {
			...
			yield () => {
				disposed = true;
				const snapshot = held.map((entry) => ({ ...entry, settingsPath: [...entry.settingsPath] }));
				for (const entry of held) this.directory.delete(entry.provider);
				held = [];
				this.emitAdaptersUpdated();
				this._decoration?.onDirectoryDisposed?.(snapshot);
			};
	...
	*/
	registerModelDiscovery(settingsNs, discover) {
		const dispose = this.ctx.effect(function* () {
			...
			this._decoration?.onDiscoveryCommitted?.(settingsNs);
			yield () => {
				this.discoveries.delete(settingsNs);
				this._decoration?.onDiscoveryDisposed?.(settingsNs);
			};
	...
	*/
	async discoverModels(settingsNs, request) {
		...
		// sent (denylist: an explicit deny blocks the outbound side effect).
	...
	*
	* When the decoration registry is attached, matched decorations wrap the
	* real adapter operation through the ordered chain executor; the official
	* `adapterStream` behavior is unchanged otherwise.
	*/
	async *adapterStream(options, prepared) {
		...
		try {
			...
			const chain = this._decorationChainFor(resolvedOptions, registration);
			if (chain === null) {
				iterator = adapter.stream(this.forAdapter(resolvedOptions, adapter))[Symbol.asyncIterator]();
				yield* _consumeAdapterIterator(iterator, options.signal);
				return;
			}
			const baseInvoke = (nextRequest) => {
				const effective = nextRequest === void 0 ? resolvedOptions : nextRequest;
				try {
					const it = adapter.stream(this.forAdapter(effective, adapter))[Symbol.asyncIterator]();
					return _consumeAdapterIterator(it, options.signal);
				} catch (error) {
					return _failureIterable(adapterFailureChunk(error, options.signal));
				}
			};
			yield* runDecorationStream(chain, baseInvoke);
		} catch (error) {
			yield adapterFailureChunk(error, options.signal);
			return;
		}
	}
	/** Build the decoration chain for one adapter operation; null when inactive. */
	_decorationChainFor(resolvedOptions, registration) {
		try {
			return this._decoration?.chainFor?.({
...
/**
* Consume one official adapter iterator with the official loop semantics:
* iterator errors become one terminal failure chunk, done ends the stream, and
* an abandoned stream closes the iterator. Single source of truth shared by
* the plain path and the decoration chain base invocation.
*/
async function* _consumeAdapterIterator(iterator, signal) {
	let completed = false;
	try {
		while (true) {
			let item;
...
/** One-shot iterable carrying a terminal chunk (used for construction failures). */
async function* _failureIterable(chunk) {
	yield chunk;
}
//#endregion
