...
import { AttachmentPipelineService } from "./pipeline-service.js";
import { ATTACHMENT_CONTRACT_SYMBOL, createAttachmentOwnerToken, markAttachmentRoot } from "./contract.js";
...
try {
	...
	try {
		const officialPackage = require.resolve("@deepseek-ai/dsh-attachment-local/package.json");
		sharp = createRequire(officialPackage)("sharp");
	} catch (error) {
		throw new Error("attachment replacement requires the approved sharp decoder", { cause: error });
	}
}
...
var LocalAttachmentStore = class extends AttachmentStore {
	static Config = z.object({
		...
		maxDurationMs: z.any(),
		maxPipelineBytes: z.any(),
		maxPipelineConcurrency: z.any(),
		maxPipelineDeadlineMs: z.any(),
		decoders: z.any(),
		pipeline: z.any()
	});
	...
	constructor(ctx, config) {
		...
		Object.defineProperty(this, ATTACHMENT_CONTRACT_SYMBOL, { value: true });
		this.ownerToken = createAttachmentOwnerToken();
		if (!markAttachmentRoot(ctx?.root ?? ctx, this.ownerToken)) throw new Error("attachment replacement owner conflict");
		this.pipelineService = new AttachmentPipelineService(ctx, { store: this, config, ownerToken: this.ownerToken });
	}
