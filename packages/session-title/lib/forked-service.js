 ...
 * MIT License, (c) DeepSeek. This file is the replacement replacement baseline for
 ...
 * replacement semantic increment is marked inline with `// replacement patch:`.
 */
import { Service } from "@deepseek-ai/cordis";
import z from "@deepseek-ai/schemastery";
import { z as z$1 } from "zod";
import { assertNever, deepFreeze, isAgentLoopRequest } from "@deepseek-ai/dsh-llm";
// replacement patch: the sole contract symbol shared with the main facade. It is
...
//.
export const SESSION_TITLE_ACTIVE_SYMBOL = Symbol.for("dsh-plugin-api.session-title.contract");
// replacement patch: pure decision/freeze/replacement helpers (see ./event-contract.js).
...
// replacement patch: diagnostics must never change policy or service outcomes.
function safeWarn(ctx, message) {
	try {
		ctx?.logger?.warn?.(message);
	} catch { /* logging must never change title results */ }
}
// replacement patch: dispatch the candidate policy waterfall for one candidate and
// interpret its decision. Synchronous per
// Cordis waterfall semantics; a listener throw is contained and a thenable
// return is treated as no decision with its rejection converged.
...
// replacement patch: lazily resolve the agent whose session.id matches, or undefined
// when the agents service is missing / lookup fails.
...
var SessionTitleService = class extends Service {
	...
	constructor(ctx, config) {
		...
		// replacement patch: stamp the contract symbol so the main facade can detect
		// this replacement without importing this package.
	...
	*/
	async refresh(session, signal) {
		...
		// replacement patch: one policy-aware collection shared by every branch of
		...
		if (registration === void 0 || registration.closing || latest === void 0) {
			...
			// replacement patch: reuse the collected candidate set instead of re-collecting.
		...
		// replacement patch: carry the collected candidate set so the provider branch
		// reuses the same post-policy candidates.
	...
	*/
	...
	onUserMessage(session, event) {
		...
		// replacement patch: one policy-aware scheduling-filter collection. The
		// candidate is only this event if it survived the eligibility policy;
		// cadence uses the same post-policy candidate set.
	...
	async runProvider(session, work, route) {
		try {
			...
			// replacement patch: one policy-aware candidate set reused by both the
			// fallback branch and the provider call; a refresh-attempt collection
			// carried on `work.messages` is not re-collected.
			...
			this.assertCurrent(session, work);
			// replacement patch: with no eligible candidates, skip provider.generate()
			// and reproduce the official no-candidate outcome — no title append,
			// no policy-specific error type.
	...
	// replacement patch: apply one interpreted decision to a candidate. proceed keeps
	// the candidate; exclude removes it without touching the log or durable
	// storage; replace is a single-pass substitution that keeps the
	// original candidate (with a redacted diagnostic) when the reference cannot
	// be resolved; malformed never reaches here (caller normalises it
	// to proceed) but is honoured defensively.
	...
	*/
	...
	// replacement patch: an optional precomputed candidate set lets a caller reuse
	// one policy-aware collection instead of triggering a second one;
	// independent fallback attempts still self-collect.
