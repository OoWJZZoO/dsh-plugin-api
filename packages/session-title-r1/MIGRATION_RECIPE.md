...
> Records `plugin-api-session-title-r1` tasks 7.1–7.3 (requirements §7, design C7).
...
- **7.1 in-repo fixture** — delivered: `packages/session-title-r1/test/migration-anchor-scenario.test.mjs`
...
2. headless smoke passes with the replacement row active (`ctx.sessionTitle`
   carries `Symbol.for('dsh-plugin-api.session-title-r1.active')`);
