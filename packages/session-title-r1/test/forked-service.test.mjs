...
// ---------------------------------------------------------------------------
// Fake Cordis harness for the vendored fork (design C4). Dispatch mode mirrors
...
// ---------------------------------------------------------------------------
// Zero-listener equivalence (req 5.2) and durable shape
// ---------------------------------------------------------------------------

test('zero listeners: fallback derives from the first eligible user message (req 5.2)', async () => {
...
// ---------------------------------------------------------------------------
// Provider face, cadence, and shared post-policy candidate set (req 5.9, 5.3)
// ---------------------------------------------------------------------------

test('refresh reuses ONE post-policy collection for fallback and provider; each candidate dispatched once per attempt (req 5.3/5.9)', async () => {
  const calls = []
  const provider = {
    id: 'test-provider',
...
// ---------------------------------------------------------------------------
// Policy decisions (req 5.4-5.8)
// ---------------------------------------------------------------------------

test('exclude removes the synthetic candidate; fallback and provider see the same post-policy set (req 5.5/5.9)', async () => {
  const calls = []
  const provider = {
    id: 'p',
    automatic: 'all-prompts',
...
test('replace substitutes a referenced message single-pass (req 5.6)', async () => {
  const policy = (payload, next) => {
...
test('replace with an unresolvable reference keeps the original and logs a redacted diagnostic (req 5.6)', async () => {
  const policy = (payload, next) => {
...
test('a malformed decision is treated as no decision with one redacted diagnostic (req 5.4/5.8)', async () => {
  const policy = (payload, next) => {
...
test('a throwing listener is contained as no decision (req 5.8)', async () => {
  const policy = (payload, next) => {
...
test('a thenable return is treated as no decision and its rejection is converged (req 5.8)', async () => {
  const policy = (payload, next) => {
...
test('the first decision in listener order settles the candidate (req 5.7)', async () => {
  const calls = []
  const firstSeen = []
...
// ---------------------------------------------------------------------------
// Empty candidate set (req 5.10) and no-collection no-dispatch (req 5.13)
// ---------------------------------------------------------------------------

test('policy excluding every candidate yields the official no-candidate outcome (req 5.10)', async () => {
  let generateCalls = 0
  const provider = {
    id: 'p',
    automatic: 'all-prompts',
...
test('runProvider with an empty candidate set skips provider.generate (req 5.10 defensive path)', async () => {
  let generateCalls = 0
...
test('no eligible user messages dispatch nothing (req 5.13)', async () => {
  const { ctx, session, service } = makeHarness()
  const result = await service.refresh(session)
  assert.equal(result, undefined)
...
test('rename is policy-free and dispatches nothing (req 5.12/5.13)', async () => {
...
// ---------------------------------------------------------------------------
// Payload immutability (req 5.11) and agent resolution (design C4)
// ---------------------------------------------------------------------------

test('listeners cannot mutate the candidate through the payload (req 5.11)', async () => {
...
test('payload.agent resolves lazily from the agents service; undefined when absent (design C4)', async () => {
