...
test('the invariant companion exports the expected { apply, inject, name } shape', () => {
  ...
  assert.equal(invariant.name, 'session-title-r1-invariant')
})

test('the package declares a resolvable ./invariant export (design C2)', () => {
  assert.equal(pkg.exports['./invariant'], './lib/invariant.js')
})

test('the companion is a no-op (this R-class bundle introduces no new durable event type)', async () => {
  let registered = null
  const ctx = {
    invariants: {
...
test('the auxiliary package remains independent of the main facade (req 1.3)', () => {
  assert.ok(
    !allSourceText().includes('@deepseek-ai/dsh-plugin-api-main'),
    'aux package sources must not import the main facade package',
  )
})
