/**
 * Vendored fork of `@deepseek-ai/dsh-session-title@0.1.0-rc.6` `lib/index.js`
 * (origin: `/usr/lib/node_modules/@deepseek-ai/dsh/node_modules/@deepseek-ai/dsh-session-title/lib/index.js`).
 * MIT License, (c) DeepSeek. This file is the R-class replacement baseline for
 * the bundle `@deepseek-ai/dsh-plugin-api-session-title`. Its public surface
 * (default/named exports, `static inject`, `static Config`, provider lifecycle,
 * durable semantics) is byte-equivalent to the official implementation; every
 * R-class semantic increment is marked inline with `// R-class patch:`.
 */
import { Service } from "@deepseek-ai/cordis";
import z from "@deepseek-ai/schemastery";
import { z as z$1 } from "zod";
import { assertNever, deepFreeze, isAgentLoopRequest } from "@deepseek-ai/dsh-llm";
// R-class patch: the sole contract symbol shared with the main facade. It is
// stamped on every forked service instance so the main package's catalog guard
// can detect this replacement without importing this package
// (requirements 1.5 / design C1).
export const SESSION_TITLE_ACTIVE_SYMBOL = Symbol.for("dsh-plugin-api.session-title-r1.active");
// R-class patch: pure decision/freeze/replacement helpers (see ./event-contract.js).
import { buildSessionTitleCandidatePayload, convergeThenable, decideSessionTitleCandidate, DECISION, redactedDecisionMessage, resolveReplacement, SESSION_TITLE_CANDIDATE } from "./event-contract.js";
...
// R-class patch: diagnostics must never change policy or service outcomes.
function safeWarn(ctx, message) {
	try {
		ctx?.logger?.warn?.(message);
	} catch { /* logging must never change title results */ }
}
// R-class patch: dispatch the candidate policy waterfall for one candidate and
// interpret its decision (requirements 5.3-5.8, design C4/D1). Synchronous per
// Cordis waterfall semantics; a listener throw is contained and a thenable
// return is treated as no decision with its rejection converged (design D1).
function dispatchCandidatePolicy(ctx, payload) {
	let raw;
	try {
		raw = ctx.waterfall(SESSION_TITLE_CANDIDATE, payload, () => undefined);
	} catch (error) {
		safeWarn(ctx, redactedDecisionMessage(DECISION.malformed, error));
		return { kind: DECISION.proceed };
	}
	const decision = decideSessionTitleCandidate(raw);
	if (decision.kind !== DECISION.malformed) return decision;
	if (decision.thenable === true) {
		convergeThenable(raw, (error) => {
			safeWarn(ctx, redactedDecisionMessage(DECISION.malformed, error));
		});
		return { kind: DECISION.proceed };
	}
	safeWarn(ctx, redactedDecisionMessage(DECISION.malformed));
	return { kind: DECISION.proceed };
}
// R-class patch: lazily resolve the agent whose session.id matches, or undefined
// when the agents service is missing / lookup fails (design C4 / R-5.3 note).
function resolveAgentForSession(ctx, session) {
	try {
		const agents = ctx?.get?.("agents");
		if (agents === null || agents === void 0 || typeof agents.list !== "function") return void 0;
		for (const agent of agents.list()) {
			if (agent !== null && typeof agent === "object" && agent.session?.id === session?.id) return agent;
		}
		return void 0;
	} catch { return void 0; }
}
...
var SessionTitleService = class extends Service {
	...
	constructor(ctx, config) {
		...
		// R-class patch: stamp the contract symbol so the main facade can detect
		// this replacement without importing this package (design C3/C1).
		Object.defineProperty(this, SESSION_TITLE_ACTIVE_SYMBOL, { value: true });
	...
	*/
	async refresh(session, signal) {
		...
		// R-class patch: one policy-aware collection shared by every branch of
		// this refresh attempt (requirements 5.3/5.9, design C4).
		const messages = this.collectEligible(session, session.events);
		...
		if (registration === void 0 || registration.closing || latest === void 0) {
			...
			// R-class patch: reuse the collected candidate set instead of re-collecting (design C4).
			const fallback = await this.ensureFallback(session, messages);
			signal?.throwIfAborted();
			return fallback;
		}
		const state = this.stateFor(session);
		const revision = this.supersede(state, "explicit title refresh superseded older generation");
		// R-class patch: carry the collected candidate set so the provider branch
		// reuses the same post-policy candidates (design C4 / requirements 5.9).
	...
	*/
	...
	onUserMessage(session, event) {
		...
		// R-class patch: one policy-aware scheduling-filter collection. The
		// candidate is only this event if it survived the eligibility policy;
		// cadence uses the same post-policy candidate set (design C4).
		const eligible = this.collectEligible(session, session.events, event.seq);
		if (!eligible.some((message) => message.seq === event.seq)) return;
		if (this.get(session)?.source.kind === "user") return;
		const registration = this.registration;
		if (registration !== void 0 && !registration.closing) {
			if (registration.provider.automatic === "all-prompts" || session.header.parentSession === void 0 && eligible.length === 1 && this.get(session) === void 0) {
				const state = this.stateFor(session);
				state.pending = {
					registration,
	...
	async runProvider(session, work, route) {
		try {
			...
			// R-class patch: one policy-aware candidate set reused by both the
			// fallback branch and the provider call; a refresh-attempt collection
			// carried on `work.messages` is not re-collected (design C4 / req 5.9).
			const messages = work.messages ?? this.collectEligible(session, session.events, work.throughSeq);
			await this.ensureFallback(session, messages);
			this.assertCurrent(session, work);
			// R-class patch: with no eligible candidates, skip provider.generate()
			// and reproduce the official no-candidate outcome — no title append,
			// no policy-specific error type (requirements 5.10, design C4).
			if (messages.length === 0) return this.get(session);
	...
	// R-class patch: policy-aware candidate collection (requirements 5.2-5.13,
	...
	collectEligible(session, events, throughSeq) {
		const raw = collectSessionTitleMessages(events, throughSeq);
		if (raw.length === 0) return raw;
		const agent = resolveAgentForSession(this.ctx, session);
		const result = [];
		for (const message of raw) {
			const sourceEvent = events[message.seq];
			const source = sourceEvent?.data?.source;
			const payload = buildSessionTitleCandidatePayload(agent, session, {
				seq: message.seq,
				text: message.text,
				...source === void 0 ? {} : { source }
			});
			const decision = dispatchCandidatePolicy(this.ctx, payload);
			const next = this.applyCandidateDecision(session, message, decision, throughSeq);
			if (next !== void 0) result.push(next);
		}
		return result;
	}
	// R-class patch: apply one interpreted decision to a candidate. proceed keeps
	// the candidate; exclude removes it without touching the log or durable
	// storage (req 5.5); replace is a single-pass substitution that keeps the
	// original candidate (with a redacted diagnostic) when the reference cannot
	// be resolved (req 5.6); malformed never reaches here (caller normalises it
	// to proceed) but is honoured defensively (req 5.4).
	applyCandidateDecision(session, message, decision, throughSeq) {
		switch (decision.kind) {
			...
			case DECISION.replace: {
				const replacement = resolveReplacement(session, decision.message, throughSeq);
				if (replacement === void 0) {
					safeWarn(this.ctx, "session-title/candidate replace reference could not be resolved; keeping the original candidate");
					return message;
				}
				return replacement;
			}
			/* v8 ignore next -- closed-union defensive default */
			default:
				return message;
		}
	}
	...
	*/
	...
	// R-class patch: an optional precomputed candidate set lets a caller reuse
	// one policy-aware collection instead of triggering a second one (design C4);
	// independent fallback attempts still self-collect (design C4 attempt boundary).
	async ensureFallback(session, precomputed) {
		this.assertServiceActive();
		const current = this.get(session);
		if (current !== void 0) return current;
		const [first] = precomputed ?? this.collectEligible(session, session.events);
