...
test('runCli emits the JSON-lines result envelope and maps exit codes', async () => {
  ...
  try {
    ...
    assert.equal(event.result.apiProtocol, '0.5')
...
test('handshake reports the executor build identity from its own manifest', () => {
  const manifest = { name: '@deepseek-ai/dsh-plugin-api-profile-manager', version: '0.1.0-rc.6-0.5', dsh: { api: '0.5' } }
  const result = runHandshake(manifest, undefined)
  assert.equal(result.outcome, 'success')
  assert.equal(result.builtForRuntime, '0.1.0-rc.6')
  assert.equal(result.apiProtocol, '0.5')
  assert.equal(result.version, '0.1.0-rc.6-0.5')
})
...
test('handshake degrades on an invalid own manifest', () => {
  ...
  assert.equal(runHandshake({ version: '0.1.0-rc.6-0.5' }, undefined).outcome, 'error')
})
