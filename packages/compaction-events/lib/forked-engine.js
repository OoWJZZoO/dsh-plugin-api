 ...
 * MIT License, (c) DeepSeek. This file is the unmodified baseline for the replacement
 * replacement bundle `@deepseek-ai/dsh-plugin-api-compaction-events`. Every replacement
 * semantic increment is marked inline with `// replacement patch:`.
 */
...
// this package.
export const COMPACTION_EVENTS_ACTIVE_SYMBOL = Symbol.for("dsh-plugin-api.compaction-events.contract");
...
// replacement patch: contain every observer dispatch so a listener failure can
...
// replacement patch: policy waterfall (5.2-5.7). Listener throw/reject is contained
...
// replacement patch: revalidate a replace-range decision against the official
...
// replacement patch: completed payload WITHOUT the summary body.
...
async function compactSurfaceRegion(dependencies, session, start, end, agent, trigger, options, signal) {
	...
	// replacement patch: policy waterfall (5.2-5.7) before any durable marker.
	...
	// replacement patch: started once before the first summarizer call (5.8).
	...
	try {
		...
		// replacement patch: completed once after the durable commit (5.9).
		...
		// replacement patch: failed exactly once for any post-start failure (5.10).
...
var BasicCompactionEngine = class extends CompactionEngine {
	...
	*/
	// replacement patch: single internal routing point for all four triggers
	//. `manual` keeps the official idle-session bracket owner,
