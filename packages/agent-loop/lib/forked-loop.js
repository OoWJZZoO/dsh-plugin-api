/**
 * Vendored from @deepseek-ai/dsh-agent-loop@0.1.0-rc.6/lib/index.js.
 *
 * The official implementation is retained for the replacement row's service,
 * factory, settings, system-prompt, driver, event, and teardown contract.
 * Replacement changes are marked with "replacement patch" comments.
 * The official package import face remains untouched.
 */
...
var ReactLoopAgent = class {
	...
	runtimeContext;
	// replacement patch: one committed route per session/turn/attempt window.
	routeWindow;
	routeDecision;
	routePolicy;
	constructor(loopCtx, id, options, session) {
		...
		// replacement patch: the service is supplied by the replacement row and
		// is optional so the official-equivalent fallback keeps the old path.
		try {
			const candidate = loopCtx.get?.("routePolicy");
			this.routePolicy = candidate?.[ROUTE_POLICY_ACTIVE_SYMBOL] === true ? candidate : void 0;
		} catch {
	...
	*/
	...
	async turn() {
		...
		// replacement patch: opening a turn creates a new route decision window.
		this.routeWindow = { sessionId: this.session.id, turn, attemptEpoch: "0" };
		this.routeDecision = void 0;
		this.routeFallbackCause = void 0;
		try {
	...
	async step(assembly) {
		...
		while (true) {
			...
			if (finish.kind === "error" || finish.kind === "aborted") {
				...
				this.routeFallbackCause = action.cause ?? action.failure;
				continue;
			}
	...
	*/
	async buildRequest(turn, step, tools, system, boundaryMessages, signal) {
		...
		// replacement patch: evaluate the ordered route owner before the
		// preserved official request waterfall. The owner never creates an
		// execution and only supplies a frozen route proposal.
		let routeDecision;
		if (this.routePolicy?.decide) {
			routeDecision = await this.routePolicy.decide({
				windowKey: this.routeWindow ?? { sessionId: this.session.id, turn, attemptEpoch: "0" },
				attemptId: this.options.attemptId,
				seed: seedConfig,
				priorDecision: this.routeDecision,
				fallbackCause: this.routeFallbackCause,
				signal
			});
			this.routeDecision = routeDecision;
		}
		const routeSeed = routeDecision?.commitState === "success" ? {
			...seedConfig,
			...routeDecision.config,
			provider: routeDecision.provider,
			model: routeDecision.model
		} : seedConfig;
		if (routeDecision?.commitState === "denied") {
			await this.dispatch.waterfall("agent/request", {
				turn,
				step,
				signal,
			...
			signal.throwIfAborted();
			const error = new Error("agent loop: no eligible route");
			error.code = "NO_ELIGIBLE_ROUTE";
			error.routeDecision = routeDecision;
			throw error;
		}
		...
		const committedConfig = routeDecision?.commitState === "success" &&
			(proposedConfig.provider !== routeDecision.provider || proposedConfig.model !== routeDecision.model);
		if (committedConfig) {
			try {
				this.loopCtx.logger?.warn?.("dsh-plugin-api agent-loop route policy: late agent/request route proposal was superseded");
			} catch {}
		}
		const routeConfig = committedConfig ? {
			...proposedConfig,
			provider: routeDecision.provider,
			model: routeDecision.model
		} : proposedConfig;
		...
		let preparedCall;
		try {
			preparedCall = await this.loopCtx.llm.prepareCall(routeConfig, signal);
...
var AgentLoop = class extends Service {
	...
	constructor(ctx, config) {
		super(ctx, "agentLoop");
		// replacement patch: identify the active component owner without
		// replacing the official package import contract.
