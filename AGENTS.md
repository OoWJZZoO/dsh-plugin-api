...
| `plugin-api-m2-integration` | M2 final reconciliation：L2/L4、A9/T10、A11、S2/O8/O13/O14、SV17 与 `pluginApi.routing` execution/session capability plane | Stage4-in-progress | `docs/specs/plugin-api-m2-integration/` | Task4 governance/registration is in progress; Task5 final verification must change this to `delivered`. `pluginApi.routing` 是 service-lifetime stable frozen composite（`ofExecution/current/on/once/wait/availability`）；A9/T10 为 compatibility delegates；47 events / 5 durable kinds / 19 services 互相独立；pre-assembly prepared-route 与 route-conditioned contribution 仅为 C proposal；唯一 full version `0.1.0-rc.6-0.3`，`dsh.api` `0.3`。 |
