...
仓库采用 **spec coding 工作流**：新 feature 必须按 Stage 0–4 推进（Stage 0–2 由 SPEC1 一口气交付，Stage 3 以对抗性审查为门，硬停机点见 §3.2）；已交付 feature 允许按对应 Tasks 维护实现、测试与文档。后续新增或重构仍须遵守本工作流。
...
├── packages/           # 辅助 replacement bundles（compaction-events/、session-title/、mcp/、attachments/、agent-loop/、tool-skill/、workspace/、api-remotes/、client-runtime/）与全量聚合 bundle @deepseek-ai/dsh-plugin-api-full
...
采用 [kevinlin/spec-coding-mcp](https://github.com/kevinlin/spec-coding-mcp) 的 spec-driven 五阶段流程。**Stage 0–2（Goal / Requirements / Design）由 SPEC1 一口气产出并提交，不设人类确认门、不派发对抗性审查，质量把关由人类酌情启动 SPEC2 承担；Stage 3（Tasks）以对抗性审查为门（§3.2），审查通过后直接进入 Stage 4（Execute）由代理自主完成。** 谁来做、要不要审查，按 §3.0.2 的四种工作流模式（SPEC1 / SPEC2 / SPEC3 / ANY）分配可用 Agent 资源；设计取舍服从 §3.0.3 的克制设计原则；多 feature 的批量与并行调度按 §3.0.4 约定；审查、提交、硬停机点与工作区纪律见 §3.2。

运行时工作流由本仓库 skill `spec-coding` 驱动（`.dsh/skills/spec-coding/SKILL.md`）；AGENTS.md 是 constitution，两者冲突时以本文件铁律为准。

### 3.0 最高原则：工程质量优先

- **以工程质量为核心导向**：当交付速度、实现便利与工程质量冲突时，工程质量优先。任何阶段都不得以“先跑起来再说”为由跳过审查门、硬停机点（§3.2）、测试或 fail-safe 约束。
...
新 feature 与维护工作按以下四种模式之一执行。开工前先判定属于哪种模式；判定存疑时，默认按 SPEC1→SPEC3 主线处理。新 feature 的立项（进入任何模式的起点）属于 milestone 范围决策，须有人类指示或已获人类确认的纲领依据（§3.2 硬停机点）。本节的模式分配只约束“谁来做、要不要审查”，不改变 §3.2 的硬停机点、对抗性审查门与提交义务。

- **SPEC1（上层文档，Stage 0–2）**：由高质模型**一口气产出 Goal、Requirements、Design 三份制品并提交**（每 feature 一提交，见 §3.2）。**该模式不设人类确认门，也不派发对抗性审查**；质量把关由人类酌情启动 SPEC2 承担。SPEC1 不产出 Tasks，也不写任何实现代码。多 feature 批量产出时，可把该批制品一并产出、逐 feature 提交，见 §3.0.4。
- **SPEC2（审查纠偏，Stage 0–2）**：专门用于审查与纠偏 SPEC1 已产出的 Goal/Requirements/Design 制品，**仅由人类启动**（用户指出文档问题或要求复审优化时）；代理不得自行派发 SPEC2 冒充质量门。它是**纯自动工作流**：只负责对当前阶段制品自动优化，不设用户确认门——优化完成即直接交付；**仅当遇到阻塞级别问题**（动摇 SPEC1 上游制品、需人类判断的取舍，或无法就地自动排除的矛盾）时才停下并请求人类裁决。它只核对当前阶段制品与上游制品的一致性、不向上溯源，并就地修订当前阶段文档；不得借纠偏推进到下一阶段或写实现代码。多 feature 复审/优化时，主 agent 可并行派发各 feature 的审查子 agent、派发侧不阻塞（结束工具调用待回醒后统一调整），见 §3.0.4。
- **SPEC3（执行，Stage 3–4）**：承接 SPEC1 已交付 Design 之后的全部工作：产出 Tasks（Stage 3），按 §3.2 通过对抗性审查后**直接进入**执行与交付（Stage 4），不再提交用户评审。Stage 4 **不再逐顶层大任务审查，改为全部顶层任务完成并验证后做一次全局终审**（见 §3.2）。
- **ANY（疑难杂症）**：任务范围不限、模型质量不限，用于修正代码逻辑、修复缺陷、更新文档等维护性工作；**通常不用于新增 feature**。ANY 不得改变任何 feature 已交付的 Goal/Requirements 验收边界，也不得夹带 spec 外功能；一旦实际工作触及新增能力或已交付验收边界，必须停下并转回 SPEC1→SPEC3 工作流。完成已授权工作所必需的**工程前置**（构建/重建脚本、校验工具、测试脚手架、仅开发期使用的 devDependency）**不属于「spec 外功能」**，前提是它不改变公共 API 面、不改变已交付验收边界、不引入运行时依赖（§6 的运行时依赖规则仍适用）；超出必要范围的工具链改动仍属夹带。
...
- **边缘情况从简**：边缘情况采用最小可用策略；需要对边缘情况做大量设计时，优先收敛回最小可用方案并记录取舍，不得自行铺开。这条对 SPEC1 尤其重要：最上层的 Goal/Requirements/Design 由它产出，过度设计最容易从那里被放大并向下传导。
- 是否属于过度设计存疑时，默认从简，并在制品或交付报告中记录取舍。

### 3.0.4 多 feature 批量与并行策略（SPEC1 / SPEC2）

当一次工作覆盖**多个 feature** 时，SPEC1 与 SPEC2 在各自单 feature 规则（§3.0.2、§3.2）之上叠加以下调度约定；单 feature（单线）行为保持原样。两条约定都只改变“怎么提交、怎么调度”，不改变各模式的门与边界，也不得夹带实现代码或越出已交付边界。

- **SPEC1（批量产出）**：批量产出多个 feature 的 spec 制品时，该批制品一并连续产出；提交按 §3.2 阶段提交规则执行（每 feature 一提交）。同批制品间的一致性由产出时自查保证；人类可随后启动 SPEC2 对整批复审纠偏。
- **SPEC2（多 feature 评审优化，主 agent 调度并行）**：SPEC2 为**纯自动优化**——被要求对**多个 feature 的制品**做复审/优化时，主 agent 自动完成审查、修订与交付，仅遇**阻塞级别问题**（动摇 SPEC1 上游制品、需人类判断的取舍、无法就地自动排除的矛盾）才停下请求人类裁决。多 feature 场景下主 agent 主要负责**调度**，不再串行逐个 feature 阻塞派审：
  - **按 feature 并行派审，派发侧不阻塞**：为每个 feature 派发一个审查子 agent，各子 agent 只核对对应 feature 当前阶段制品与上游制品的一致性、不向上溯源；全部派发完成后主 agent **直接结束工具调用**、**不阻塞等待**，由运行时的回醒机制在子 agent 返回结论时唤醒主 agent（DSH 即后台派发 + 结果回醒；与 SPEC3 强制 `run_in_background: false` 的阻塞派审相反）。这些子 agent 是 SPEC2 审查本身的按 feature 委托执行，不是对 SPEC2 再叠一层对抗性审查。
  - **回醒后按 feature 线串行调整**：各子 agent 返回意见后，主 agent 再据意见做**实际调整**；**每条 feature 线内部保持串行**（该 feature 的审查 → 针对意见集中修订 → 实质修订后按需再派审，逐 feature 推进）。并行只发生在“不同 feature 之间”，绝不发生在“同一 feature 的审查与调整之间”。
...
- **硬停机点（代理永不自主决定，必须请求人类裁决）**：
  - **版本与发布决策**：步进或解冻 §3.0.1 冻结的任何版本字段、对外发布、公开 profile 分发。
  - **milestone 范围增减**：新 feature 立项、从 milestone 移出、验收边界的重大改变。
  - 以下一切门与代理自主权都以不触及这两点为前提；工作区外动作（§2.1）与官方文件约束（§2）另行独立生效。
- **Stage 0–2（Goal / Requirements / Design）**：不设人类确认门、不派发对抗性审查。SPEC1 一口气产出三份制品并按阶段提交规则提交；人类可随时启动 SPEC2 复审纠偏（§3.0.2），代理不得自行派发 SPEC2 冒充质量门。
...
- **阶段提交（强制）**：每个大于 `0` 的 Stage 完工后（Stage 0–2 以 SPEC1 交付三份制品为完工，每 feature 一提交；Stage 3 以对抗性审查通过为完工），代理**必须**在承载该阶段成果的主/集成分支或工作分支创建提交，随后才能进入下一 Stage、派生或更新任何 worktree，或交付该阶段成果。提交只可包含本阶段制品及必要附带改动，且必须先通过 `git diff --check`；若提交失败或存在无法归属的变更，该阶段不得推进，必须先处理并向用户说明。
- **工作区清洁（阶段边界）**：阶段开工时若工作区已有脏改动，**不得默认“保留现状、绕着走”**。这些脏改动通常是上一阶段/上一工作流遗忘的提交；先 `git status`/`git diff` 查明归属——属于上一阶段成果的，与当前阶段成果一并提交；属于当前工作的，纳入本阶段交付一并提交；确实无法归属或与当前工作无关的，立即停下向用户说明，不得擅自丢弃或静默携带。每个阶段完工（含阶段提交）后，工作区必须回到干净状态；不得把未提交成果留在 worktree 里等待下一阶段。
...
- **对抗性审查（按工作流模式执行，模式定义见 §3.0.2）**：
  - **SPEC1（Stage 0–2）**：不派发对抗性审查，也不设人类确认门；三份制品一口气产出并提交，质量把关由人类酌情启动 SPEC2 承担（§3.0.2）。
  - **SPEC2（Stage 0–2 审查纠偏）**：SPEC2 本身就是审查纠偏工作流，仅由人类启动；审查只核对当前阶段制品与上游制品的一致性、不向上溯源，并就地修订当前文档。**SPEC2 是纯自动工作流，不设用户确认门**：优化完成即直接交付，不逐次交用户评审；仅当遇到**阻塞级别问题**（动摇 SPEC1 上游制品、需人类判断的取舍、无法就地自动排除的矛盾）时，停下并请求人类裁决。无需对 SPEC2 再叠一层对抗性审查。涉及多个 feature 时，审查可由主 agent 按 feature 并行派发给审查子 agent，派发侧不阻塞（结束工具调用待回醒后统一调整），但每条 feature 线内部保持串行，见 §3.0.4。
  - **SPEC3（Stage 3）**：Tasks 产出后、进入 Stage 4 前，必须先调用一个子 agent 做对抗性审查，并**阻塞性等待其完成**（在 DSH 工具中必须使用 `run_in_background: false`；禁止以后台/异步方式派审后继续主线）。**仅小修改（如一两处文字或单点修正）无需再对抗性审查**。审查只核对 tasks 与 SPEC1 已交付的 Goal/Requirements/Design 的一致性、并按 `docs/standards/` 适用分册比对 tasks/design 的规范符合性，不向上溯源。返回“无偏差”即通过审查门，**直接进入 Stage 4，不再提交用户评审**；有意见则集中修订，实质修订后再次派审，同样阻塞且串行，直至通过。
  ...
  - **ANY**：不设固定审查门，但代码修改仍须满足 §6 的测试、fail-safe 与不夹带 spec 外功能等约束，且不得越出已交付边界（§3.0.2）。
  ...
  - **再次派审的复用与判据（通用，防无效重复派审）**：
    - **判据先行**：每轮审查收到意见后、决定是否再次派审之前，代理必须先对该轮**实际 diff** 逐条分类（实质 / 非实质，见下两款），并在台账或报告中写明本轮判定；不得沿用上一轮的判定，也不得以「本轮产生了闭合动作」为由默认再派审。
    - **实质修订（触发再次派审的唯一条件）**：仅限——(a) 实现代码（`lib/`、`packages/*/lib/`、`scripts/`、bundle patch）；(b) 测试断言的行为或覆盖范围；(c) registry、`docs/standards/` 分册或上游制品中**改变契约、形状、验收边界或证据口径**的内容；(d) 公共 API 形状、失败语义、版本或装配含义。
    - **非实质修订（就地修完即可继续交付，不得据此再派审）**：错别字与字符、标点、转义、Markdown 表格与空行等排版结构、引证路径或文件名的写法、台账计数与数字回填、措辞与表述顺序——**只要结论本身与证据不变**。判定存疑时按非实质处理并就地修复；但若某条意见其实指向实现或契约（而非文档表述），则按实质处理。非实质修订仍须复跑该交付的机械校验并在报告中如实登记。
    - **复用同一审查子 agent**：确需再次派审时，必须优先用运行时提供的「向既有子 agent 发消息」原语（DSH/ZCode 为 `SendMessage`，以该轮审查子 agent 的 id 为目标）把**本轮审查范围、上一轮意见清单与本轮闭合说明**交给**同一个**子 agent，不得为新一轮另开子 agent；仅当该子 agent 已不可用（会话结束且不可恢复、或其模型规格无法满足本节要求）时才新开，并在派发说明中写明原因。复用其已建立的上下文（已读制品、已跑校验、已确认事实）以提高上下文利用率。**复用不等于放松**：每轮仍须给出该轮的独立复核范围并要求逐条重验，不得以「上轮已确认」为由免验。
    - **历史教训**：M11 batch-2 Stage 4 的第十、十一轮终审发现全部是台账文字与表格格式，代理仍按「闭合 → 再派审」的惯性连续重派了多轮，浪费大量资源。规则原文早已写明「仅小修改无需对抗性审查」，代理的失职在于未对该轮实际 diff 重新判定。
- **Stage 4（Execute）通用规则**：
  - **连续执行纪律（不得因非阻塞因素停下）**：Stage 3 审查门通过后，代理 SHALL 在同一工作序列内完成 `tasks.md` 的全部顶层任务，直至 Stage 4 完成判定（全局终审通过 + 完成提交）。**只有两类因素可以合法停下**：(a) 触及本节硬停机点（版本与发布、milestone 范围）并已按 Req 13.6 登记为阻塞项；(b) 因环境、工具链或权限缺失而确实无法完成的已授权工作（按 §6「阻塞项登记与解除」登记）。**不得**以批次边界、会话长度、上下文占用、任务规模、已交付部分成果等非阻塞因素终止未完成的主体工作。**「阻塞项」的判定标准是代理「无法」完成，而非「尚未」完成**：不得把未完成的主体工作登记为「阻塞项」以规避本纪律；已登记的主体工作按 §6 承接时直接继续执行。
  - 按 `tasks.md` 的顶层大任务/批次顺序执行，一个大批次内的所有子项、实现文件和测试作为整体交付；不得借任务合并掩盖未完成的验收标准。
  - 若执行中发现 spec 错误：实现细节/设计矛盾由代理先修订对应 spec 文档（requirements/design/tasks）保持一致，并在最终报告中列出修订；触及**硬停机点**（版本与发布、milestone 范围）时暂停并请求人类裁决，其余矛盾由代理自主修订并登记。
  - 代理仍需遵守 fail-safe、测试、不夹带 spec 外功能等全部约束；全部任务完成并验证后，先完成全局终审，通过后向用户交付完整结果报告。
- **历史/在途制品衔接**：写入在途 spec 制品的旧门约定与本节冲突时，一律以本节为准；代理在推进在途制品前，应先把其执行注同步为本节规则。
...
- Codex 审查 agent 不得同时承担修复任务。修复只能由主 agent 在收到最终审查结论后执行；**实质修复**后必须重新派出独立只读审查（实质 / 非实质的判据与再次派审的复用方式见 §3.2「再次派审的复用与判据」）。Codex 复用同一审查子 agent 时，该 agent 已按本节要求以 DeepSeek-Flash(max) 与隔离派发创建，故复用不改变其模型规格；**不得复用**通过全量 fork 继承主 agent 上下文的 agent。
...
- **轻量契约路径（小批次小规模并行）**：同一批 feature 数量少、组件边界清晰、且无跨仓库消费者迁移验收时，契约包**无需**落成正式 spec 制品（不必建 `m*-contract` 目录）；在 `temp/` 放一份临时契约文档即可，但内容仍必须覆盖命名规范、共享文件编辑边界、冻结文件、失败呈现与合并顺序，并由任务书分发时引用。临时契约文档**非制品**：不登记、不进入 `docs/specs/`、不单独提交、用毕即删。是否适用该路径由用户按批裁量；该简化只作用于契约载体，**不豁免**任何 Stage 门、R 类硬约束（`docs/standards/capability-strategy.md`）或三阶段协议本身。
...
3. 事件 API 保留 Cordis 的 `ctx.on` + `emit/serial/parallel/waterfall`，只增加稳定类型、只读 payload 与 `priority`（lowest/low/normal/high/highest/monitor）。**订阅权不等于生产权**：canonical 系统事件的派发只授予其 producer authority，`event → producer owner` 的映射由 canonical 事件目录条目承载（registry 的 `producerAuthority`），派发前按调用者身份判定，非 producer 收到 typed `denied` 且事件不被派发，目录中未声明 producer 的条目 fail-closed；门面自身的转译生产路径以内部 owner 身份取得权限。**能力边界变化（M11 交付）**：第三方经门面 `pluginApi.events.emit/serial/parallel/bail/waterfall` 派发 canonical 系统事件由「可用」变为 typed `denied`；第三方自定义事件的唯一受支持路径仍是 owner-scoped 的 `events.define`，其语义不变。权威细则见 `docs/standards/api-idioms.md` §4 与 `composition-and-authority.md` §8。
4. 需要优先“转译”的语义钩子：
...
- **对于尚未走完 Stage 0–3 的新 feature**（Stage 0–2 由 SPEC1 交付、Stage 3 经对抗性审查门），只允许写该 feature 的 `AGENTS.md` 变更与 `docs/specs/**` 制品；不得提前写实现代码。该 feature 的 Tasks 通过对抗性审查门（进入 Stage 4）后，才允许创建或修改 `lib/`、`package.json`、`test/`、`scripts/` 等实现产物；已交付 feature 的维护必须有对应 Tasks、治理变更或 ANY 工作流的明确人类指示作为依据，且不得借维护新增 feature（§3.0.2）。承接**人类裁决或已交付任务书中已登记但未完成的工作**（包括 `temp/` 下 handoff 的「接续项」、spec 文首现状注、交付报告中的后续项）属于 ANY 的明确人类指示，无需再逐项请示，前提是该登记可溯源到具体的人类裁决或已交付任务条目。**代理自行新增的「留待后续 / 下轮处理」条目不构成任何授权**，且必须写明其来源依据；无来源依据的一律作废，不得用作延后理由。
...
- **治理魔法字母不进入实现代码**：`lib/`、`packages/`、`test/` 等实现与测试代码，以及 `package.json`、bundle patch 等实现产物中，不得出现从治理文档（AGENTS.md、`docs/standards/capability-strategy.md`、`docs/specs/**`）泄漏出的分类字母、需求/feature 编号或带治理代号的魔法标识。治理编号只允许存在于 `docs/` 治理/规格制品与本文件的登记溯源文字中。例如以下 token（含等价字符串字面量、行 id、feature 名、Symbol 键、错误文案、包描述、目录/文件名）禁止出现在实现代码里（清单不穷尽，凡属治理编号/后缀/代号同型者一律禁止）：
  ...
  ```
  运行时可见的名字必须使用中立、面向能力/语义的命名（例如包名 `@deepseek-ai/dsh-plugin-api-compaction-events`，行/feature 名只表达能力、不带 `r1` 之类治理后缀）；既有实现若违反本规则，必须在后续维护任务中清理，不得继续新增此类泄漏。
- 临时验证脚本放 `temp/`，用完即删。
...
> 公共 API path 与版本基线发生变化时，除本文件 §4 第 2 条指向的 registry 外，同步面还包括 `docs/specs/plugin-api-features/feature-list.md`、`README.md` 与受影响的 `docs/specs/**` 历史制品；历史制品以文首「公共契约现状注」的形式追加旧 path → 现行 path 映射，不改写其已交付的验收边界。
